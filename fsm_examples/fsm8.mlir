fsm.machine @fsm8() -> () {initialState = "0"} {
	...
	fsm.state @2 output {
		...
		fsm.transition @3 guard {
				%tmp = comb.icmp sle 8 : i16
				fsm.return %tmp
			} action {
				%tmp = comb.mul 1 : i16
				fsm.update x2, %tmp : i16
			}
		fsm.transition @17 guard {
				%tmp = comb.icmp slt x2, 7 : i16
				fsm.return %tmp
			} action {
				%tmp = comb.add x3, 2 : i16
