fsm.machine @fsm0() -> () {initialState = "0"} {
	...
	fsm.state @6 output {
		...
		fsm.transition @2 guard {
				%tmp = comb.icmp ne x0, 1 : i16
				fsm.return %tmp
			} action {
				%tmp = comb.add x3, 3 : i16
				fsm.update x2, %tmp : i16
			}
		fsm.transition @3 guard {
