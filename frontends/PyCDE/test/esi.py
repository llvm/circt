...
class HostComms:
  ...
  commands_from_host = esi.FromServer(types.none)
  commands_to_host = esi.ToServer(types.none)
...
class Commanded:
  ...
  @generator
  def construct(ports):
    (nonechannel, _) = types.channel(types.none).wrap(None, 0)
    HostComms.commands_to_host("go_foo", nonechannel)
