...
class System:
  ...
  output SystemVerilog."""
    ...
    __slots__ = [
        "mod", "top_modules", "name", "passed", "_old_system_token",
        "_op_cache", "_generate_queue", "output_directory", "files",
        "mod_files", "packaging_funcs", "sw_api_langs", "_instance_roots",
        "_placedb", "_appid_index"
    ]
    ...
    def runtime_output_dir(self):
        return self._return_create_if_necessary(self.output_directory /
                                                "runtime")
    ...
    number of generators to run. Useful for debugging. Maybe."""
        i = 0
        with self:
            while len(self._generate_queue) > 0 and (iters is None
                                                     or i < iters):
    ...
    PASS_PHASES = [
        ...
        "builtin.module(lower-esi-bundles, lower-esi-ports, lower-esi-to-hw)",
        ...
        for lang in sw_api_langs:
            ...
            if not services_file.exists():
                raise FileNotFoundError(
                    "Could not locate ESI services description. " +
...
class _OpCache:
    ...
    def __init__(self, module: ir.Module):
        ...
        self._instance_hier_obj_cache: dict[(ir.FlatSymbolRefAttr,
                                             ir.StringAttr),
                                            InstanceHierarchyRoot] = {}
        self._instance_cache: dict[Instance, msft.DynamicInstanceOp] = {}

        self._module_inside_sym_cache: Dict[ir.Operation,
                                            Dict[ir.Attribute,
                                                 ir.Operation]] = {}
        ...
        if self._instance_hier_cache is None:
            ...
            for op in self._module.operation.regions[0].blocks[0]:
                if isinstance(op, msft.InstanceHierarchyOp):
                    self._instance_hier_cache[(op.top_module_ref,
                                               op.instName)] = op
    ...
    the instance doesn't have a static op in the IR."""
        ...
        if inst not in self._instance_cache:
            ref = hw.InnerRefAttr.get(
                ir.StringAttr.get(inst._inside_of_symbol), inst.symbol.symName)
    ...
    Python Instance corresponding to 'op'."""
        ...
        if isinstance(op, msft.DynamicInstanceOp):
            parent_inst = self.get_or_create_inst_from_op(
                op.operation.parent.opview)
            instance_ref = hw.InnerRefAttr(op.instanceRef)
            return parent_inst._children()[hw.InnerSymAttr.get(
                instance_ref.name)]
        raise TypeError(
    ...
    def get_dyn_insts_in_inst(
            self,
            inst: ir.Operation) -> Dict[ir.Attribute, msft.DynamicInstanceOp]:
