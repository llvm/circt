...
def generate_msft_module_op(generator: Generator, spec_mod: _SpecializedModule,
  ...
  with ir.InsertionPoint(entry_block), generator.loc, BackedgeBuilder(), bc:
...
class _SpecializedModule(_PyProxy):
  ...
  def __init__(self,
               orig_cls: type,
               cls: type,
               parameters: Union[dict, ir.DictAttr],
               extern_name: str,
    ...
    assert isinstance(self.parameters, ir.DictAttr)
...
def _get_module_cache_key(func,
  ...
  return (func, ir.Attribute(params))
...
class DesignPartition:
  ...
  def __init__(self, name: str):
    sym_name = ir.StringAttr.get(name)
