...
class ModuleLikeBuilderBase(_PyProxy):
  ...
  variable in `Module`."""
        ...
        for attr_name, attr in self.cls_dct.items():
            ...
            if attr_name == "Attributes":
                self.attributes = {
                    ...
                    for mod_attr in attr if isinstance(mod_attr, tuple)
                }
                self.attributes.update({
                    mod_attr: ir.UnitAttr.get()
                    for mod_attr in attr if not isinstance(mod_attr, tuple)
                })
                continue
    ...
    (more importantly) reduces the amount of bookkeeping necessary."""
        ...
        for idx, (name, port_type) in enumerate(self.inputs):
            proxy_attrs[name] = property(
                lambda self, idx=idx: self._get_input(idx))
    ...
    class GeneratorCtxt:
        ...
        def __init__(self, builder: ModuleLikeBuilderBase,
                     ports: PortProxyBase, ip, loc: ir.Location) -> None:
            ...
            if builder.clocks is not None and len(builder.clocks) == 1:
                ...
                self.clk = ClockSignal(ports._block_args[clk_port],
                                       ClockType())
...
class ModuleBuilder(ModuleLikeBuilderBase):
    ...
    def instantiate(self, module_inst, inputs, instance_name: str):
        ...
        missing = list(
            filter(lambda name: name not in circt_inputs,
                   port_input_lookup.keys()))
        if len(missing) > 0:
            raise ValueError(
                f"Missing input signals for ports: {', '.join(missing)}")
    ...
    `Wire` construct and assign the signal to that wire later on."""
        ...
        for (_, param) in self.sig.parameters.items():
            if param.name == "instance_name":
                ...
                kwargs["instance_name"] = _BlockContext.current(
                ).uniquify_symbol(instance_name)
...
class modparams:
  ...
  previous requirement."""
    ...
    def __init__(self, func: builtins.function):
        ...
        for (_, param) in self.sig.parameters.items():
            if param.kind == param.VAR_KEYWORD:
                raise TypeError(
                    "Module parameter definitions cannot have **kwargs")
            if param.kind == param.VAR_POSITIONAL:
                raise TypeError(
                    "Module parameter definitions cannot have *args")
    ...
    def __call__(self, *args, **kwargs):
        ...
        params = {
            n: v
            for n, v in param_values.arguments.items() if not n.startswith("_")
        }
        ...
        if not issubclass(cls, Module):
            raise ValueError(
                "Parameterization function must return Module class")
