    ...
    def __init__(self, cls: type):
        ...
        for (attr_name, attr) in vars(cls).items():
            if isinstance(attr, Bundle):
                ...
                setattr(
                    self, attr_name,
                    _RequestToServerConnection(self, attr.bundle_type,
                                               attr_name))
            elif isinstance(attr, ServiceDecl.From):
                setattr(
                    self, attr_name,
                    _RequestFromServerConnection(self, attr.bundle_type,
                                                 attr_name))
                ...
                with ir.InsertionPoint.at_block_begin(ports_block):
                    for (_, attr) in self.__dict__.items():
                        if isinstance(attr, _RequestToServerConnection):
                            raw_esi.ToServerOp(
                                attr._name, ir.TypeAttr.get(attr.type._type))
  ...
  server' (to client) connection request."""
    ...
    def __call__(self, appid: AppID):
        ...
        return _FromCirctValue(
            raw_esi.RequestToClientConnectionOp(self.type._type,
                                                self.service_port,
  ...
  built where they are instantiated."""
    ...
    def instantiate(self, impl, inputs: Dict[str, Signal], appid: AppID):
        ...
        if impl.decl is not None:
            decl_sym = ir.FlatSymbolRefAttr.get(
                impl.decl._materialize_service_decl())
...
class PureModuleBuilder(ModuleLikeBuilderBase):
    ...
    will use these.) Store the results for later use."""
        ...
        if len(self.inputs) != 0 or len(self.outputs) != 0 or len(
                self.clocks) != 0:
    ...
    boring."""
        proxy_attrs = {}
        return type(self.modcls.__name__ + "Ports", (PortProxyBase, ),
                    proxy_attrs)
