...
class Signal:
    ...
    `cycles`: number of registers to add."""
        ...
        if clk is None:
            ...
            if clk is None:
                raise ValueError(
                    "If 'clk' not specified, must be in clock block")
    ...
    def name(self) -> Optional[str]:
        ...
        if isinstance(owner, ir.Block) and isinstance(owner.owner,
            ...
            mod_type = hw.ModuleType(
                ir.TypeAttr(owner.owner.module_type).value)
...
class InOutSignal(Signal):
    ...
    def read(self):
        if self.read_value is None:
            self.read_value = _FromCirctValue(
                sv.ReadInOutOp.create(self).results[0])
...
def _validate_idx(size: int, idx: Union[int, BitVectorSignal]):
    ...
    if isinstance(idx, int):
        ...
        if idx.type.width != (size - 1).bit_length():
            raise ValueError(
                "Index must be exactly clog2 of the size of the array")
    else:
  ...
  a user is expected to reapply signedness semantics if needed."""
    ...
    def _exec_cast(self, targetValueType, type_getter, width: int = None):
        if width is not None and width != self.type.width:
            return self.pad_or_truncate(width)._exec_cast(
                targetValueType, type_getter)
    ...
    num_bits`."""
        ...
        with get_user_loc():
            ...
            ret = comb.ExtractOp(0, ir.IntegerType.get_signless(num_bits),
                                 shifted)
            return ret
    ...
    def __and__(self, other):
        from .dialects import comb
        return self.__exec_signless_binop_nocast__(other, comb.AndOp, "&",
                                                   "and")
    ...
    def __xor__(self, other):
        from .dialects import comb
        return self.__exec_signless_binop_nocast__(other, comb.XorOp, "^",
                                                   "xor")
...
class ArraySignal(Signal):
  ...
  """
    ...
    def roll(self, shift, axis=None):
        from .ndarray import NDArray
        return np.roll(NDArray(from_value=self), shift=shift,
                       axis=axis).to_circt()
...
class ChannelSignal(Signal):
    ...
    def buffer(
        ...
        reset: BitsSignal,
        stages: int,
        output_protocol: Optional[ChannelSignaling] = None,
    ) -> ChannelSignal:
    ...
    output of that buffer."""
        ...
        if output_protocol is None:
            ...
            res_type = Channel(inner_type, output_protocol,
                               self.type.data_delay)
    ...
    def transform(self, transform: Callable[[Signal],
...
class BundleSignal(Signal):
    ...
    def unpack(self, **kwargs: ChannelSignal) -> Dict[str, ChannelSignal]:
        ...
        for name, value in kwargs.items():
            ...
            if not bc.channel.castable(value.type):
                raise TypeError(
                    f"Expected channel type {bc.channel}, got {value.type} "
                    f"on channel '{name}'")
        ...
        with get_user_loc():
            unpack_op = esi.UnpackBundleOp(
                [bc.channel._type for bc in to_channels], self.value, operands)
        ...
        if not all(
            [bc.channel.castable(ret[bc.name].type) for bc in to_channels]):
    ...
    def connect(self, other: BundleSignal):
        ...
        from .constructs import Wire
        froms = [(bc.name, Wire(bc.channel)) for bc in other.type.channels
