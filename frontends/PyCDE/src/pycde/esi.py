...
class _HostMem(ServiceDecl):
  ...
  # Burst (list) read request: 'length' selects how many list items to read
  # from the host, returned as a windowed list. Used by 'read_list' (the
  # single-message 'read' has no 'length' field).
  ReadReqBurstType = StructType([
      ("address", UInt(64)),
      ("tag", TagType),
      ("length", UInt(64)),
  ])
  ...
  def read_list(self,
    ...
    'ReadReqBurstType' ({address, tag, length}), where the request's 'length'
    is the number of list items to read starting at 'address'; the items are
    returned as a list. To receive the list, the client requests a *windowed
    channel*: the response is a window over struct{tag, data:
