...
def OneItemBuffersFromHost(client_type: Type):
  ...
  1) Host sends address of buffer address via MMIO write.
  2) Device reads data from said buffer and sends down the channel.
  3) Device writes '1' to the last byte in the buffer.
  """
  ...
  class OneItemBuffersFromHost(esi.EngineModule):
    ...
    def build(ports):
      ...
      client_type_bytes = UInt(64)(client_type.bitwidth // 8)
      ...
      # read_req_valid = Wire(Bits(1))
      # read_resp_ready = Wire(Bits(1))
      # output_valid = Wire(Bits(1))
      # buffer_loc_ready = Wire(Bits(1))
      # write_done_valid = Wire(Bits(1))

      # buffer_loc, buffer_loc_valid = buffer_loc.output.unwrap(buffer_loc_ready)

      # read_req_chan, read_req_ready = Channel(esi.HostMem.ReadReqType).wrap(
      #     {
      #         "address": buffer_loc.as_uint(),
      #         "tag": 0,
      #     }, read_req_valid)

      # read_resp = ports.hostmem_read.unpack(req=read_req_chan)["resp"]
      # read_resp_data, read_resp_valid = read_resp.unwrap(read_resp_ready)

      # ports.output_channel, output_channel_ready = Channel(client_type).wrap(
      #     read_resp_data.data, output_valid)
      # write_done_address = (buffer_loc.as_uint() +
      #                       (client_type.bitwidth // 8)).as_uint(64)
      # write_done_chan, write_done_ready = Channel(
      #     OneItemBuffersFromHost.hostmem_write.type.req).wrap(
      #         {
      #             "address": write_done_address,
      #             "tag": 0,
      #             "data": {
      #                 "valid": 1,
      #                 "client_data": Bits(8)(1),
      #             }
      #         }, write_done_valid)
      # ports.hostmem_write.unpack(req=write_done_chan)

      # # Dont't initiate a read if we've nowhere to put the response.
      # read_req_valid.assign(buffer_loc_valid & output_channel_ready)
      # buffer_loc_ready.assign(read_req_ready & )
