...
class System:
  ...
  def emit_outputs(self):
    self.run_passes()
    circt.export_split_verilog(self.mod, str(self.output_directory))

    # Copy the specified aux files.
    for fname in self.copy_files:
      dest = os.path.join(self.output_directory, os.path.basename(fname))
