...
from pyrtg import test, sequence, target, entry, rtg, Label, Set, Integer, Bag, rtgtest, Immediate, IntegerRegister, Memory, CPUCore, Array, Bool, If, Else, EndIf, For, Foreach
...
@test(("lower", Integer.type()), ("upper", Integer.type()),
      ("step", Integer.type()), ("iter_arg0", Integer.type()),
      ("iter_arg1", Integer.type()))
def test2_for(lower, upper, step, iter_arg0, iter_arg1):
...
# CHECK-NEXT: [[IDX0:%.+]] = index.constant 0
# CHECK-NEXT: [[IDX1:%.+]] = index.constant 1
# CHECK-NEXT: [[ARR:%.+]] = rtg.array_create [[[IDX1]], [[IDX1]], [[IDX1]]] : !rtg.array<3 x index>
# CHECK-NEXT: [[RESULT:%.+]] = scf.for [[ARG0:%.+]] = [[IDX0]] to [[IDX3]] step [[IDX1]] iter_args([[ARG1:%.+]] = [[IDX0]]) -> (index) {
# CHECK-NEXT:   [[VAL1:%.+]] = rtg.array_get %arr0[[[ARG0]]] : !rtg.array<3 x index>
# CHECK-NEXT:   [[VAL2:%.+]] = rtg.array_get [[ARR]][[[ARG0]]] : !rtg.array<3 x index>
...
@test(("arr0", Array.type(Integer.type(), 3)))
