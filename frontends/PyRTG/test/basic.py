...
from pyrtg import test, sequence, target, entry, rtg, Label, Set, Integer, Bag, rtgtest, Immediate, IntegerRegister, Memory, CPUCore, Array, Bool
...
# MLIR-LABEL: rtg.target @Tgt2 : !rtg.dict<memory0: !rtg.isa.memory<32>, memory1: !rtg.isa.memory<32>>
# MLIR-NEXT: [[C0:%.+]] = index.constant 8
# MLIR-NEXT: [[C1:%.+]] = index.constant 2
# MLIR-NEXT: [[MEM0:%.+]] = rtg.isa.memory_alloc [[C0]], [[C1]] : !rtg.isa.memory<32>
# MLIR-NEXT: [[MEM1:%.+]] = rtg.isa.memory_alloc [[C0]], [[C1]] : !rtg.isa.memory<32>
# MLIR-NEXT: rtg.yield [[MEM0]], [[MEM1]] : !rtg.isa.memory<32>, !rtg.isa.memory<32>
# MLIR-NEXT: }
...
class Tgt2:
  ...
  @entry
  def memory0():
    return Memory.declare(size=8, align=Integer(2), address_width=32)
...
# MLIR-NEXT: [[ARR0:%.+]] = rtg.array_create [[[IDX1]], [[IDX2]], [[IDX3]]] : !rtg.array<3 x index>
# MLIR-NEXT: [[RES0:%.+]] = rtg.array_get [[ARR0]][[[IDX2]]] : !rtg.array<3 x index>
# MLIR-NEXT: [[ARR1:%.+]] = rtg.array_create [] : !rtg.array<0 x !rtg.array<5 x index>>
# MLIR-NEXT: [[ARR2:%.+]] = rtg.array_create [[[IDX0]], [[IDX1]], [[IDX2]]] : !rtg.array<3 x index>
# MLIR-NEXT: [[ARR3:%.+]] = rtg.array_create [[[IDX10]], [[IDX11]], [[IDX12]]] : !rtg.array<3 x index>
# MLIR-NEXT: [[ARR4:%.+]] = rtg.array_create [[[ARR2]], [[ARR3]]] : !rtg.array<2 x !rtg.array<3 x index>>
# MLIR-NEXT: [[ARR5:%.+]] = rtg.array_dyn_create [[IDX2]], [[IDX1]] : index
# MLIR-NEXT: [[ARR6:%.+]] = rtg.array_set [[ARR5]][[[IDX1]]], [[IDX3]] : !rtg.array<index>
# MLIR-NEXT: [[SIZE:%.+]] = rtg.array_size [[ARR6]] : !rtg.array<index>
# MLIR-NEXT: rtg.yield [[RES0]], [[ARR1]], [[ARR4]], [[SIZE]] : index, !rtg.array<0 x !rtg.array<5 x index>>, !rtg.array<2 x !rtg.array<3 x index>>, index
...
class Tgt4:
  ...
  @entry
  def arr0():
    arr0 = Array.create(Integer(1), Integer(2), Integer(3))
    return arr0[2]

  @entry
  def arr1():
    return Array.create_empty(Array.type(Integer.type(), 5))

  @entry
  def arr2():
    return Array.from_list(
        [[Integer(y * 10 + x) for x in range(3)] for y in range(2)])

  @entry
  def arr3():
    return Array.create_dynamic(2, Integer(1)).set(1, Integer(3)).size()
...
# MLIR-LABEL: rtg.test @test6_memories(mem = {{%.+}}: !rtg.isa.memory<32>)
...
# MLIR-NEXT: [[BASE:%.+]] = rtg.isa.memory_get_base_address %mem : !rtg.isa.memory<32>
# MLIR-NEXT: rtgtest.rv32i.auipc [[REG]], [[BASE]] : !rtg.isa.immediate<32>
# MLIR-NEXT: rtg.test.failure


@test(("mem", Memory.type(32)))
def test6_memories(mem):
