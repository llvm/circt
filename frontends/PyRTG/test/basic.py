...
from pyrtg import test, sequence, config, Config, Param, rtg, Label, Set, Integer, Bag, rtgtest, Immediate, IntegerRegister, Array, Bool, MemoryBlock, Memory, Tuple, embed_comment
...
class Tgt2(Config):
  ...
  mem_blk = Param(loader=lambda: MemoryBlock.declare(
      size=64, base_address=0, address_width=32))
...
class Tgt4(Config):
  ...
  arr0 = Param(
      loader=lambda: Array.create(Integer(1), Integer(2), Integer(3))[2])
  arr1 = Param(loader=lambda: Array.create_empty(Array.ty(Integer.ty())))
  arr2 = Param(loader=lambda: Array.from_list(
      [[Integer(y * 10 + x) for x in range(3)] for y in range(2)]))
  arr3 = Param(loader=lambda: Array.create(Integer(2), Integer(1)).set(
      1, Integer(3)).size())
