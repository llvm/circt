...
def main():
  ...
  if args.dir is not None:
    ...
    run(args.input, args.dir)
  else:
    with tempfile.TemporaryDirectory() as dir:
      run(args.input, Path(dir))


def run(input: Path, dir: Path):
  # Figure out where the arcilator binary is.
  arcilator = Path(
      subprocess.check_output(["which", "arcilator"],
