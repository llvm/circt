...
def parse_arguments():
    ...
    parser.add_argument(
        "--overlay",
        required=True,
        help="Directory that contains overlay files to symlink.",
    )
    parser.add_argument(
        "--target",
        required=True,
        help="Directory in which to create the fused directory tree.",
    )
    return parser.parse_args()


def _symlink_abs(from_path, to_path):
    os.symlink(os.path.abspath(from_path), to_path)
    ...
    for root, dirs, files in os.walk(args.overlay):
        ...
        for src_entry in os.listdir(os.path.join(args.src, rel_root)):
            if src_entry not in dirs:
                ...
                if not os.path.exists(os.path.join(args.target, relpath)):
                    _symlink_abs(
