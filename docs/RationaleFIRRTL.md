...
and new names are generated at Verilog export time. This has the effect of
renumbering intermediate values and saves on memory during the compilation
process. Names generated by Chisel typically look like `_T_12`, and names
generated by the SCF look like `_GEN_12`. The FIRRTL compiler will not
discard these names if the object has been annotated with `DontTouch`.
