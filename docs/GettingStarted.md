...
the `hw` dialect, that replaces all of the wire names with `foo_<n>`. This example is very basic and is
...
e.g. in `examples/circt-standalone/include/CIRCTStandalone/CIRCTStandalonePasses.td`:
```
def RenameWires
    : Pass<"circt-standalone-rename-wires", "hw::HWModuleOp"> {
  let summary = "Rename all hw.wire operations in a module";
  let description = [{
    This pass walks a hardware module and renames each `hw.wire` operation to
    `foo_<n>`.
  }];
  let dependentDialects = ["::circt::hw::HWDialect"];
}
```    
Once this is added, compile the standalone example. This will generate a base class for your pass following the naming
...
2) Create a new file that contains your pass in the standalone example's pass folder, e.g. in `examples/circt-standalone/lib/CIRCTStandalone`.
...
This file should implement:  
  ...
  ```cpp
  void RenameWires::runOnOperation() {
  ...
  ```
  - The pass constructor and registration are generated from the TableGen description. Include the generated pass header as shown above; no separate constructor declaration is needed.
    
3) Define the generated pass declarations and registration in the pass header, e.g. in `examples/circt-standalone/include/CIRCTStandalone/CIRCTStandalonePasses.h`:
```cpp
...
#include "CIRCTStandalone/CIRCTStandalonePasses.h.inc"
```
4) Make sure to add a test to check your pass, e.g. in `examples/circt-standalone/test/CIRCTStandalone`:
```mlir
...
```
Now re-run cmake and compile the standalone example and you should be able to run your new pass!
By default, every pass is accessible using the `circt-standalone-opt` tool through the name that was defined in the tablegen description,
e.g. `circt-standalone-opt --circt-standalone-rename-wires <file_name>.mlir`.
These passes can also be included in certain compilation pipelines that are well packaged in tools like `circt-opt` when loaded as a plugin.
...
- Pass Source: [CIRCTStandalonePasses.cpp](https://github.com/llvm/circt/blob/main/examples/circt-standalone/lib/CIRCTStandalone/CIRCTStandalonePasses.cpp)
- Pass Test: [pass.mlir](https://github.com/llvm/circt/blob/main/examples/circt-standalone/test/CIRCTStandalone/pass.mlir)
- Standalone Pass Header: [CIRCTStandalonePasses.h](https://github.com/llvm/circt/blob/main/examples/circt-standalone/include/CIRCTStandalone/CIRCTStandalonePasses.h)
- Tablegen Description: [CIRCTStandalonePasses.td](https://github.com/llvm/circt/blob/main/examples/circt-standalone/include/CIRCTStandalone/CIRCTStandalonePasses.td)
- [CMakeLists.txt](https://github.com/llvm/circt/blob/main/examples/circt-standalone/lib/CIRCTStandalone/CMakeLists.txt)
