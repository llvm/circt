...
| usages       | array   | List of strings (matching regex of [A-Za-z0-9]+), each indicates a usage this view is intended for |
...
| directory      | string | Directory where Grand Central outputs go, except the bindfiles  |
| filedirectory  | string | Full path where each bindfile will be written                   |
| filename       | string | Filename relative to the filebasedirectory and usage where each bindfile will be written   |
...
The directory is a base directory in which there will be a subdirectory created for each usage 
in `usages`, i.e. `<directory>/<usage>`

The directory member has no effect on the filename member, i.e., the directory
will not be prepended to the filename. However the `filebasedirectory` member does.
There will be a file created at this location for each usages encountered:
`<filebasedirectory>/<usage>/<filename>`.

Example:
```json
{
  "class": "sifive.enterprise.grandcentral.ExtractGrandCentralAnnotation",
  "basedirectory": "gct-dir",
  "filebasedirectory": "gct-dir",
...
| directory| string | A directory where all the usage subdirectories will be placed            |
| filename | string | A filename relative to the `<directory>/<usage>` where a YAML representation of the interface should be placed |
...
for each usage in usages for each of the ViewAnnotations.
