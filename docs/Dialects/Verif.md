...
To prove that the contract holds, it can be pulled out into a `verif.formal` op along with all ops in its fan-in cone.
...
Once the contract is proven, it can be assumed to hold everywhere.
...
With the contract proven it can be assumed to hold by inlining it everywhere and replacing its results `%z0` and `%z1` with symbolic values.
