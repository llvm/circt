...
To revert to the legacy behavior where all clocks are toggled synchronously at the exact same time every cycle, you can pass the `--sync-clocks` flag to `circt-opt --lower-to-bmc`.
