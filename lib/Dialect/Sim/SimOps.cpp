...
static LogicalResult verifyInProceduralRegion(Operation *op) {
  // Check if we know for sure that the parent is not procedural.
  auto *parentOp = op->getParentOp();

  if (!parentOp)
    return op->emitOpError("must be within a procedural region.");
...
LogicalResult PrintFormattedProcOp::verify() {
  return verifyInProceduralRegion(getOperation());
}

LogicalResult FFlushProcOp::verify() {
  return verifyInProceduralRegion(getOperation());
}
