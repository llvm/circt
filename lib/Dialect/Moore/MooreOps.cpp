...
LogicalResult VariableOp::canonicalize(VariableOp op,
  ...
  for (auto *user : op->getUsers()) {
    ...
    if (auto assignOp = dyn_cast<ContinuousAssignOp>(user)) {
      if (uniqueAssignOp || assignOp.getDst() != op)
        return failure();
...
LogicalResult NetOp::canonicalize(NetOp op, PatternRewriter &rewriter) {
  ...
  // If all users of the net op are reads (besides the unique assignment),
  // directly replace the reads with the net's assigned value.
  if (allUsesAreReads && op.getAssignment()) {
