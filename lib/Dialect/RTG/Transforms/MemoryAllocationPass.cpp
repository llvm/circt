...
void MemoryAllocationPass::runOnOperation() {
  ...
  for (auto &op : *targetOp.getBody()) {
    ...
    APInt baseAddr = memBlock.getBaseAddress();
    APInt size = memBlock.getSize();

    if (failed(
            adjustAPIntWidth(size, baseAddr.getBitWidth(), memBlock.getLoc())))
      return signalPassFailure();

    slot.nextFree = baseAddr;
    slot.maxAddr = baseAddr + size;
  }
  ...
  // Iterate through the test an allocate memory for each 'memory_alloc'
  // operation.
