...
struct Loop {
  Loop(unsigned loopId, Block *header, Block *tail)
      : loopId(loopId), header(header), tail(tail) {}
  ...
  unsigned loopId;
  /// The block that dominates the entire loop.
  Block *header;
  /// The block that contains the back-edge to the header.
  Block *tail;
  /// The body of the loop.
  SmallDenseSet<Block *, 8> body;
  /// Other loops that contain this loop.
  SmallVector<Loop *, 1> parentLoops;
...
static llvm::raw_ostream &operator<<(llvm::raw_ostream &os, const Loop &loop) {
  os << "#" << loop.loopId << " from ";
  loop.header->printAsOperand(os);
  os << " to ";
  loop.tail->printAsOperand(os);
  return os;
}
...
/// Find all blocks that are part of the loop body. These are all blocks
/// starting at the header which may reach the tail.
void Loop::collectBody() {
  body.insert(header);
  SmallVector<Block *, 16> stack;
  stack.push_back(tail);
  while (!stack.empty()) {
    auto *block = stack.pop_back_val();
    if (body.insert(block).second)
      stack.append(block->pred_begin(), block->pred_end());
  }
}
...
bool Loop::match() {
  ...
  for (auto &pred : header->getUses()) {
    ...
    IntegerAttr boundAttr;
    if (pred.getOwner()->getBlock() == tail) {
  ...
  if (auto addOp = indVarNext.getDefiningOp<comb::AddOp>();
    ...
    if (!matchPattern(addOp.getOperand(1), m_Constant(&incAttr)))
      return failMatch("non-constant increment");
...
void Loop::unroll() {
  ...
  for (unsigned trip = 0; trip < tripCount; ++trip) {
    ...
    auto *clonedTail = mapper.lookup(tail);
    ...
    for (auto &blockOperand : llvm::make_early_inc_range(header->getUses()))
      if (blockOperand.getOwner()->getBlock() != tail)
        blockOperand.set(clonedHeader);

    // Update the back-edge in the cloned tail to point to the original loop
    ...
    for (auto *block : orderedBody) {
      ...
      body.insert(newBlock);
      for (auto *parent : parentLoops)
        parent->body.insert(newBlock);
    }
  ...
  for (auto &block : *header->getParent()) {
    ...
    while (true) {
      ...
      if (!body.contains(otherBlock) || !otherBlock->getSinglePredecessor())
        break;
      ...
      branchOp.erase();
      body.erase(otherBlock);
      for (auto *parent : parentLoops)
        parent->body.erase(otherBlock);
      otherBlock->erase();
    }
  }
}
...
void UnrollLoopsPass::runOnOperation(CombinationalOp op) {
  ...
  // Find all loop headers and back edges. If a block has a successor that
  // dominates the block, that successor is a potential loop header and the edge
  // from that tail block to the successor is the back-edge.
  DominanceInfo domInfo(op);
  SmallDenseMap<Block *, unsigned> numTailsPerHeader;
  SmallDenseMap<Block *, unsigned> numHeadersPerTail;
  SmallVector<std::pair<Block *, Block *>> loopCandidates;
  for (auto &block : op.getBody()) {
    for (auto *succ : block.getSuccessors()) {
      if (domInfo.dominates(succ, &block)) {
        LLVM_DEBUG({
          llvm::dbgs() << "- Found candidate loop header ";
          succ->printAsOperand(llvm::dbgs());
          llvm::dbgs() << ", back-edge from ";
          block.printAsOperand(llvm::dbgs());
          llvm::dbgs() << "\n";
        });
        ++numTailsPerHeader[succ];
        ++numHeadersPerTail[&block];
        loopCandidates.push_back({succ, &block});
      }
    }
  }

  // We only support simple loops where there is a single back-edge to the
  // header, and the tail block has a back-edge to a single header. Create a
  // data structure for each loop we can potentially unroll.
  SmallVector<Loop> loops;
  for (auto [header, tail] : loopCandidates) {
    Loop loop(loops.size(), header, tail);

    // We need a unique back-edge to this header block.
    if (numTailsPerHeader.lookup(header) != 1) {
      loop.failMatch("multiple back-edges");
      continue;
    }

    // We need a unique header fomr the tail block.
    if (numHeadersPerTail.lookup(tail) != 1) {
      loop.failMatch("tail block branches to multiple headers");
      continue;
    }

    // Create analyze the loop body and check if it matches the pattern we can
    // unroll.
    loop.collectBody();
    if (loop.match())
  ...
  // Sort the loops by increasing body block count. This will ensure that when
  // we unroll the loops in this order, we'll unroll the smaller inner loops
  // first before we unroll outer loops.
  llvm::sort(loops, [](const auto &a, const auto &b) {
    return a.body.size() < b.body.size();
  });

  // Determine which loops are contained within other loops. This will allow us
  // to unroll inner loops while keeping the metadata of outer loops up-to-date.
  for (unsigned inIdx = 0; inIdx < loops.size(); ++inIdx)
    for (unsigned outIdx = inIdx + 1; outIdx < loops.size(); ++outIdx)
      if (loops[outIdx].body.contains(loops[inIdx].header))
        loops[inIdx].parentLoops.push_back(&loops[outIdx]);
  ...
  LLVM_DEBUG({
    ...
    for (auto &loop : loops) {
      os << "- Loop " << loop << ":\n";
      os << "  - Body: ";
      llvm::interleaveComma(loop.body, os,
                            [&](auto *block) { block->printAsOperand(os); });
      os << " (" << loop.body.size() << " blocks)\n";
      os << "  - Parent loops: ";
      if (loop.parentLoops.empty())
        os << "none";
      else
        llvm::interleaveComma(loop.parentLoops, os,
                              [&](auto *loop) { os << "#" << loop->loopId; });
      os << "\n";
      os << "  - Exit: ";
