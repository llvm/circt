...
bool Deseq::isValidResetValue(Value value) {
  // HACK: Just return true for now. This turns out to be pretty hard to check,
  // since some real-world Verilog examples would make the register reset value
  // be an input port that is then tied to a static value. We have very little
  // chance of checking that here.
  return true;
