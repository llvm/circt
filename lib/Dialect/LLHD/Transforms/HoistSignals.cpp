...
namespace {
/// An operand value on a drive operation. Can represent constant `Attribute`
/// operands, regular `Value` operands, a dont-care value, or a null value.
/// Separately tracks the type of the value to facilitate working with constants
/// and dont-cares.
struct DriveValue {
  ...
  static DriveValue dontCare(Type type) {
    return DriveValue(Attribute{}, type);
  }

  /// Create a `DriveValue` from a non-null constant `TypedAttr`.
  DriveValue(TypedAttr attr) : data(attr), type(attr.getType()) {}
  ...
  DriveValue(Value value) : data(value), type(value.getType()) {
    Attribute attr;
    if (auto *defOp = value.getDefiningOp())
      if (m_Constant(&attr).match(defOp))
        if (isa<IntegerAttr, TimeAttr>(attr))
          data = attr;
  }

  /// Create a `DriveValue` from a `Value` if it is non-null, or an `TypedAttr`
  /// if the value is null.
  DriveValue(Value value, TypedAttr attrIfValueNull)
      : data(value ? Data(value) : Data(attrIfValueNull)),
        type(value ? value.getType() : attrIfValueNull.getType()) {}

  bool operator==(const DriveValue &other) const { return data == other.data; }
  bool operator!=(const DriveValue &other) const { return data != other.data; }

  bool isNull() const { return isa<Value>(data) && !data; }
  bool isDontCare() const { return isa<Attribute>(data) && !data; }
  bool isAttr() const { return isa<Attribute>(data); }
  bool isValue() const { return isa<Value>(data); }
  explicit operator bool() const { return !isNull(); }

  Value getValueOrNull() const { return dyn_cast_or_null<Value>(data); }
  Attribute getAttrOrNull() const { return dyn_cast_or_null<Attribute>(data); }
  Value getValue() const { return cast<Value>(data); }
  Attribute getAttr() const { return cast<Attribute>(data); }
  ...
  void dump(llvm::raw_ostream &os) const {
    if (isNull())
      ...
      os << "dont-care";
    else if (auto value = getValueOrNull())
      os << value;
    else
      os << getAttr();
  }
...
private:
  typedef PointerUnion<Value, Attribute> Data;
  Data data;
  Type type;
  explicit DriveValue(Data data, Type type) : data(data), type(type) {}
};
  ...
  for (auto &block : processOp.getBody()) {
    ...
    for (auto &op : llvm::make_early_inc_range(
      ...
      auto operands = DriveOperands{
          driveOp.getValue(),
          driveOp.getTime(),
          DriveValue(driveOp.getEnable(), trueAttr),
      };
...
void DriveHoister::finalizeDriveSets() {
  ...
  for (auto &[slot, driveSet] : driveSets) {
    ...
    // Determine which of the uniform values are defined outside the current
    // region. We will not have to create process results for these. Instead, we
    // can just use the uniform value directly.
    auto clearIfDefinedInsideRegion = [&](DriveValue &driveValue) {
      if (auto value = driveValue.getValueOrNull())
        if (processOp.getBody().isAncestor(value.getParentRegion()))
          driveValue = DriveValue();
    };
    clearIfDefinedInsideRegion(driveSet.uniform.value);
    clearIfDefinedInsideRegion(driveSet.uniform.delay);
    clearIfDefinedInsideRegion(driveSet.uniform.enable);
  }
...
void DriveHoister::hoistDrives() {
  ...
  auto materialize = [&](DriveValue driveValue) {
    if (auto value = driveValue.getValueOrNull())
      return value;
    auto attr = driveValue.getAttr();
    auto &slot = materializedConstants[attr];
    if (!slot) {
      auto *op = processOp->getDialect()->materializeConstant(
          builder, attr, driveValue.getType(), processOp.getLoc());
      assert(op && "constant could not be materialized");
      slot = op->getResult(0);
    }
    return slot;
  };
