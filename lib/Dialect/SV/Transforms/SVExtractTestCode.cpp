...
void SVExtractTestCodeImplPass::runOnOperation() {
  ...
  // Collect modules according to the hierarchy of the instance graph. Modules
  // are sorted so that modules whose instances are higher in the hierarchy come
  // first. This allows us to process modules "bottom up", so test code is
  // extracted from children before parents. That allows the parents to
  // potentially extract test code from their children.
  SetVector<hw::HWModuleOp> sortedModules;
  for (auto *child : llvm::breadth_first(instanceGraph))
    if (auto hwMod = dyn_cast_or_null<hw::HWModuleOp>(child->getModule()))
      sortedModules.insert(hwMod);

  for (auto &rtlmod :
       llvm::make_early_inc_range(llvm::reverse(sortedModules))) {
