...
llvm::SmallVector<std::pair<mlir::Value, std::string>> valueToName(const llvm::SmallVector<mlir::Value>& values, llvm::SmallVector<std::pair<mlir::Value, std::string>> &currentMap, bool tokenFlag){
...
DotNode createDCNode(Operation &op, llvm::SmallVector<std::pair<mlir::Value, std::string>> &valuesMap){
  if (auto castOp = llvm::dyn_cast<dc::ForkOp>(op))
    return DotNode{.nodeType="fork", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::JoinOp>(op))
    return DotNode{.nodeType="join", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::BranchOp>(op))
    return DotNode{.nodeType="branch", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::MergeOp>(op))
    return DotNode{.nodeType="merge", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::SourceOp>(op))
    return DotNode{.nodeType="source", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::SinkOp>(op))
    return DotNode{.nodeType="sink", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::SelectOp>(op))
    return DotNode{.nodeType="select", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::BufferOp>(op))
    return DotNode{.nodeType="buffer", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::FromESIOp>(op))
    return DotNode{.nodeType="fromESI", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::PackOp>(op))
    return DotNode{.nodeType="pack", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::ToESIOp>(op))
    return DotNode{.nodeType="toESI", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto castOp = llvm::dyn_cast<dc::UnpackOp>(op))
    return DotNode{.nodeType = "unpack", .incoming =valueToName(op.getOperands(), valuesMap, false), .outgoing = valueToName(op.getResults(), valuesMap, true)};
}
...
DotNode createCombNode(Operation &op, llvm::SmallVector<std::pair<mlir::Value, std::string>> &valuesMap){
  if (auto addOp = llvm::dyn_cast<comb::AddOp>(op))
    return DotNode{.nodeType="+", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  ...
  if (auto mulOp = llvm::dyn_cast<comb::MulOp>(op))
    return DotNode{.nodeType="x", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  if (auto muxOp = llvm::dyn_cast<comb::MuxOp>(op))
    return DotNode{.nodeType="mux", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
  ...
  return DotNode{.nodeType="combNull", .incoming=valueToName(op.getOperands(), valuesMap, false), .outgoing=valueToName(op.getResults(), valuesMap, false)};
}

namespace {
/// Emit the dot nodes
struct DCDotPrintPass
    : public circt::dc::impl::DCDotPrintBase<
          DCDotPrintPass> {
    ...
    for(auto [id, n] : llvm::enumerate(nodes)){
      if (n.nodeType == "unpack")
        for (auto ic: n.incoming)
          os <<"token_"<<ic.second<<" -> "<<id<<R"( [label = ")"<<ic.second<<"\"]\n";
    }
