...
LogicalResult HandshakeLowerExtmemToHWPass::wrapESI(
  ...
  auto extMod = hw::HWModuleExternOp::create(
      b, loc, StringAttr::get(ctx, "__" + func.getNameAttr().getValue() + "_hw"), newPortInfo);
  ...
  auto wrapperMod = hw::HWModuleOp::create(
      b, loc, StringAttr::get(ctx, func.getNameAttr().getValue() + "_esi_wrapper"),
      wrapperModPortInfo);
  ...
  auto instance =
      hw::InstanceOp::create(b, loc, extMod, func.getNameAttr().getValue(), instanceArgs);
