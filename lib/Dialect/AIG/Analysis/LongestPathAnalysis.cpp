...
public:
  ...
  Oracle(Context *ctx, Location loc) : ctx(ctx), loc(loc) {
...
LogicalResult LocalVisitor::initializeAndRun() {
  oracle = std::make_unique<Oracle>(ctx, module->getLoc());
...
FailureOr<LocalVisitor *> Oracle::getOrComputeLocalVisitor(Operation *add) {
  assert(add);
