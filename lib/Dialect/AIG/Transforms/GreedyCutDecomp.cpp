...
struct AndInverterOpToCutPattern : public mlir::RewritePattern {
  AndInverterOpToCutPattern(MLIRContext *context)
      : mlir::RewritePattern(AndInverterOp::getOperationName(), 1, context) {}

  LogicalResult matchAndRewrite(Operation *opT,
                                PatternRewriter &rewriter) const override {
    auto op = cast<AndInverterOp>(opT);
...
static aig::CutOp mergeCuts(Location loc, MutableArrayRef<Operation *> cuts,
        ...
        for (auto [i, cut] : llvm::enumerate(cuts)) {
          ...
          for (auto [arg, input] : llvm::zip(
            ...
            if (it != inputsToBlockArg.end()) {
              ...
              arg.replaceAllUsesWith(valueToNewValue.at(input));
            }
          }
  ...
  output.replaceAllUsesWith(cutOp.getResult(0));
  {
  ...
  for (auto oldCut : llvm::reverse(cuts)) {
    auto oldCutOutput =
        cast<aig::CutOp>(oldCut).getBodyBlock()->getTerminator();
    oldCutOutput->erase();
    cutOp.getBodyBlock()->getOperations().splice(
        cutOp.getBodyBlock()->begin(),
        cast<aig::CutOp>(oldCut).getBodyBlock()->getOperations());
...
struct MergeCutPattern : public mlir::RewritePattern {
  MergeCutPattern(MLIRContext *context, unsigned cutLimit)
      : mlir::RewritePattern(aig::CutOp::getOperationName(), 1, context),
        cutLimit(cutLimit) {}
  const unsigned cutLimit;

  LogicalResult matchAndRewrite(Operation *opT,
                                PatternRewriter &rewriter) const override {
    auto op = cast<aig::CutOp>(opT);
    if (op.getNumOperands() >= cutLimit)
      return failure();

    // FIXME: This is really really dumb, but it is just a proof of concept.
...
struct SinkConstantPattern : public mlir::RewritePattern {
  SinkConstantPattern(MLIRContext *context)
      : mlir::RewritePattern(aig::CutOp::getOperationName(), 1, context) {}
  ...
  LogicalResult matchAndRewrite(Operation *opT,
    ...
    newCut.getBodyBlock()->getOperations().splice(
        newCut.getBodyBlock()->begin(), block->getOperations());
