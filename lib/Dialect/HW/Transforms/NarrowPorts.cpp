//===- NarrowPorts.cpp - hw module port narrowing -------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This transform narrows the ports of a hw.module to the required width.
//
//===----------------------------------------------------------------------===//
...
void HWSpecializePass::runOnOperation() {
  ...
  for (auto hwModule : module.getOps<hw::HWModuleOp>()) {
    for (auto arg : hwModule.getPorts()) {
      if (arg.isInput()) {
        // compute demanded bits
        // rather check if only use is a bitselect.
      } else if (arg.isOutput()) {
        // Check if producer is an extension.
      }
    }
  }
...
std::unique_ptr<Pass> circt::hw::createNarrowPortsPass() {
