...
InstanceGraph::InstanceGraph(Operation *operation)
  ...
  for (auto &node : nodes) {
    ...
    if (hwModuleLikeNode && cast<mlir::SymbolOpInterface>(hwModuleLikeNode.getOperation()).getVisibility() == mlir::SymbolTable::Visibility::Public)
...
igraph::InstanceGraphNode *InstanceGraph::addHWModule(HWModuleLike module) {
  ...
  if (cast<mlir::SymbolOpInterface>(module.getOperation()).getVisibility() == mlir::SymbolTable::Visibility::Public)
    entry.addInstance({}, node);
  return node;
}
