...
private:
  void lowerMod(HWMutableModuleLike mod);
  void lowerInstance(HWInstanceLike inst);

  /// Lower an individual port, modifing 'port'. Returns 'true' if the port was
  /// changed.
  bool lowerPort(HWMutableModuleLike mod, hw::PortInfo &port);
...
bool ESITypesPass::lowerPort(HWMutableModuleLike mod, hw::PortInfo &port) {
...
void ESITypesPass::lowerMod(HWMutableModuleLike mod) {
...
void ESITypesPass::lowerInstance(HWInstanceLike inst) {
