...
static Slice decodeList(rtl::ArrayType type, capnp::schema::Field::Reader field,
  ...
  auto ptr = ptrSection.slice(field.getSlot().getOffset() * 64, 64)
                 .name(Twine(field.getName().cStr()) + "ptr");
  ...
  Slice fieldSlice =
      TypeSwitch<Type, Slice>(type)
          ...
          .Case([&](rtl::ArrayType at) {
            return decodeList(at, field, ptrSection);
          });

  fieldSlice.name("fieldBits");
...
Value TypeSchemaImpl::buildDecoder(OpBuilder &b, Value clk, Value valid,
  ...
  for (auto field : st.getFields()) {
    ...
    fieldValues[idx] =
        decodeField(fieldTypes[idx], field, dataSection, ptrSection);
  }
