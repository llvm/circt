...
public:
  ...
  /// Create a "child" service of this service. Used when a service already
  /// exists in the active services table, but a new one wants to replace it.
  /// Useful for cases where the child service needs to use the parent service.
  /// Defaults to calling the `getService` method on `AcceleratorConnection` to
  /// get the global service, implying that the child service does not need to
  /// use the service it is replacing.
