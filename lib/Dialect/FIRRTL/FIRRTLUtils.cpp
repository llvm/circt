...
void circt::firrtl::emitConnect(ImplicitLocOpBuilder &builder, Value dst,
  ...
  // If the src is uninferred, but the destination is not, extend the src.  The
  // src cannot be smaller than the destination, if it is there will be a
  // failure in InferWidths.
  auto dstWidth = dstType.getBitWidthOrSentinel();
  auto srcWidth = srcType.getBitWidthOrSentinel();
  if (srcWidth < 0 || dstWidth < 0 ||
    srcWidth < dstWidth) {
    if (type_cast<IntType>(srcType).isSigned())
      src = SExtOp::create(builder, dstType.getConstType(srcType.isConst()), src);
    else
      src = ZExtOp::create(builder, dstType.getConstType(srcType.isConst()), src);
    srcType = type_dyn_cast<FIRRTLBaseType>(src.getType());
    // Const-cast as needed.  Only src can be const (or both).  This is fragile.
    // async regreset presents it's reset type as non-const, but looks back in
    // dataflow to check that it really is constant.  This needs to be fixed.
    if (srcType.isConst() && !dstType.isConst()) {
      src = ConstCastOp::create(builder, srcType.getConstType(false), src);
      srcType = type_dyn_cast<FIRRTLBaseType>(src.getType());
    }    
    MatchingConnectOp::create(builder, dst, src);
    return;
...
bool circt::firrtl::walkDrivers(FIRRTLBaseValue value, bool lookThroughWires,
  ...
  while (true) {
    ...
    while (true) {
      ...
      // Look through unary ops generated by emitConnect.
      if (isa<PadPrimOp, TailPrimOp, ZExtOp, SExtOp>(op)) {
        auto input = op->getOperand(0);
        auto next = getFieldRefFromValue(input);
        fieldRef = next.getSubField(fieldRef.getFieldID());
        continue;
