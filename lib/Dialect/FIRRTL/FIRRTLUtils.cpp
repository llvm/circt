...
bool circt::firrtl::walkDrivers(FIRRTLBaseValue value,
  ...
  // Pending "continue-from" points produced when a looked-through op fans out
  // to more than one driver operand.  The first driver of each op is traced
  // inline (updating `fieldRef`).  Every additional driver is deferred here as
  // an (original, fieldRef) pair and walked once the worklist drains.
  SmallVector<std::pair<FieldRef, FieldRef>> pending;

  // Reused buffer for the driver operands a looked-through op fans out to.
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (auto blockArg = dyn_cast<BlockArgument>(val)) {
        ...
        // Base case: this is one of the module's input ports.
        if (direction == Direction::In) {
          if (!onTerminal(original, fieldRef))
            return false;
          break;
        }
      ...
      // Every other value is either looked through, skipped, or is a terminal
      // driver.  `lookThrough` decides which via a `WalkDriverLookThrough`
      // outcome and, for the look-through case, which driver operands to
      // continue from.
      drivers.clear();
      auto outcome = lookThrough(fieldRef, drivers);
      if (outcome == WalkDriverLookThrough::Terminal) {
        if (!onTerminal(original, fieldRef))
          return false;
        break;
      }
      // Skip: stop descending this branch without reporting a terminal.  Fall
      // through to the worklist/pending drain below.
      if (outcome == WalkDriverLookThrough::Skip)
        break;

      // Look through this value.  An op that pushes no drivers (e.g. a wire) is
      // driven by connects, so scan its drivers via the worklist.  Otherwise,
      // fan out to every pushed driver operand: trace the first inline (to
      // preserve the fast path) and defer the rest onto `pending`.
      if (drivers.empty()) {
        addToWorklist(original, fieldRef);
        break;
      }
      for (Value input : llvm::drop_begin(drivers)) {
        auto next = getFieldRefFromValue(input);
        pending.emplace_back(original, next.getSubField(fieldRef.getFieldID()));
      }
      auto next = getFieldRefFromValue(drivers.front());
      fieldRef = next.getSubField(fieldRef.getFieldID());
    }

    // Process the next element on the stack.
    while (true) {
      // If there is nothing left in the workstack, either continue from a
      // deferred fan-out driver or, if none remain, we are done.
      if (workStack.empty()) {
        if (pending.empty())
          return true;
        std::tie(original, fieldRef) = pending.pop_back_val();
        break;
      }
