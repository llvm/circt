...
LogicalResult MemOp::canonicalize(MemOp op, PatternRewriter &rewriter) {
  // If memory has known, but zero width, eliminate it.  LowerTypes is run
  // before this, so we can't leave structs sitting around.  Thus we must do the
  // recursion to fix up the ports.
  ...
  for (auto port : op->getResults()) {
    for (auto user : port.getUsers()) {
      ...
      if (sfop.getType().isa<FlipType>()) {
        // Inputs to the memory.  Delete the connect driving the subfield. Since
        // this came from a memory, there should only be connects as users.
        ...
        rewriter.replaceOpWithNewOp<ConstantOp>(
            sfop, sfop.getType().cast<IntType>(), APInt());
      }
    }
  }
  rewriter.eraseOp(op);
