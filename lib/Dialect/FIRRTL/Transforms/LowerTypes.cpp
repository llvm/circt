...
private:
  ...
  // reference to a GlobalRefOp. This new lowered field element will get a
...
void LowerTypesPass::runOnOperation() {
  ...
  auto updateNamepath = [&](GlobalRefOp nla, StringAttr oldNLAname,
                            bool removeNLA) {
  ...
  for (auto nlaToSym : nlaToNewSymList) {
    ...
    if (nlaToSym.newSym) {
      ...
      if (nlaName == prevNLA) {
        ...
        auto newNLAop = theBuilder.create<GlobalRefOp>(
  ...
  for (auto *nlaOp : nlasToRemove) {
    ...
    auto nla = cast<GlobalRefOp>(nlaOp);
