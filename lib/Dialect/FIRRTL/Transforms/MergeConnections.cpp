...
struct MergeConnection {
  ...
  bool peelConnect(MatchingConnectOp connect) {
    ...
    // If dest is derived from mem op or has a ground type, we cannot merge
    // them. If the connect's destination is a root value, we cannot merge.
    ...
    auto getMergedValue = [&](auto aggregateType) {
      ...
      for (auto idx : llvm::seq(0u, (unsigned)aggregateType.getNumElements())) {
        ...
        // If the value is an argument, it is not derived from an aggregate
        // value.
      ...
      if (canUseSourceParent) {
        LLVM_DEBUG(llvm::dbgs()
                       ...
                       << " ,fieldID= " << destFieldRef.getFieldID() << " to "
                       << sourceParent << "\n";);
...
struct MergeConnectionsPass
  ...
  void runOnOp(Op op) {
    ...
    MergeConnection<Op> mergeConnection(op, enableAggressiveMerging);
