...
namespace {
/// Walk the ops in `block` bottom-up, back-to-front order.
template <typename T>
...
static bool isAncestor(Block *block, Block *other) {
  while (other) {
    if (block == other)
      return true;
    other = other->getParentOp()->getBlock();
  }
  return false;
}
...
//===----------------------------------------------------------------------===//
// ModuleLayerSink and CircuitLayerSink
//===----------------------------------------------------------------------===//

namespace {

class CircuitLayerSink;

class ModuleLayerSink {
public:
  static bool run(CircuitLayerSink &globals, FModuleOp module) {
    return ModuleLayerSink(globals, module)();
  }

private:
  ModuleLayerSink(CircuitLayerSink &globals, FModuleOp module)
      : globals(globals), module(module) {}

  bool operator()();
  void moveLayersToBack(Operation *op);
  void moveLayersToBack(Block *block);

  CircuitLayerSink &globals;
  FModuleOp module;
  bool changed;
};
...
class CircuitLayerSink {
public:
  static bool run(InstanceGraph &instanceGraph, CircuitOp circuit) {
    return CircuitLayerSink(instanceGraph, circuit)();
  }

  InstanceGraph &getInstanceGraph() { return instanceGraph; }
  CircuitOp getCircuit() const { return circuit; }
  EffectInfo &getEffectInfo() { return effectInfo; }

private:
  CircuitLayerSink(InstanceGraph &instanceGraph, CircuitOp circuit)
...
bool ModuleLayerSink::operator()() {
  ...
  DemandInfo demandInfo(globals.getEffectInfo(), module);
...
bool CircuitLayerSink::operator()() {
  for (auto *root : instanceGraph) {
    for (auto *node : llvm::post_order_ext(root, visited)) {
      auto *op = node->getModule().getOperation();
      effectInfo.update(op);
      if (auto module = dyn_cast<FModuleOp>(node->getModule().getOperation()))
        changed |= ModuleLayerSink::run(*this, module);
    }
  }
  return changed;
}
...
void LayerSinkPass::runOnOperation() {
  ...
  auto changed = CircuitLayerSink::run(instanceGraph, getOperation());
  if (!changed)
