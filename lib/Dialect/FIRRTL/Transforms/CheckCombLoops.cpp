...
public:
  ...
  // Local implementation of getFieldRefFromValue, returns the Aggregate type
  /// Get the FieldRef from a value. Customized implementation of
  /// getFieldRefFromValue. If `field` is a subfield, then this will return the
  /// base aggregate Value and the corresponding fieldId. This will return
  /// nullptr if thie field is a result of a register and can be ignored.
  FieldRef getFieldRef(Value field) {
    if (field.isa<BlockArgument>() ||
        !isa<SubfieldOp, SubaccessOp, SubindexOp>(field.getDefiningOp()))
  ...
  void getChildren(Value node, Node &inputArg,
    ...
    // Case: Instance port: Add the port fields that have a combinational path
    // in the referenced module, to the children.
    //
  ...
  /// the set and ignore all other fields. This is used for MemOp ports.
  void gatherAggregateLeafs(Value val,
    ...
    // base Value.
    auto baseStorageLocId = storageLocId;
    ...
    storageLocId += valType.getMaxFieldID() + 1;
    auto handleSub = [&](Value res, size_t fieldId, bool isGround) {
      if (isGround) {
    ...
    // Traverse the IR, to get all the lowred fields of the aggregate type. This
    // representation is equivalent to the LowerTypes transformation.
  ...
  LogicalResult processModule() {
    ...
    // Gather all the Ground type Values which can serve as the root ofor a DFS
    // traversal. First use ports as the seed.
    for (auto arg : module.getArguments()) {
      addRoots(arg);
    }
    // Then use Wires, Instances ans Memory results as the seed.
