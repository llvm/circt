...
public:
  MutableNLA(GlobalRefOp nla, CircuitNamespace *circuitNamespace)
  ...
  /// utility.)
  GlobalRefOp applyUpdates(NLATable &nlaTable) {
    ...
    // the original NLA.
    OpBuilder b(nla);
    auto writeBack = [&](StringAttr root, StringAttr sym) -> GlobalRefOp {
      ...
      return b.create<GlobalRefOp>(b.getUnknownLoc(), sym,
...
private:
  ...
  bool doesNLAMatchCurrentPath(GlobalRefOp nla);
...
bool Inliner::doesNLAMatchCurrentPath(GlobalRefOp nla) {
...
void Inliner::run() {
  ...
  for (auto nla : circuit.getBody()->getOps<GlobalRefOp>()) {
