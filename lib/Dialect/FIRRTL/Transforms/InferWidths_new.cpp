...
// 实现FieldRef的operator== !=
bool FieldRef::operator==(const Zero&) const { 
    return false; 
}
bool FieldRef::operator!=(const Zero&) const { 
    return true; 
...
public:
    ...
    friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os, const Term& term) {
        return os << term.coe() << " * [" << term.var().getValue() << " (fieldID: " << term.var().getFieldID() << ")]";
    } 
};
...
public:
    ...
    Terms combine_term(const Term& term) const {
        ...
        // 查找具有相同变量的项
        auto it = std::find_if(result.terms_.begin(), result.terms_.end(),
    ...
    friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os, const Terms& terms) {
        bool first = true;
...
public:
    // 构造函数
    Constraint1(FieldRef lhs_var, int rhs_const, Terms rhs_terms, std::optional<FieldRef> rhs_power)
        : lhs_var1_(lhs_var), rhs_const1_(rhs_const), 
          rhs_terms1_(rhs_terms), rhs_power_(rhs_power) {}
    ...
    void set_rhs_power(const std::optional<FieldRef>& terms) { rhs_power_ = terms; }
    ...
    friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os, const Constraint1& c) {
      os << "[" << c.lhs_var1().getValue() << " (fieldID: " << c.lhs_var1().getFieldID() << ")] >= " << c.rhs_const1();
        if (!c.rhs_terms1().empty()) {
            os << " + " << c.rhs_terms1();
        }
        if (c.rhs_power().has_value()) {
            os << " + 2 ^ [" << c.rhs_power().value().getValue() << " (fieldID: " << c.rhs_power().value().getFieldID() << ")]";
        }
...
public:
    // 构造函数
    Constraint_Min(FieldRef lhs, int const1, int const2, std::optional<FieldRef> fr1, std::optional<FieldRef> fr2)
        : lhs_(lhs), const1_(const1), const2_(const2), 
          fr1_(fr1), fr2_(fr2) {}
    ...
    friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os, const Constraint_Min& c) {
      os << "[" << c.lhs().getValue() << " (fieldID: " << c.lhs().getFieldID() << ")] >= min(" << c.const1();
      if (c.fr1().has_value()) {
          os << " + [" << c.fr1().value().getValue() << " (fieldID: " << c.fr1().value().getFieldID() << ")]";
      }
      
      os << ", " << c.const2();
      if (c.fr2().has_value()) {
          os << " + [" << c.fr2().value().getValue() << " (fieldID: " << c.fr2().value().getFieldID() << ")]";
      }
      os << ")\n";
...
public:
    ...
    // 打印输出
    friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os, const Constraint2& c) {
...
// 定义图类型
using DependencyGraph = boost::adjacency_list<
    boost::vecS, 
    boost::vecS, 
    boost::directedS,
    FieldRef
...
private:
    // 待求解约束集合
    std::unordered_map<FieldRef, std::vector<Constraint1>, FieldRef::Hash> constraints_;
    ...
    // 顶点映射（变量名 → 顶点描述符）
    std::unordered_map<FieldRef, boost::graph_traits<DependencyGraph>::vertex_descriptor, FieldRef::Hash> vertex_map_;

public:
    explicit ConstraintSolver(std::unordered_map<FieldRef, std::vector<Constraint1>, FieldRef::Hash> &constraints,
        DependencyGraph &dependency_graph, std::unordered_map<FieldRef, boost::graph_traits<DependencyGraph>::vertex_descriptor, FieldRef::Hash> &vertex_map) 
        : constraints_(constraints), dependency_graph_(dependency_graph), vertex_map_(vertex_map) {}

    // 获取或添加顶点
    boost::graph_traits<DependencyGraph>::vertex_descriptor get_or_add_vertex(const FieldRef& var) {
    ...
    std::vector<Constraint1> constraints() const {
      ...
      for (const auto& [_, constraints_vec] : constraints_) {
          result.insert(result.end(), 
                        constraints_vec.begin(), 
                        constraints_vec.end());
      }
      return result;
    }

    const std::unordered_map<FieldRef, std::vector<Constraint1>, FieldRef::Hash>& constraints_map() const {
        return constraints_;
    }
    ...
    // 获取解
    const Valuation& solution() const {
        return solution_;
    }
};
...
SCCResult ConstraintSolver::computeSCC() {
    ...
    for (int i = 0, j = result.total_components - 1; i < result.total_components; ++i, --j) {
...
{
    // 创建哈希集合用于快速查找
    std::unordered_set<FieldRef, FieldRef::Hash> targetSet(targetVars.begin(), targetVars.end());
...
std::vector<Constraint1> filterConstraints(
    const std::vector<FieldRef>& targetVars,
    std::unordered_map<FieldRef, std::vector<Constraint1>, FieldRef::Hash>& constraints_map)
{
    std::vector<Constraint1> result;
...
std::pair<Terms, long long> remove_solved(const Valuation& values, const Terms& terms) {
    Terms new_terms;
...
std::vector<Constraint1> remove_solveds(const Valuation& values, const std::vector<Constraint1>& cs) {
    std::vector<Constraint1> result;
...
std::optional<Valuation> merge_solution(
...
Valuation bab(const std::vector<Constraint1>& constraints, const std::vector<FieldRef>& tbsolved);
...
public:
    InferenceMapping(SymbolTable &symtbl,
    ...
    const std::unordered_map<FieldRef, boost::graph_traits<DependencyGraph>::vertex_descriptor, FieldRef::Hash>& vertex_map() const {
        return vertex_map_;
    }
    ...
    // 获取解
    const Valuation& final_solution() const {
...
std::vector<Constraint1> InferenceMapping::list_Constraint_Min(const Constraint_Min& minc) {
...
void InferenceMapping::generateConstraints(Value value) {
  // generate constraints for every unknown width in the type. If this is a Bundle
  // type or a FVector type, we will have to potentially create many constraints.
  unsigned fieldID = 0;
...
void InferenceMapping::generateConstraints(FieldRef lhs, FieldRef rhs, FIRRTLType type) {
  // Fast path.
  if (lhs == rhs)
    return;

  // Co-iterate the two field refs, recurring into every leaf element and generate
  auto fieldID = 0;
  std::function<void(FIRRTLBaseType)> generate = [&](FIRRTLBaseType type) {
    auto width = type.getBitWidthOrSentinel();
    if (width >= 0) {
...
/// generate constraints to ensure `larger` are greater than or equal to `smaller`. 
  ...
  std::function<void(FIRRTLBaseType, Value, Value)> generate =
      ...
      //if (larger && smaller) {
        ...
        /*} else if (auto enumType = type_dyn_cast<FEnumType>(type)) {
          ...
          auto baseType_result = getBaseType(fieldRef_result.getValue().getType());
          auto type_result = hw::FieldIdImpl::getFinalTypeByFieldID(baseType_result, fieldRef_result.getFieldID());
          auto width_result = cast<FIRRTLBaseType>(type_result).getBitWidthOrSentinel();
          LLVM_DEBUG(llvm::dbgs() << "\nlooking at width_result : " << width_result <<"\n\n");
          if (width_result >= 0) { } else if (width_result == -1) 
          { // lhs 是uninferred，generate
            FieldRef fieldRef_input(smaller, fieldID);
            auto baseType_input = getBaseType(fieldRef_input.getValue().getType());
            auto type_input = hw::FieldIdImpl::getFinalTypeByFieldID(baseType_input, fieldRef_input.getFieldID());
            auto width_input = cast<FIRRTLBaseType>(type_input).getBitWidthOrSentinel();
            LLVM_DEBUG(llvm::dbgs() << "\nlooking at width_input : " << width_input <<"\n\n");
            if (width_input >= 0) 
            { // rhs已知
              Constraint1 temp_c(fieldRef_result, static_cast<int>(width_input), {}, std::nullopt);
              LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c << "\n");
              addConstraint(temp_c);
            } else if (width_input == -1) 
            { // rhs含有uninferred，则rhs作为变量生成不等式
              if (is_invalid) { 
                    // 要求invalid变量的位宽严格等于 被声明为invalid的变量的位宽，所以同时添加 lhs>=rhs 和 rhs>=lhs
                    LLVM_DEBUG(llvm::dbgs() << "this one is connected to invalid\n"); 
                    Term t_dest(1, fieldRef_result), t_src(1, fieldRef_input);
                    Constraint1 c_dest(fieldRef_result, 0, {t_src}, std::nullopt), c_src(fieldRef_input, 0, {t_dest}, std::nullopt);
                    LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << c_dest << "\nand : " << c_src << "\n");
                    addConstraint(c_src); addConstraint(c_dest); 
              } else { 
  ...
  std::function<void(FIRRTLBaseType, Value, Value, Value)> generate =
      [&](FIRRTLBaseType type, Value result, Value lhs, Value rhs) {
        ...
        /*} else if (auto enumType = type_dyn_cast<FEnumType>(type)) {
          ...
          auto type_result = hw::FieldIdImpl::getFinalTypeByFieldID(baseType_result, fieldRef_result.getFieldID());
          auto width_result = cast<FIRRTLBaseType>(type_result).getBitWidthOrSentinel();
          if (width_result >= 0) { } else {
            // uninferred
            FieldRef fr_l(lhs, fieldID);
            auto baseType_l = getBaseType(fr_l.getValue().getType());
            auto type_l = hw::FieldIdImpl::getFinalTypeByFieldID(baseType_l, fr_l.getFieldID());
            ...
            if (wl >= 0) { // lhs已知
              ...
              auto type_r = hw::FieldIdImpl::getFinalTypeByFieldID(baseType_r, fr_r.getFieldID());
              auto wr = cast<FIRRTLBaseType>(type_r).getBitWidthOrSentinel();
              if (wr >= 0) { // rhs已知 
              } else if (wr == -1) { // rhs 未知ground
                Term t_2(1, fr_r);
                Constraint1 temp_c1(fieldRef_result, static_cast<int>(wl), {}, std::nullopt), temp_c2(fieldRef_result, 0, {t_2}, std::nullopt);
              ...
              if (wr >= 0) { // rhs已知 
                Term t_1(1, fr_l);
                Constraint1 temp_c2(fieldRef_result, static_cast<int>(wr), {}, std::nullopt), temp_c1(fieldRef_result, 0, {t_1}, std::nullopt);
                LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
                addConstraint(temp_c1); 
                //if (fr_l != fr_r) 
                  addConstraint(temp_c2);
                ...
                Constraint1 temp_c1(fieldRef_result, 0, {t_1}, std::nullopt), temp_c2(fieldRef_result, 0, {t_2}, std::nullopt);
                LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
                addConstraint(temp_c1); 
                //if (fr_l != fr_r) 
                  addConstraint(temp_c2);
...
LogicalResult InferenceMapping::extractConstraints(Operation *op) {
  ...
  TypeSwitch<Operation *>(op)
      .Case<ConstantOp>([&](auto op) {
        ...
        auto v = op.getValue();
        if (type_cast<FIRRTLType>(getBaseType(op.getResult().getType())).hasUninferredWidth()) {
      ...
      .Case<InvalidValueOp>([&](auto op) {
        generateConstraints(op.getResult());
      }) 
      ...
      .Case<NodeOp>([&](auto op) {
        ...
        generateConstraints(FieldRef(op.getResult(), 0), FieldRef(op.getInput(), 0),
      ...
      .Case<SubindexOp, SubaccessOp>([&](auto op) {
        ...
        generateConstraints(FieldRef(op.getInput(), 1), FieldRef(op.getResult(), 0),
                   op.getType());
        generateConstraints(FieldRef(op.getResult(), 0), FieldRef(op.getInput(), 1), 
                   op.getType());
      })
      ...
      .Case<AddPrimOp, SubPrimOp>([&](auto op) {
        if (type_cast<FIRRTLType>(getBaseType(op.getLhs().getType())).hasUninferredWidth()) {
          if (type_cast<FIRRTLType>(getBaseType(op.getRhs().getType())).hasUninferredWidth()) {
            FieldRef fr_1(op.getLhs(), 0), fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
            Term t_1(1, fr_1), t_2(1, fr_2);
            Constraint1 temp_c1(fr_dest, 1, {t_1}, std::nullopt), temp_c2(fr_dest, 1, {t_2}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
            addConstraint(temp_c1); 
            ...
            Term t_1(1, fr_1);
            Constraint1 temp_c1(fr_dest, 1, {t_1}, std::nullopt), temp_c2(fr_dest, static_cast<int>(w) + 1, {}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
            addConstraint(temp_c1); 
            //if (op.getLhs() != op.getRhs()) 
              addConstraint(temp_c2);
          ...
          if (type_cast<FIRRTLType>(getBaseType(op.getRhs().getType())).hasUninferredWidth()) {
            ...
            Term t_2(1, fr_2);
            Constraint1 temp_c1(fr_dest, static_cast<int>(w) + 1, {}, {}), temp_c2(fr_dest, 1, {t_2}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
            addConstraint(temp_c1); 
            //if (op.getLhs() != op.getRhs()) 
              addConstraint(temp_c2);
            ...
            FieldRef fr_dest(op.getResult(), 0);
            Constraint1 temp_c1(fr_dest, static_cast<int>(w0) + 1, {}, {}), temp_c2(fr_dest, static_cast<int>(w1) + 1, {}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
          }*/
        }
      })
      .Case<MulPrimOp>([&](auto op) {
        ...
        if (w0 >= 0) { //lhs 已知，不使用变量名
          ...
          if (w1 >= 0) { //rhs 已知，不使用变量名
            ...
            FieldRef fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
            Term t_2(1, fr_2);
            Constraint1 temp_c(fr_dest, static_cast<int>(w0), {t_2}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c << "\n");
            addConstraint(temp_c);
          }
          ...
          if (w1 >= 0) { //rhs 已知，不使用变量名
            FieldRef fr_1(op.getLhs(), 0), fr_dest(op.getResult(), 0);
            Term t_1(1, fr_1);
            Constraint1 temp_c(fr_dest, static_cast<int>(w1), {t_1}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c << "\n");
            addConstraint(temp_c);
          } else if (w1 == -1) { // rhs未知，使用变量名
            FieldRef fr_1(op.getLhs(), 0), fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
      ...
      .Case<RemPrimOp>([&](auto op) {
        ...
        if (w0 >= 0) { //lhs 已知，不使用变量名
          ...
          if (w1 >= 0) { //rhs 已知，不使用变量名
            ...
            FieldRef fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
            Constraint_Min temp_c(fr_dest, static_cast<int>(w0), 0, std::nullopt, fr_2);
            LLVM_DEBUG(llvm::dbgs() << "build constraint_min : " << temp_c << "\n");
            addConstraint(temp_c);
          }
        } else if (w0 == -1) { // lhs未知，使用变量名
          auto w1 = getBaseType(op.getRhs().getType()).getBitWidthOrSentinel();
          if (w1 >= 0) { //rhs 已知，不使用变量名
            FieldRef fr_1(op.getLhs(), 0), fr_dest(op.getResult(), 0);
            Constraint_Min temp_c(fr_dest, 0, static_cast<int>(w1), fr_1, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint_min : " << temp_c << "\n");
            addConstraint(temp_c); 
          } else if (w1 == -1) { // rhs未知，使用变量名
            FieldRef fr_1(op.getLhs(), 0), fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
      ...
      .Case<AndPrimOp, OrPrimOp, XorPrimOp>([&](auto op) {
        if (type_cast<FIRRTLType>(getBaseType(op.getLhs().getType())).hasUninferredWidth()) {
          if (type_cast<FIRRTLType>(getBaseType(op.getRhs().getType())).hasUninferredWidth()) {
            FieldRef fr_1(op.getLhs(), 0), fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
            Term t_1(1, fr_1), t_2(1, fr_2);
            Constraint1 temp_c1(fr_dest, 0, {t_1}, std::nullopt), temp_c2(fr_dest, 0, {t_2}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
            addConstraint(temp_c1); 
            //if (op.getLhs() != op.getRhs()) 
              addConstraint(temp_c2);
            ...
            FieldRef fr_1(op.getLhs(), 0), fr_dest(op.getResult(), 0);
            Term t_1(1, fr_1);
            Constraint1 temp_c1(fr_dest, 0, {t_1}, std::nullopt), temp_c2(fr_dest, static_cast<int>(w), {}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
            addConstraint(temp_c1); 
            //if (op.getLhs() != op.getRhs()) 
              addConstraint(temp_c2);
          ...
          if (type_cast<FIRRTLType>(getBaseType(op.getRhs().getType())).hasUninferredWidth()) {
            ...
            FieldRef fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
            Term t_2(1, fr_2);
            Constraint1 temp_c1(fr_dest, static_cast<int>(w), {}, {}), temp_c2(fr_dest, 0, {t_2}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
            addConstraint(temp_c1); 
            //if (op.getLhs() != op.getRhs()) 
              addConstraint(temp_c2);
      ...
      .Case<CatPrimOp>([&](auto op) {
        ...
        if (op.getInputs().empty()) {
          ...
          auto w = getBaseType(op.getInputs().front().getType()).getBitWidthOrSentinel();
          if (w >= 0) { //第一项为已知
            Terms result_t = {};
      ...
      .Case<DShlPrimOp>([&](auto op) {
        ...
        if (w0 >= 0) { //lhs 已知，不使用变量名
          ...
          if (w1 >= 0) { //rhs 已知，不使用变量名
            FieldRef fr_1(op.getLhs(), 0), fr_dest(op.getResult(), 0);
            Term t_1(1, fr_1);
            Constraint1 temp_c(fr_dest, (1 << static_cast<int>(w1)) - 1, {t_1}, std::nullopt);
            LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c << "\n");
            addConstraint(temp_c); 
          } else if (w1 == -1) { // rhs未知，使用变量名
            FieldRef fr_1(op.getLhs(), 0), fr_2(op.getRhs(), 0), fr_dest(op.getResult(), 0);
      ...
      .Case<NegPrimOp>([&](auto op) {
        if (type_cast<FIRRTLType>(getBaseType(op.getType())).hasUninferredWidth()) {
      ...
      .Case<BitsPrimOp>([&](auto op) {
        if (type_cast<FIRRTLType>(getBaseType(op.getType())).hasUninferredWidth()) {
          FieldRef fr_dest(op.getResult(), 0);
          Constraint1 temp_c(fr_dest, op.getHi() - op.getLo() + 1, {}, std::nullopt);
          LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c << "\n");
          addConstraint(temp_c); 
        }
      })
      ...
      .Case<PadPrimOp>([&](auto op) {
        if (type_cast<FIRRTLType>(getBaseType(op.getType())).hasUninferredWidth()) {
          FieldRef fr_src(op.getInput(), 0), fr_dest(op.getResult(), 0);
          Term t_src(1, fr_src);
          Constraint1 temp_c1(fr_dest, 0, {t_src}, std::nullopt), temp_c2(fr_dest, op.getAmount(), {}, std::nullopt);
          LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
          addConstraint(temp_c1); addConstraint(temp_c2);
        }
      })
      ...
      .Case<ShrPrimOp>([&](auto op) {
        if (type_cast<FIRRTLType>(getBaseType(op.getType())).hasUninferredWidth()) {
          ...
          Constraint1 temp_c1(fr_dest, -op.getAmount(), {t_src}, std::nullopt), temp_c2(fr_dest, minWidth, {}, std::nullopt);
          LLVM_DEBUG(llvm::dbgs() << "build constraint1 : " << temp_c1 << "\nand : " << temp_c2 << "\n");
          addConstraint(temp_c1); addConstraint(temp_c2);
        }
      })
      ...
      .Case<mlir::UnrealizedConversionCastOp>(
        [&](auto op) { 
        if (type_cast<FIRRTLType>(getBaseType(op.getResult(0).getType())).hasUninferredWidth()) {
      ...
      .Case<InstanceOp>([&](auto op) {
        ...
        for (auto [result, arg] :
          ...
          generateConstraints({result, 0}, {arg, 0}, // 这里是产生FieldRef, 相当于把instance端口看作bundle
                     type_cast<FIRRTLType>(result.getType()));
          generateConstraints({arg, 0}, {result, 0}, // 这里是产生FieldRef, 相当于把instance端口看作bundle
                     type_cast<FIRRTLType>(result.getType()));
        }
      })
      ...
      .Case<MemOp>([&](MemOp op) {
        ...
        for (unsigned i = 0, e = op.getResults().size(); i < e; ++i) {
          ...
          for (auto fieldIndex : dataFieldIndices(op.getPortKind(i))) {
            generateConstraints(FieldRef(result, portType.getFieldID(fieldIndex)),
                        firstData, dataType);
            generateConstraints(firstData, FieldRef(result, portType.getFieldID(fieldIndex)),
                        dataType);
          }
        }
      })
  ...
  if (auto fop = dyn_cast<Forceable>(op); fop && fop.isForceable())
    generateConstraints(FieldRef(fop.getDataRef(), 0), FieldRef(fop.getDataRaw(), 0),
               fop.getDataType());
...
// 构建约束图的函数
GraphResult build_constraint_graph(const std::vector<Constraint1>& constraints) {
    GraphResult result;
...
Valuation my_floyd(const std::vector<Constraint1>& constraints) {
    ...
    DistanceMatrix dist(vertex_count, std::vector<int>(vertex_count, 
    ...
    for (size_t i = 0; i < vertex_count; ++i) {
        ...
        if (min_target_index < vertex_count) {
            ...
            LLVM_DEBUG(llvm::dbgs() << "最短路径: " << source_node << " → " << min_target_node 
                      << " (距离: " << min_distance << ")\n");
        } else {
...
public:
    dfs_visitor(std::vector<typename DependencyGraph::vertex_descriptor>& predecessor, 
...
find_path(const DependencyGraph& g, 
    ...
    std::vector<vertex_descriptor> predecessor(
        num_vertices, 
        boost::graph_traits<DependencyGraph>::null_vertex()
    );
...
// 构建依赖关系图
DependencyGraphResult buildDependencyGraph(const std::vector<Constraint1>& constraints_) {
        DependencyGraphResult result;

        // 添加节点的辅助函数
        auto get_or_add_vertex = [&](const FieldRef& node) -> boost::graph_traits<DependencyGraph>::vertex_descriptor {
...
std::vector<Constraint1> orderConstraints(const std::vector<FieldRef>& vars, 
                                         const std::vector<Constraint1>& constraints) {
    std::vector<Constraint1> result;
    if (vars.size() < 2) {
    ...
    for (size_t i = 0; i < vars.size() - 1; ++i) {
        ...
        // 如果未找到匹配约束，抛出异常
        if (!constraintFound) {
            LLVM_DEBUG(llvm::dbgs() << "No matching constraint found for variable pair: "
...
Constraint1 substitute_constraint(const Constraint1& c, const FieldRef& v, 
    ...
    for (const auto& term : terms) {
        ...
        auto exist_it = std::find_if(new_terms_list.begin(), new_terms_list.end(),
...
Constraint1 substitute_c(const Constraint1& c1, const Constraint1& c2) {
    return substitute_constraint(
        c1, 
        c2.lhs_var1(), 
        c2.rhs_terms1().get_terms(), 
        c2.rhs_const1()
    );
}

std::optional<Constraint1> substitute_cs(const std::vector<Constraint1>& constraints) {
    ...
    new_tail.insert(new_tail.end(), std::next(std::next(constraints.begin())), constraints.end());
...
std::optional<int> compute_ub(const Constraint1& c) {
    ...
    // 查找左侧变量在右侧项中的位置
    auto found_it = std::find_if(terms.begin(), terms.end(),
        [&](const Term& t) { return t.var() == lhs_var; });
...
std::optional<int> solve_ub_case1(const FieldRef& x, const FieldRef& var, const Constraint1& c, 
    const std::vector<Constraint1>& constraints, const DependencyGraphResult& result) {
    // c : lhs >= coe * var + ... + cst c 
    // 找 x >= ? * lhs + ... 和 var >= ? * x + ... 
    LLVM_DEBUG(llvm::dbgs() << "Solving [" 
        << x.getValue() << " (fieldID: " << x.getFieldID() << ")]\n");
    auto find_vertex = [&](const FieldRef& p) -> std::optional<decltype(result.g)::vertex_descriptor> {
    ...
    std::vector<FieldRef> path1 = find_path(result.g, var_vertex.value(), x_vertex.value());
    std::vector<FieldRef> path0 = find_path(result.g, x_vertex.value(), lhs_vertex.value());

    if (path0.empty()) {
        LLVM_DEBUG(llvm::dbgs() << "No path found!\n");
    } else {
        LLVM_DEBUG(llvm::dbgs() << "Path found: "
            << "[" << path0[0].getValue() << " (fieldID: " << path0[0].getFieldID() << ")]");

        for (size_t i = 1; i < path0.size(); ++i) {
            LLVM_DEBUG(llvm::dbgs() << " -> " 
            << "[" << path0[i].getValue() << " (fieldID: " << path0[i].getFieldID() << ")]");
        }
        LLVM_DEBUG(llvm::dbgs() << "\n");
    }
    if (path1.empty()) {
        ...
        LLVM_DEBUG(llvm::dbgs() << "Path found: " 
            << "[" << path1[0].getValue() << " (fieldID: " << path1[0].getFieldID() << ")]");
        for (size_t i = 1; i < path1.size(); ++i) {
            LLVM_DEBUG(llvm::dbgs() << " -> " 
            << "[" << path1[i].getValue() << " (fieldID: " << path1[i].getFieldID() << ")]");
        }
        LLVM_DEBUG(llvm::dbgs() << "\n");
    }
    ...
    // 按顺序添加元素
    conslist.insert(conslist.end(), conslist0.begin(), conslist0.end()); // 添加 conslist0
    conslist.push_back(c);                                            // 添加单个元素 c
    conslist.insert(conslist.end(), conslist1.begin(), conslist1.end()); // 添加 conslist1
...
Valuation solve_ubs_case1(const std::vector<FieldRef>& tbsolved, const FieldRef& var, const Constraint1& c, 
    const std::vector<Constraint1>& constraints, const DependencyGraphResult& result) {
    Valuation v;
    for (const FieldRef& x : tbsolved) {
        ...
        if (ub.has_value()) {
            ...
            LLVM_DEBUG(llvm::dbgs() << "[" << x.getValue() << " (fieldID: " << x.getFieldID() << ")]"
            << "求解失败\n"); }
    };
    return v;
}    

std::optional<int> solve_ub_case2(const FieldRef& x, const FieldRef& var1, const FieldRef& var2, const Constraint1& c, 
    const std::vector<Constraint1>& constraints, const DependencyGraphResult& result) {
    ...
    std::vector<FieldRef> path2 = find_path(result.g, var2_vertex.value(), x_vertex.value());
    std::vector<FieldRef> path1 = find_path(result.g, var1_vertex.value(), x_vertex.value());
    std::vector<FieldRef> path0 = find_path(result.g, x_vertex.value(), lhs_vertex.value());
    ...
    if (path2.empty()) {
        ...
        LLVM_DEBUG(llvm::dbgs() << "Path found: " 
            << "[" << path2[0].getValue() << " (fieldID: " << path2[0].getFieldID() << ")]");
        for (size_t i = 1; i < path2.size(); ++i) {
            LLVM_DEBUG(llvm::dbgs() << " -> " 
            << "[" << path2[i].getValue() << " (fieldID: " << path2[i].getFieldID() << ")]");
        }
        LLVM_DEBUG(llvm::dbgs() << "\n");
    }
    ...
    conslist.push_back(c);                                            // 添加单个元素 c
    conslist.insert(conslist.end(), conslist1.begin(), conslist1.end()); // 添加 conslist1
    conslist.insert(conslist.end(), conslist2.begin(), conslist2.end()); // 添加 conslist2
...
Valuation solve_ubs_case2(const std::vector<FieldRef>& tbsolved, const FieldRef& var1, const FieldRef& var2, const Constraint1& c, 
    const std::vector<Constraint1>& constraints, const DependencyGraphResult& result) {
    Valuation v;
    for (const FieldRef& x : tbsolved) {
        // 求解当前变量
...
// 查找系数大于1项
std::optional<Constraint1> findCWithCoeGreaterThanOne(const std::vector<Constraint1>& constraints) {
    for (const auto& c : constraints) {
...
// 查找多于2项
std::optional<Constraint1> findCWithTwoVars(const std::vector<Constraint1>& constraints) {
    for (const auto& c : constraints) {
...
std::vector<Constraint1> relax_all_powers(const std::vector<Constraint1>& constraints) {
    std::vector<Constraint1> result;
    result.reserve(constraints.size());  
    std::transform(constraints.begin(), constraints.end(), std::back_inserter(result),
                  [](const Constraint1& c) {
                      return relax_power(c);
                  });
    return result;
}

// 移除 rhs_terms1 为空的约束
std::vector<Constraint1> remove_only_const(const std::vector<Constraint1>& constraints) {
    std::vector<Constraint1> result;
    result.reserve(constraints.size()); 
    std::copy_if(constraints.begin(), constraints.end(), std::back_inserter(result),
                [](const Constraint1& c) {
...
std::optional<Valuation> solve_ubs(const std::vector<Constraint1>& constraints, const std::vector<FieldRef>& tbsolved) {
    ...
    if (c.has_value()) {
        LLVM_DEBUG(llvm::dbgs() << "成功获取系数大于1的约束: " << c.value() << "\n");
        auto var = c.value().rhs_terms1().findVarWithCoeGreaterThanOne();
        if (var.has_value()) {
            Valuation ubs = solve_ubs_case1(tbsolved, var.value(), c.value(), processed_constraints, result);
            for (const auto& [pv, value] : ubs) {
                LLVM_DEBUG(llvm::dbgs() << "[" << pv.getValue() << " (fieldID: " << pv.getFieldID() << ")] : " << value << "\n");
            }
            return ubs;
        } else {
            std::cerr << "没有系数大于1的项\n"; 
        ...
        if (c.has_value()) {
            LLVM_DEBUG(llvm::dbgs() << "成功获取多于1项的约束: " << c.value() << "\n");
            auto var = c.value().rhs_terms1().findFirstTwoVars();
            if (var.has_value()) {
                Valuation ubs = solve_ubs_case2(tbsolved, var.value().first, var.value().second, c.value(), processed_constraints, result);
                for (const auto& [pv, value] : ubs) {
                    LLVM_DEBUG(llvm::dbgs() << "[" << pv.getValue() << " (fieldID: " << pv.getFieldID() << ")] : " << value << "\n");
                }
                return ubs;
...
std::optional<Valuation> prioritize_fst(
    std::optional<Valuation> v1,
...
// 主分支定界函数
std::optional<Valuation> bab_bin(
    const std::vector<FieldRef>& scc,
    const Bounds& bounds,
    ...
    if (unsat1) {
        ...
        if (v) {
            ...
            if (it != bounds.end()) {
                ...
                // 优先搜索下半区
                auto left_sol = bab_bin(
                    scc, 
                    update_ub(bounds, *v, mid), 
                    cs1, 
                    cs2);
        ...
        // 无右侧可分割变量，检查左侧变量
            ...
            if (it != bounds.end()) {
                ...
                // 优先搜索上半区
                auto right_sol = bab_bin(
                    scc, 
                    update_lb(bounds, c1.lhs_var1(), mid + 1), 
                    cs1, 
                    cs2);
                ...
                return bab_bin(
                    scc, 
                    update_ub(bounds, c1.lhs_var1(), mid), 
                    cs1, 
                    cs2);
            }
            return std::nullopt; // 不可能的情况
...
Bounds combineValuations(
    const std::vector<FieldRef>& vars,
    const Valuation& val1,
    const Valuation& val2) 
{
    Bounds result;
...
Valuation bab(const std::vector<Constraint1>& constraints, const std::vector<FieldRef>& tbsolved) {
    ...
    std::optional<Valuation> solution = bab_bin(
        tbsolved, 
        bounds, 
        constraints,  // 类型1约束列表 
        {}  // 类型2约束列表
    ...
    if (solution.has_value()) {
        ...
        //     LLVM_DEBUG(llvm::dbgs() << "[" << var.getValue() << " (fieldID: " << var.getFieldID() << ")]"
        //     << "\n = " << val << "\n";
        // }
        ...
        // LLVM_DEBUG(llvm::dbgs() << "\n约束验证:\n";
        // for (const auto& c : constraints) {
        //     LLVM_DEBUG(llvm::dbgs() << c << (c.satisfies(*solution) ? " 满足" : " 违反") << "\n";
        // }
...
LogicalResult InferenceMapping::solve() {
  ...
  std::transform(constraints_min_.begin(), constraints_min_.end(), std::back_inserter(min_list),
              [this](const Constraint_Min& c) { 
  ...
  for (auto cs : flatten_min) {
    ...
    if (failed(solve_result)) {
      ...
      for (const auto& [pv, value] : current_solution) {
        LLVM_DEBUG(llvm::dbgs() //<< "[" << pv.getValue() << " (fieldID: " << pv.getFieldID() << ")] : \n"
...
void InferWidthsPass_new::runOnOperation() {
  ...
  // std::cout << "solve time : " << solve_duration.count() / 1000.0 << " ms\n";
  // std::cout << "update time : " << update_duration.count() / 1000.0 << " ms\n";
}
