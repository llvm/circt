  ...
  createNLAs(Operation *fromModule, ArrayRef<Attribute> baseNamepath,
    ...
    for (auto *instanceRecord : fromNode->uses()) {
      ...
      if (!cacheEntry) {
        ...
        auto nla = hw::HierPathOp::create(builder, loc,
                                          /*sym_visibility=*/{}, "nla",
                                          arrayAttr);
  ...
  void runOnOperation() override {
    ...
    for (auto [i, module] : llvm::enumerate(modules)) {
      ...
      if (it != moduleInfoToModule.end()) {
        ...
        if (!canRemoveModule(cast<mlir::SymbolOpInterface>(module.getOperation()))) {
          // Record that this module's name is staying the same.
          dedupMap[moduleName] = moduleName;
          // If both modules are public, then we can't dedup anything.
          if (!canRemoveModule(cast<mlir::SymbolOpInterface>(original.getOperation())))
            continue;
