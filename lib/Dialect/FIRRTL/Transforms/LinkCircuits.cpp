  ...
  circuit.walk([&](Operation *op) {
    ...
    TypeSwitch<Operation *>(op)
        .Case<CircuitOp>([&](CircuitOp circuit) {
          auto processAttr = [&](NamedAttribute namedAttr) {
                ...
                if (auto tokens = tokenizePath(target.getValue()))
                  if (auto *newModule = renameTable.lookup(tokens->module)) {
                    ...
                    return NamedAttribute{
                        namedAttr.getName(),
                        StringAttr::get(target.getContext(), tokens->str())};
                  }
            return namedAttr;
          };
          ...
          llvm::transform(
              ...
              [&](Attribute attr) {
                ...
                auto newElements =
                    to_vector(map_range(anno.getValue(), processAttr));
                return DictionaryAttr::getWithSorted(anno.getContext(),
                                                     newElements);
              });
...
static LogicalResult linkExtmodule(SymbolOpInterface collidingOp,
...
LogicalResult LinkCircuitsPass::mergeCircuits() {
  ...
  for (auto circuit : circuits) {
    ...
    append_range(mergedAnnotations, circuit.getAnnotations().getValue());
    ...
    for (auto *op : opsToMove) {
      if (auto symbolOp = dyn_cast<SymbolOpInterface>(op))
        if (auto collidingOp = cast_or_null<SymbolOpInterface>(
                mergedSymbolTable.lookup(symbolOp.getNameAttr())))
          if (failed(linkExtmodule(collidingOp, symbolOp)))
