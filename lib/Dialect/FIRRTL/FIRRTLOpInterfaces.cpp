...
LogicalResult circt::firrtl::verifyModuleLikeOpInterface(FModuleLike module) {
  ...
  for (auto [index, domain] : llvm::enumerate(module.getDomainInfo())) {
    auto type = cast<TypeAttr>(portTypes[index]).getValue();
    if (isa<DomainType>(type)) {
      if (!isa<FlatSymbolRefAttr>(domain))
        return module.emitOpError()
               << "domain information for domain port " << portNames[index]
    ...
    if (llvm::any_of(domains.getValue(), [](Attribute attr) {
          ...
          if (!arrayAttr)
            return false;
          return llvm::any_of(arrayAttr, [](Attribute attr) {
            return !isa<IntegerAttr>(attr);
          });
        }))
      return module.emitOpError()
             << "domain information for non-domain port" << portNames[index]
