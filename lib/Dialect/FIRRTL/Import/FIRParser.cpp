...
private:
  ...
  ParseResult parseOptionalEnabledLayer(SymbolRefAttr &result);
  ParseResult parseOptionalKnownLayer(SymbolRefAttr &result);
...
FIRCircuitParser::parseOptionalModuleLayerSpec(ArrayAttr &enabledLayers) {
  ...
  while (true) {
    ...
    // Try to parse an enablelayer spec.
    if (parseOptionalEnabledLayer(layer))
      return failure();
    if (layer) {
      enabledLayersBuffer.push_back(layer);
      continue;
    }
...
FIRCircuitParser::parseOptionalExtModuleLayerSpec(ArrayAttr &enabledLayers,
  ...
  while (true) {
    ...
    // Try to pare a knownlayer spec.
    if (parseOptionalKnownLayer(layer))
      return failure();
    if (layer) {
      knownLayersBuffer.push_back(layer);
      continue;
    }
...
ParseResult FIRCircuitParser::parseOptionalEnabledLayer(SymbolRefAttr &result) {
  if (getToken().getKind() != FIRToken::kw_enablelayer) {
    result = nullptr;
    return success();
  }
  consumeToken();
  return parseLayerName(result);
}

ParseResult FIRCircuitParser::parseOptionalKnownLayer(SymbolRefAttr &result) {
  if (getToken().getKind() != FIRToken::kw_knownlayer) {
    result = nullptr;
    return success();
  }
  consumeToken();
