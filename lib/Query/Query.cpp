...
Filter *AndFilter::clone() { return new AndFilter(filters); }
...
Filter *OrFilter::clone() { return new OrFilter(filters); }

bool InstanceFilter::matches(Operation *op, FilterData &data) {
  return testForMatch(filter, data, op);
}

Filter *InstanceFilter::clone() { return new InstanceFilter(filter, child); }
