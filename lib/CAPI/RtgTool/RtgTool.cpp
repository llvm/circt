...
CirctRtgToolOptions circtRtgToolOptionsCreateDefault() {
  auto *options = new RtgToolOptions();
  return wrap(options);
}
...
void circtRtgToolOptionsUnsetSeed(CirctRtgToolOptions options) {
  unwrap(options)->unsetSeed();
}
...
void circtRtgToolOptionsSetDebugMode(CirctRtgToolOptions options, bool enable) {
  unwrap(options)->setDebugMode(enable);
}
