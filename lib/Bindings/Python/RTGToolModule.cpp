...
public:
  PyRtgToolOptions() { options = circtRtgToolOptionsCreateDefault(); }
  PyRtgToolOptions(CirctRtgToolOutputFormat outputFormat,
                   std::optional<unsigned> seed, bool verifyPasses,
                   bool verbosePassExecution, bool debugMode,
  ...
  void setSeed(std::optional<unsigned> seed) {
    if (!seed)
      circtRtgToolOptionsUnsetSeed(options);
    else
      circtRtgToolOptionsSetSeed(options, *seed);
  }
  ...
  void setDebugMode(bool enable) {
    circtRtgToolOptionsSetDebugMode(options, enable);
  }
...
void circt::python::populateDialectRTGToolSubmodule(py::module &m) {
  ...
  py::class_<PyRtgToolOptions>(m, "Options")
      .def(py::init<>())
      .def(py::init<CirctRtgToolOutputFormat, std::optional<unsigned>, bool,
                    bool, bool, const std::vector<const char *> &,
                    const std::string &>(),
           py::arg("output_format") = CIRCT_RTGTOOL_OUTPUT_FORMAT_ASM,
           py::arg("seed") = std::nullopt, py::arg("verify_passes") = true,
      ...
      .def("set_verbose_pass_execution",
           ...
           py::arg("enable"))
      .def("set_debug_mode", &PyRtgToolOptions::setDebugMode,
           "Specify whether passes that support a debug mode should use it",
           py::arg("enable"))
