...
private:
  static LogicalResult lowerBoothArray(PatternRewriter &rewriter, Value a,
    ...
    // Directly derive the Booth controls from the carry-save input.  This is
    // the XOR-based carry-save Booth recoder from Zimmermann and Tran's Fig.
    // 3.  Each radix-4 slice examines the two bits in its own radix-4 digit,
    // the high bit of the preceding digit, and a one-bit state from that
    // preceding slice.  Crucially, the state generated by a slice does not
    // depend on its input state, so there is no carry-propagation chain.
    SmallVector<Value> aBits = extractBits(rewriter, a);
    SmallVector<Value> bBits = extractBits(rewriter, b);
    ...
    SmallVector<Value> encNegs;
    partialProducts.reserve(op.getNumResults());
    unsigned recoderLimit = width + (width % 2 == 0 ? 1 : 3);
    encNegs.reserve((recoderLimit + 1) / 2);
    Value encNegPrev;
    Value recoderCarry = zero;

    // Generate one radix-4 digit for bit positions i and i+1.  The trailing
    // all-zero slice absorbs the two's-complement correction of a negative
    // top digit.  Rows which lie completely above the result width are
    // dropped below.
    for (unsigned i = 0; i < recoderLimit; i += 2) {
      Value aim1 = i > 0 && i - 1 < width ? aBits[i - 1] : zero;
      Value bim1 = i > 0 && i - 1 < width ? bBits[i - 1] : zero;
      Value ai = i < width ? aBits[i] : zero;
      Value bi = i < width ? bBits[i] : zero;
      Value aip1 = i + 1 < width ? aBits[i + 1] : zero;
      Value bip1 = i + 1 < width ? bBits[i + 1] : zero;

      Value prevOr = rewriter.createOrFold<comb::OrOp>(loc, aim1, bim1, true);
      Value prevAnd = rewriter.createOrFold<comb::AndOp>(loc, aim1, bim1, true);
      Value curPropagate =
          rewriter.createOrFold<comb::XorOp>(loc, ai, bi, true);
      Value curGenerate = rewriter.createOrFold<comb::AndOp>(loc, ai, bi, true);
      Value highPropagate =
          rewriter.createOrFold<comb::XorOp>(loc, aip1, bip1, true);

      // The majority term is the central three-AND/OR gate in Fig. 3.
      Value propagatePrev =
          rewriter.createOrFold<comb::AndOp>(loc, curPropagate, prevAnd, true);
      Value majority = rewriter.createOrFold<comb::OrOp>(
          loc, ValueRange{curGenerate, propagatePrev}, true);

      // n_i selects a negative partial product.  x1_i and x2_i select
      // magnitude one and two respectively.
      Value encNeg = rewriter.createOrFold<comb::XorOp>(loc, highPropagate,
                                                        majority, true);
      Value encOne = rewriter.createOrFold<comb::XorOp>(
          loc, ValueRange{curPropagate, prevOr, recoderCarry}, true);
      Value oneOrPrev = rewriter.createOrFold<comb::OrOp>(
          loc, ValueRange{curPropagate, prevOr}, true);
      Value twoCandidate = rewriter.createOrFold<comb::XorOp>(
          loc, ValueRange{highPropagate, curGenerate, oneOrPrev}, true);
      Value encOneInv =
          rewriter.createOrFold<comb::XorOp>(loc, encOne, one, true);
      Value encTwo = rewriter.createOrFold<comb::AndOp>(
          loc, ValueRange{twoCandidate, encOneInv}, true);

      // c_(i+1) is independent of c_i.  Passing this value to the next slice
      // therefore does not form a ripple path.
      Value highPropagateInv =
          rewriter.createOrFold<comb::XorOp>(loc, highPropagate, one, true);
      Value recoderCarryNext = rewriter.createOrFold<comb::AndOp>(
          loc, ValueRange{highPropagateInv, majority}, true);
