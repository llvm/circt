...
struct FIRRTLLowering : public FIRRTLVisitor<FIRRTLLowering, LogicalResult> {
  ...
  std::optional<Value> getLoweredFmtOperandLegacy(Value operand);
  ...
  LogicalResult
  loweredFmtOperandsLegacy(ValueRange operands,
                           SmallVectorImpl<Value> &loweredOperands);
...
/// Reserved for verification path
/// Zero bit operands are rewritten as one bit zeros and signed integers are
/// wrapped in $signed().
std::optional<Value> FIRRTLLowering::getLoweredFmtOperandLegacy(Value operand) {
  // Handle special substitutions.
  if (type_isa<FStringType>(operand.getType())) {
    if (isa<TimeOp>(operand.getDefiningOp()))
      return sim::TimeOp::create(builder);
...
// Reserved for verification path
LogicalResult FIRRTLLowering::loweredFmtOperandsLegacy(
    mlir::ValueRange operands, SmallVectorImpl<Value> &loweredOperands) {
  for (auto operand : operands) {
    std::optional<Value> loweredValue = getLoweredFmtOperandLegacy(operand);
    if (!loweredValue)
      return failure();
    // Skip if the lowered value is null.
    if (*loweredValue)
...
LogicalResult FIRRTLLowering::lowerVerificationStatement(
  ...
  if (!isCover && opMessageAttr && !opMessageAttr.getValue().empty()) {
    ...
    if (failed(loweredFmtOperandsLegacy(opOperands, messageOps)))
      return failure();
  ...
  auto emit = [&]() {
    ...
    case VerificationFlavor::SVA: {
      ...
      sv::EventControl event = circt::sv::EventControl::AtPosEdge;
