...
void FIRRTLModuleLowering::runOnOperation() {
  ...
  void FIRRTLModuleLowering::lowerMemoryDecls(ArrayRef<FirMemory> mems,
    ...
    for (auto &mem : mems) {
      ...
      NamedAttribute genAttrs[] = {
          ...
          b.getNamedAttr("numReadPorts",
                         b.getUI32IntegerAttr(mem.numReadPorts)),
          ...
          b.getNamedAttr("writeLatency",
                         b.getUI32IntegerAttr(mem.writeLatency)),
          ...
          b.getNamedAttr("writeClockIDs",
                         b.getI32ArrayAttr(mem.writeClockIDs))};

      // Make the global module for the memory
      auto memoryName = b.getStringAttr(mem.getFirMemoryName());
      b.create<hw::HWModuleGeneratedOp>(mem.loc, memorySchema, memoryName,
                                        ports, StringRef(), ArrayAttr(),
                                        genAttrs);
    }
  }
  ...
  void FIRRTLModuleLowering::lowerFileHeader(CircuitOp op,
    ...
    if (!state.used_RANDOMIZE_GARBAGE_ASSIGN &&
        !state.used_RANDOMIZE_REG_INIT && !state.used_RANDOMIZE_MEM_INIT &&
  ...
  LogicalResult FIRRTLModuleLowering::lowerPorts(
    ...
    for (auto firrtlPort : firrtlPorts) {
      ...
      loweringState.processRemainingAnnotations(moduleOp,
                                                firrtlPort.annotations);
    }
    return success();
  }
  ...
  hw::HWModuleExternOp FIRRTLModuleLowering::lowerExtModule(
  ...
  hw::HWModuleOp FIRRTLModuleLowering::lowerModule(
      FModuleOp oldModule, Block * topLevelModule,
      CircuitLoweringState & loweringState) {
    // Map the ports over, lowering their types as we go.
    SmallVector<PortInfo> firrtlPorts = oldModule.getPorts();
    SmallVector<hw::PortInfo, 8> ports;
  ...
  static SmallVector<SubfieldOp> getAllFieldAccesses(Value structValue,
    ...
    for (auto op : structValue.getUsers()) {
      ...
      auto elemIndex =
          fieldAccess.input().getType().cast<BundleType>().getElementIndex(
              field);
  ...
  struct FIRRTLLowering : public FIRRTLVisitor<FIRRTLLowering, LogicalResult> {
    ...
    LogicalResult visitExpr(SubPrimOp op) {
  ...
  /// each element.
  Value FIRRTLLowering::getExtOrTruncAggregateValue(
      Value array, FIRRTLType sourceType, FIRRTLType destType,
      bool allowTruncate) {
    ...
    std::function<LogicalResult(Value, FIRRTLType, FIRRTLType)> recurse =
        [&](Value src, FIRRTLType srcType,
            FIRRTLType destType) -> LogicalResult {
      return TypeSwitch<FIRRTLType, LogicalResult>(srcType)
          ...
          .Case<BundleType>([&](BundleType srcStructType) {
            ...
            if (destStructType.getNumElements() !=
                srcStructType.getNumElements())
              return failure();
  ...
  /// This returns a null value for FIRRTL values that cannot be lowered, e.g.
  /// unknown width integers.
  Value FIRRTLLowering::getLoweredAndExtOrTruncValue(Value value,
                                                     Type destType) {
    assert(value.getType().isa<FIRRTLType>() && destType.isa<FIRRTLType>() &&
           "input/output value should be FIRRTL");
  ...
  void FIRRTLLowering::addToAlwaysBlock(
      sv::EventControl clockEdge, Value clock, ::ResetType resetStyle,
      sv::EventControl resetEdge, Value reset, std::function<void(void)> body,
      std::function<void(void)> resetBody) {
  ...
  void FIRRTLLowering::addIfProceduralBlock(
      Value cond, std::function<void(void)> thenCtor,
      std::function<void(void)> elseCtor) {
  ...
  /// lowering.
  ///
  FIRRTLLowering::UnloweredOpResult FIRRTLLowering::handleUnloweredOp(
      Operation * op) {
  ...
  LogicalResult FIRRTLLowering::visitExpr(SpecialConstantOp op) {
    return setLowering(
        op, getOrCreateIntConstant(APInt(/*bitWidth*/ 1, op.value())));
  }
  ...
  LogicalResult FIRRTLLowering::visitExpr(SubfieldOp op) {
    ...
    if (value.getType().isa<sv::InOutType>()) {
      auto field = op.input().getType().cast<BundleType>().getElementName(
          op.fieldIndex());
    ...
    return setLoweringTo<hw::StructExtractOp>(
        op, resultType, value,
        op.input().getType().cast<BundleType>().getElementName(
            op.fieldIndex()));
  }
  ...
  LogicalResult FIRRTLLowering::visitDecl(RegResetOp op) {
    ...
    if (op.resetSignal().getType().isa<AsyncResetType>()) {
      if (!firrtl::isConstant(op.resetValue())) {
        ...
        diag.attachNote(op.resetValue().getLoc())
      ...
      for (size_t i = 0, e = op.getNumResults(); i != e; ++i) {
        ...
        auto addInput = [&](StringRef portLabel, StringRef portLabel2,
          ...
          argNames.push_back(builder.getStringAttr(
              portLabel + Twine(portNumber) + portLabel2));
        };
  ...
  LogicalResult FIRRTLLowering::visitExpr(HWStructCastOp op) {
    ...
    if (auto opStructType =
            op.getOperand().getType().dyn_cast<hw::StructType>())
  ...
  LogicalResult FIRRTLLowering::visitExpr(XorRPrimOp op) {
    ...
    return setLoweringTo<comb::ParityOp>(op, builder.getIntegerType(1),
                                         operand);
  }
  ...
  /// specified binary operator.
  LogicalResult FIRRTLLowering::lowerCmpOp(
      Operation * op, ICmpPredicate signedOp, ICmpPredicate unsignedOp) {
    ...
    return setLoweringTo<comb::ICmpOp>(
        op, resultType, lhsIntType.isSigned() ? signedOp : unsignedOp, lhs,
        rhs);
  }
  ...
  LogicalResult FIRRTLLowering::lowerVerificationStatement(
    ...
    auto emit = [&]() {
      ...
      // Handle the `ifElseFatal` format, which does not emit an SVA but rather
      // a process that uses $error and $fatal to perform the checks.
    ...
    // This is a bit awkward since we want to translate an array of guards into
    // a recursive call to `addToIfDefBlock`.
  ...
  LogicalResult FIRRTLLowering::visitStmt(CoverOp op) {
    return lowerVerificationStatement(op, "cover__", op.clock(), op.predicate(),
                                      op.enable(), op.messageAttr(),
                                      op.operands(), op.nameAttr(),
                                      op.isConcurrent(), op.eventControl());
  }
