...
// Cache explicitly clocked atoms while lowering one property. The original
// LTL value maps to its sampled i1 value and validity requirements so repeated
// uses share the same sampling register.
...
// clock event. For example:
//   createPast(input, [(clockA, posedge, 2), (clockB, negedge, 1)])
// produces:
//   input -> clockA.posedge reg -> clockA.posedge reg -> clockB.negedge reg
...
static void lowerTemporalLTLToCore(hw::HWModuleOp module) {
  ...
  for (auto *op : assertLikes) {
    ...
    if (failed(lowered))
      continue;
    // `valid` is false while lowering-generated registers only contain their
    // initial values, and true once all required sampled history is available.
