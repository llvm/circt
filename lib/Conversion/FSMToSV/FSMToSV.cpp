...
LogicalResult MachineOpConverter::dispatch() {
  ...
  for (auto args :
       llvm::zip(machineOp.getArguments(), hwModuleOp.getArguments_HWML())) {
    auto machineArg = std::get<0>(args);
    auto hwModuleArg = std::get<1>(args);
    machineArg.replaceAllUsesWith(hwModuleArg);
  }

  auto clock = hwModuleOp.getArgument_HWML(clkRstIdxs.clockIdx);
  auto reset = hwModuleOp.getArgument_HWML(clkRstIdxs.resetIdx);
