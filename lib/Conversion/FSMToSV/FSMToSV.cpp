...
LogicalResult MachineOpConverter::dispatch() {
  ...
  auto stateNext =
      buildStateCaseMux(loc, stateReg, {nextStateFromState}, [&](size_t) {
        return b.getStringAttr("next_state");
      })[0];
  nextStateBackedge.setValue(b.create<sv::ReadInOutOp>(loc, stateNext));

  // Create output port assignments.
  llvm::SmallVector<Value> outputValues =
      getOutputAssignments(stateConvResults);
