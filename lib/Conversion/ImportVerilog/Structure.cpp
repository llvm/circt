...
Context::convertModuleHeader(const slang::ast::InstanceBodySymbol *module) {
  ...
  // It's used to tag where a hierarchical name is on the output ports.
...
Context::convertModuleBody(const slang::ast::InstanceBodySymbol *module) {
  ...
  // Because we mark all hierarchical names as the module's output ports, we
  // should ensure the number of operands of this module's terminator and its
  // output ports' number remain consistent.
  for (auto &hierPath : hierPaths[module])
