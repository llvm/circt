...
LogicalResult Converter::absorbRegs(HWModuleOp module) {
  auto emptyNameAttr = StringAttr::get(module.getContext(), "");
  ...
  for (auto &arc : arcUses) {
    ...
    for (auto result : arc.getResults()) {
      ...
      absorbedNames[result.getResultNumber()] =
          tapRegisters ? regOp.getNameAttr() : emptyNameAttr;
    }
  ...
  for (auto [clockAndResetAndOp, regOps] : regsByInput) {
    ...
    for (auto regOp : regOps) {
      ...
      if (it == mapping.end()) {
        ...
        names.push_back(tapRegisters ? regOp->getAttrOfType<StringAttr>("name")
                                     : emptyNameAttr);
      }
