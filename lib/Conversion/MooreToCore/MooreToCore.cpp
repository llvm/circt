...
struct ReadMemBIOpConversion : public OpConversionPattern<ReadMemBIOp> {
  ...
  matchAndRewrite(ReadMemBIOp op, OpAdaptor adaptor,
    ...
    // Type analysis
    auto destRefTy = dyn_cast<llhd::RefType>(adaptor.getDest().getType());
    if (!destRefTy)
      return rewriter.notifyMatchFailure(op, "unsupported destination type");

    auto queueTy = dyn_cast<sim::QueueType>(destRefTy.getNestedType());
    bool isQueue = static_cast<bool>(queueTy);
    SmallVector<uint64_t> dims;
    Type elemTy;

    if (isQueue) {
      dims.push_back(0);
      elemTy = cast<sim::QueueType>(destRefTy.getNestedType()).getElementType();
    } else {
      Type t = destRefTy.getNestedType();
      while (auto arr = dyn_cast<hw::ArrayType>(t)) {
        if (dims.size() == op.getDimLows().size())
          break;
    ...
    ArrayRef<bool> dimDescs = op.getDimDescending();
    int64_t lo0 = op.getDimLows()[0];
    bool desc0 = dimDescs[0];
    uint64_t wordsPerEntry = 1;

    for (unsigned k = 1; k < dims.size(); ++k)
      wordsPerEntry *= dims[k];

    int64_t elemW = hw::getBitWidth(elemTy);
    if (elemW <= 0)
      return rewriter.notifyMatchFailure(op, "unsupported element type");

    bool isHex = (op.getBase() == moore::MemBase::Hex);
    int shift = isHex ? 4 : 1;
    unsigned accW = std::max<unsigned>(elemW, 64);

    // Types
    IntegerType i1Ty = rewriter.getI1Type();
    ...
    IntegerType iNTy = rewriter.getIntegerType(elemW);
    IntegerType accTy = rewriter.getIntegerType(accW);

    bool hasStart = (bool)adaptor.getStartAddr();
    bool hasFinish = (bool)adaptor.getFinishAddr();

    // Helpers
    auto cstI = [&](IntegerType t, int64_t v) -> Value {
      return hw::ConstantOp::create(rewriter, loc, t, v);
    };
    auto cst8 = [&](int64_t v) { return cstI(i8Ty, v); };
    auto cst64 = [&](int64_t v) { return cstI(i64Ty, v); };
    auto cstAcc = [&](int64_t v) { return cstI(accTy, v); };

    Value lo0C, hi0C;
    if (isQueue) {
      lo0C = cst64(0);
      Value q0 = llhd::ProbeOp::create(rewriter, loc, adaptor.getDest());
      Value size = sim::QueueSizeOp::create(rewriter, loc, q0);
      hi0C = arith::SubIOp::create(rewriter, loc, size, cst64(1));
    } else {
      lo0C = cst64(lo0);
      hi0C = cst64(lo0 + (int64_t)dims[0] - 1);
    }

    auto svToHw = [&](Value sv) -> Value {
      if (isQueue)
        return sv;
      if (desc0)
        return arith::SubIOp::create(rewriter, loc, sv, lo0C);
      return arith::SubIOp::create(rewriter, loc, hi0C, sv);
    };

    Value winLo = lo0C, winHi = hi0C;
    if (adaptor.getSliceLeft()) {
      Value a =
          arith::ExtSIOp::create(rewriter, loc, i64Ty, adaptor.getSliceLeft());
      Value b =
          arith::ExtSIOp::create(rewriter, loc, i64Ty, adaptor.getSliceRight());
      Value aLeB =
          arith::CmpIOp::create(rewriter, loc, arith::CmpIPredicate::sle, a, b);
      winLo = arith::SelectOp::create(rewriter, loc, aLeB, a, b);
      winHi = arith::SelectOp::create(rewriter, loc, aLeB, b, a);
    }

    Value startSv = hasStart ? arith::ExtSIOp::create(rewriter, loc, i64Ty,
                                                      adaptor.getStartAddr())
                                   .getResult()
                             : winLo;
    Value finishSv = hasFinish ? arith::ExtSIOp::create(rewriter, loc, i64Ty,
                                                        adaptor.getFinishAddr())
                                     .getResult()
                               : winHi;

    Value up = hasFinish ? arith::CmpIOp::create(rewriter, loc,
                                                 arith::CmpIPredicate::sle,
                                                 startSv, finishSv)
                         : cstI(i1Ty, 1);

    auto inRange = [&](Value a, Value lo, Value hi) {
      Value lok = arith::CmpIOp::create(rewriter, loc,
                                        arith::CmpIPredicate::sge, a, lo);
      Value hok = arith::CmpIOp::create(rewriter, loc,
                                        arith::CmpIPredicate::sle, a, hi);
      return arith::AndIOp::create(rewriter, loc, lok, hok).getResult();
    };

    Value argsOk = cstI(i1Ty, 1);
    auto andOk = [&](Value c) {
      argsOk = arith::AndIOp::create(rewriter, loc, argsOk, c);
    };

    if (adaptor.getSliceLeft()) {
      andOk(inRange(winLo, lo0C, hi0C));
      andOk(inRange(winHi, lo0C, hi0C));
    }

    if (hasStart)
      andOk(inRange(startSv, winLo, winHi));
    if (hasFinish)
      andOk(inRange(finishSv, winLo, winHi));

    auto eqChar = [&](Value b, uint8_t c) -> Value {
      return arith::CmpIOp::create(rewriter, loc, arith::CmpIPredicate::eq, b,
                                   cst8(c));
    };

    auto isWS = [&](Value b) -> Value {
      Value a =
          arith::OrIOp::create(rewriter, loc, eqChar(b, ' '), eqChar(b, '\t'));
      Value c =
          arith::OrIOp::create(rewriter, loc, eqChar(b, '\n'), eqChar(b, '\r'));
      return arith::OrIOp::create(rewriter, loc,
                                  arith::OrIOp::create(rewriter, loc, a, c),
                                  eqChar(b, '\f'));
    };

    auto decodeHex = [&](Value b) -> std::pair<Value, Value> {
      using P = arith::CmpIPredicate;
      Value in09 = arith::AndIOp::create(
          rewriter, loc,
          arith::CmpIOp::create(rewriter, loc, P::uge, b, cst8('0')),
          arith::CmpIOp::create(rewriter, loc, P::ule, b, cst8('9')));
      Value inaf = arith::AndIOp::create(
          rewriter, loc,
          arith::CmpIOp::create(rewriter, loc, P::uge, b, cst8('a')),
          arith::CmpIOp::create(rewriter, loc, P::ule, b, cst8('f')));
      Value inAF = arith::AndIOp::create(
          rewriter, loc,
          arith::CmpIOp::create(rewriter, loc, P::uge, b, cst8('A')),
          arith::CmpIOp::create(rewriter, loc, P::ule, b, cst8('F')));
      Value valid = arith::OrIOp::create(
          rewriter, loc, arith::OrIOp::create(rewriter, loc, in09, inaf), inAF);
      Value d09 = arith::ExtUIOp::create(
          rewriter, loc, i64Ty,
          arith::SubIOp::create(rewriter, loc, b, cst8('0')));
      Value daf = arith::ExtUIOp::create(
          rewriter, loc, i64Ty,
          arith::AddIOp::create(
              rewriter, loc, arith::SubIOp::create(rewriter, loc, b, cst8('a')),
              cst8(10)));
      Value dAF = arith::ExtUIOp::create(
          rewriter, loc, i64Ty,
          arith::AddIOp::create(
              rewriter, loc, arith::SubIOp::create(rewriter, loc, b, cst8('A')),
              cst8(10)));
      Value digit = arith::SelectOp::create(
          rewriter, loc, in09, d09,
          arith::SelectOp::create(rewriter, loc, inaf, daf, dAF));
      return {digit, valid};
    };

    auto decodeBin = [&](Value b) -> std::pair<Value, Value> {
      Value is1 = eqChar(b, '1');
      Value valid = arith::OrIOp::create(rewriter, loc, eqChar(b, '0'), is1);
      Value digit = arith::ExtUIOp::create(
          rewriter, loc, i64Ty,
          arith::SelectOp::create(rewriter, loc, is1, cst8(1), cst8(0)));
      return {digit, valid};
    };

    auto decodeDigit = [&](Value b) {
      return isHex ? decodeHex(b) : decodeBin(b);
    };

    auto isXZ = [&](Value b) -> Value {
      return arith::OrIOp::create(
          rewriter, loc,
          arith::OrIOp::create(rewriter, loc, eqChar(b, 'x'), eqChar(b, 'X')),
          arith::OrIOp::create(rewriter, loc, eqChar(b, 'z'), eqChar(b, 'Z')));
    };

    // Open file (fopen).
    auto fd = sim::SVFOpenOp::create(
        rewriter, loc, adaptor.getFilename(),
        sim::SVFOpenModeAttr::get(ctx, sim::SVFOpenMode::Read));
    Value fdOk = arith::CmpIOp::create(rewriter, loc, arith::CmpIPredicate::ne,
                                       fd.getResult(), cstI(i32Ty, 0));
    Value stream; // assign in ^streamOk.

    auto emitMsg = [&](StringRef prefix, StringRef suffix) {
      Value pre = sim::FormatLiteralOp::create(rewriter, loc, prefix);
      Value fn = sim::FormatStringOp::create(
          rewriter, loc, adaptor.getFilename(), rewriter.getBoolAttr(false),
          rewriter.getI8IntegerAttr(32), IntegerAttr());
      Value post = sim::FormatLiteralOp::create(rewriter, loc, suffix);
      Value msg = sim::FormatStringConcatOp::create(rewriter, loc,
                                                    ValueRange{pre, fn, post});
      sim::PrintFormattedProcOp::create(rewriter, loc, msg);
    };

    // Split: everything after op goes to contBlock
    Block *entryBlock = rewriter.getInsertionBlock();
    Block *contBlock = rewriter.splitBlock(entryBlock, Block::iterator(op));

    // Pre-allocate all blocks
    SmallVector<Location> L2{loc, loc}, L3{loc, loc, loc};
    SmallVector<Location> L4{loc, loc, loc, loc}, L5{loc, loc, loc, loc, loc};
    SmallVector<Location> L6{loc, loc, loc, loc, loc, loc};
    auto mkBlk = [&](ArrayRef<Type> tys, ArrayRef<Location> ls) {
      return rewriter.createBlock(contBlock, tys, ls);
    };

    // Prologue gates
    Block *argsChkBlock = rewriter.createBlock(contBlock);
    Block *streamOkBlock = rewriter.createBlock(contBlock);
    Block *openFailBlock = rewriter.createBlock(contBlock);
    // Token dispatch
    Block *startBlock = mkBlk({i64Ty, i64Ty, i1Ty, i64Ty}, L4);
    Block *chkSlashBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *chkAtBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *chkDigitBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *notAtBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    // Comments
    Block *seenSlashBlock = mkBlk({i64Ty, i64Ty, i1Ty, i64Ty}, L4);
    Block *chkComBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *chkStarComBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *lineComBlock = mkBlk({i64Ty, i64Ty, i1Ty, i64Ty}, L4);
    Block *lcByteBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *blockComBlock = mkBlk({i64Ty, i64Ty, i1Ty, i64Ty}, L4);
    Block *bcByteBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *seenStarBlock = mkBlk({i64Ty, i64Ty, i1Ty, i64Ty}, L4);
    Block *starByteBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    Block *starElseBlock = mkBlk({i64Ty, i64Ty, i8Ty, i1Ty, i64Ty}, L5);
    // @addr parsing
    Block *addrFirstBlock = mkBlk({i1Ty, i64Ty}, L2);
    Block *addrFirstByteBlock = mkBlk({i8Ty, i1Ty, i64Ty}, L3);
    Block *readAddrBlock = mkBlk({i64Ty, i1Ty, i64Ty}, L3);
    Block *addrByteBlock = mkBlk({i64Ty, i8Ty, i1Ty, i64Ty}, L4);
    Block *addrAccBlock = mkBlk({i64Ty, i1Ty, i64Ty}, L3);
    Block *validateJmpBlock = mkBlk({i64Ty, i1Ty, i64Ty}, L3);
    Block *validateRedispBlock = mkBlk({i64Ty, i8Ty, i1Ty, i64Ty}, L4);
    // Value parsing
    Block *readValueBlock = mkBlk({i64Ty, i64Ty, accTy, i1Ty, i64Ty}, L5);
    Block *valueByteBlock = mkBlk({i64Ty, i64Ty, accTy, i8Ty, i1Ty, i64Ty}, L6);
    // Commit
    Block *commitBlock = mkBlk({i64Ty, i64Ty, accTy, i1Ty, i64Ty}, L5);
    Block *commitRedispBlock =
        mkBlk({i64Ty, i64Ty, accTy, i8Ty, i1Ty, i64Ty}, L6);
    // Epilogue
    Block *epilogueBlock = rewriter.createBlock(contBlock, {i1Ty, i64Ty}, L2);
    Block *warnBlock = rewriter.createBlock(contBlock);
    Block *tooManyBlock = rewriter.createBlock(contBlock);
    Block *fcloseBlock = rewriter.createBlock(contBlock);
    Block *errorBlock = rewriter.createBlock(contBlock);

    // Helper: emit sim.proc.read at the current insertion point
    // Returns (byte: i8, eof: i1)
    auto readByte = [&]() -> std::pair<Value, Value> {
      auto readOp = sim::ProcReadOp::create(rewriter, loc, stream);
      return {readOp.getResult(), readOp.getEof()};
    };

    // entry -> check fd
    rewriter.setInsertionPointToEnd(entryBlock);
    cf::CondBranchOp::create(rewriter, loc, fdOk, argsChkBlock, ValueRange{},
                             openFailBlock, ValueRange{});

    // ^argsChk: slice must lie within the array, start/finish within the
    // window.
    rewriter.setInsertionPointToEnd(argsChkBlock);
    cf::CondBranchOp::create(rewriter, loc, argsOk, streamOkBlock, ValueRange{},
                             errorBlock, ValueRange{});

    // ^streamOk: wrap the file descriptor into a readable byte stream.
    rewriter.setInsertionPointToEnd(streamOkBlock);
    stream =
        sim::SVChannelToInputStreamOp::create(rewriter, loc, fd.getResult())
            .getStream();
    cf::BranchOp::create(
        rewriter, loc, startBlock,
        ValueRange{startSv, cst64(0), cstI(i1Ty, 0), cst64(0)});

    // ^openfail: fd invalid
    rewriter.setInsertionPointToEnd(openFailBlock);
    {
      emitMsg("$readmem(\"", "\"): error: cannot open file\n");
      cf::BranchOp::create(rewriter, loc, contBlock);
    }

    // ^start(%cur, %inner, %seenAt, %wordCount): read one byte
    rewriter.setInsertionPointToEnd(startBlock);
    {
      Value cur = startBlock->getArgument(0);
      Value inner = startBlock->getArgument(1);
      Value seenAt = startBlock->getArgument(2);
      Value wordCount = startBlock->getArgument(3);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(rewriter, loc, eof, epilogueBlock,
                               ValueRange{seenAt, wordCount}, chkSlashBlock,
                               ValueRange{cur, inner, byte, seenAt, wordCount});
    }

    // ^chkSlash(%cur, %inner, %b, %seenAt, %wordCount): whitespace -> ^start;
    // else -> ^chkAt
    rewriter.setInsertionPointToEnd(chkSlashBlock);
    {
      Value cur = chkSlashBlock->getArgument(0);
      Value inner = chkSlashBlock->getArgument(1);
      Value b = chkSlashBlock->getArgument(2);
      Value seenAt = chkSlashBlock->getArgument(3);
      Value wordCount = chkSlashBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, isWS(b), startBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               chkAtBlock,
                               ValueRange{cur, inner, b, seenAt, wordCount});
    }

    // ^chkAt(%cur, %inner, %b, %seenAt, %wordCount): '/' -> ^seenSlash; else ->
    // ^chkDigit
    rewriter.setInsertionPointToEnd(chkAtBlock);
    {
      Value cur = chkAtBlock->getArgument(0);
      Value inner = chkAtBlock->getArgument(1);
      Value b = chkAtBlock->getArgument(2);
      Value seenAt = chkAtBlock->getArgument(3);
      Value wordCount = chkAtBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '/'), seenSlashBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               chkDigitBlock,
                               ValueRange{cur, inner, b, seenAt, wordCount});
    }

    // ^chkDigit(%cur, %inner, %b, %seenAt, %wordCount): '@' ->
    // ^addrFirst(cur/inner dropped, seenAt set); else -> ^notAt
    rewriter.setInsertionPointToEnd(chkDigitBlock);
    {
      Value cur = chkDigitBlock->getArgument(0);
      Value inner = chkDigitBlock->getArgument(1);
      Value b = chkDigitBlock->getArgument(2);
      Value seenAt = chkDigitBlock->getArgument(3);
      Value wordCount = chkDigitBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '@'), addrFirstBlock,
                               ValueRange{cstI(i1Ty, 1), wordCount}, notAtBlock,
                               ValueRange{cur, inner, b, seenAt, wordCount});
    }

    // ^notAt(%cur, %inner, %b, %seenAt, %wordCount): digit or xz starts a value
    // token -> ^valueByte; anything else is malformed -> ^error
    rewriter.setInsertionPointToEnd(notAtBlock);
    {
      Value cur = notAtBlock->getArgument(0);
      Value inner = notAtBlock->getArgument(1);
      Value b = notAtBlock->getArgument(2);
      Value seenAt = notAtBlock->getArgument(3);
      Value wordCount = notAtBlock->getArgument(4);
      Value validDigit = decodeDigit(b).second;
      Value canStart = arith::OrIOp::create(rewriter, loc, validDigit, isXZ(b));
      cf::CondBranchOp::create(
          rewriter, loc, canStart, valueByteBlock,
          ValueRange{cur, inner, cstAcc(0), b, seenAt, wordCount}, errorBlock,
          ValueRange{});
    }

    // ^seenSlash(%cur, %inner, %seenAt, %wordCount): read next byte
    rewriter.setInsertionPointToEnd(seenSlashBlock);
    {
      Value cur = seenSlashBlock->getArgument(0);
      Value inner = seenSlashBlock->getArgument(1);
      Value seenAt = seenSlashBlock->getArgument(2);
      Value wordCount = seenSlashBlock->getArgument(3);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(rewriter, loc, eof, errorBlock, ValueRange{},
                               chkComBlock,
                               ValueRange{cur, inner, byte, seenAt, wordCount});
    }

    // ^chkCom(%cur, %inner, %b, %seenAt, %wordCount): '/' -> line comment; else
    // -> ^chkStarCom.
    rewriter.setInsertionPointToEnd(chkComBlock);
    {
      Value cur = chkComBlock->getArgument(0);
      Value inner = chkComBlock->getArgument(1);
      Value b = chkComBlock->getArgument(2);
      Value seenAt = chkComBlock->getArgument(3);
      Value wordCount = chkComBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '/'), lineComBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               chkStarComBlock,
                               ValueRange{cur, inner, b, seenAt, wordCount});
    }

    // ^chkStarCom(%cur, %inner, %b, %seenAt, %wordCount): '*' -> block comment;
    // else marlformed -> ^error.
    rewriter.setInsertionPointToEnd(chkStarComBlock);
    {
      Value cur = chkStarComBlock->getArgument(0);
      Value inner = chkStarComBlock->getArgument(1);
      Value b = chkStarComBlock->getArgument(2);
      Value seenAt = chkStarComBlock->getArgument(3);
      Value wordCount = chkStarComBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '*'), blockComBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               errorBlock, ValueRange{});
    }

    // ^lineComment(%cur, %inner, %seenAt, %wordCount): skip until '\n'
    rewriter.setInsertionPointToEnd(lineComBlock);
    {
      Value cur = lineComBlock->getArgument(0);
      Value inner = lineComBlock->getArgument(1);
      Value seenAt = lineComBlock->getArgument(2);
      Value wordCount = lineComBlock->getArgument(3);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(rewriter, loc, eof, epilogueBlock,
                               ValueRange{seenAt, wordCount}, lcByteBlock,
                               ValueRange{cur, inner, byte, seenAt, wordCount});
    }

    //^lcByte(%cur, %inner, %b, %seenAt, %wordCount): '\n' ends the line comment
    //-> ^start; else keep skipping.
    rewriter.setInsertionPointToEnd(lcByteBlock);
    {
      Value cur = lcByteBlock->getArgument(0);
      Value inner = lcByteBlock->getArgument(1);
      Value b = lcByteBlock->getArgument(2);
      Value seenAt = lcByteBlock->getArgument(3);
      Value wordCount = lcByteBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '\n'), startBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               lineComBlock,
                               ValueRange{cur, inner, seenAt, wordCount});
    }

    // ^blockComment(%cur, %inner, %seenAt, %wordCount): skip until '*'
    rewriter.setInsertionPointToEnd(blockComBlock);
    {
      Value cur = blockComBlock->getArgument(0);
      Value inner = blockComBlock->getArgument(1);
      Value seenAt = blockComBlock->getArgument(2);
      Value wordCount = blockComBlock->getArgument(3);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(rewriter, loc, eof, errorBlock, ValueRange{},
                               bcByteBlock,
                               ValueRange{cur, inner, byte, seenAt, wordCount});
    ...
    // ^bcByte(%cur, %inner, %b, %seenAt, %wordCount): '*' may close the block
    // comment -> ^start: else ^starElse.
    rewriter.setInsertionPointToEnd(bcByteBlock);
    {
      Value cur = bcByteBlock->getArgument(0);
      Value inner = bcByteBlock->getArgument(1);
      Value b = bcByteBlock->getArgument(2);
      Value seenAt = bcByteBlock->getArgument(3);
      Value wordCount = bcByteBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '*'), seenStarBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               blockComBlock,
                               ValueRange{cur, inner, seenAt, wordCount});
    }

    // ^seenStar(%cur, %inner, %seenAt, %wordCount): read next byte; '/' -> end;
    // '*' -> loop; else -> blockComment
    rewriter.setInsertionPointToEnd(seenStarBlock);
    {
      Value cur = seenStarBlock->getArgument(0);
      Value inner = seenStarBlock->getArgument(1);
      Value seenAt = seenStarBlock->getArgument(2);
      Value wordCount = seenStarBlock->getArgument(3);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(rewriter, loc, eof, errorBlock, ValueRange{},
                               starByteBlock,
                               ValueRange{cur, inner, byte, seenAt, wordCount});
    ...
    // ^starByte(%cur, %inner, %b, %seenAt, %wordCount): '/' closes the comment
    // -> ^start; else ^starElse.
    rewriter.setInsertionPointToEnd(starByteBlock);
    {
      Value cur = starByteBlock->getArgument(0);
      Value inner = starByteBlock->getArgument(1);
      Value b = starByteBlock->getArgument(2);
      Value seenAt = starByteBlock->getArgument(3);
      Value wordCount = starByteBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '/'), startBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               starElseBlock,
                               ValueRange{cur, inner, b, seenAt, wordCount});
    }

    // ^starElse(%cur, %inner, %b, %seenAt, %wordCount): '*' -> recheck
    // ^seenStar; else back to ^blockComment.
    rewriter.setInsertionPointToEnd(starElseBlock);
    {
      Value cur = starElseBlock->getArgument(0);
      Value inner = starElseBlock->getArgument(1);
      Value b = starElseBlock->getArgument(2);
      Value seenAt = starElseBlock->getArgument(3);
      Value wordCount = starElseBlock->getArgument(4);
      cf::CondBranchOp::create(rewriter, loc, eqChar(b, '*'), seenStarBlock,
                               ValueRange{cur, inner, seenAt, wordCount},
                               blockComBlock,
                               ValueRange{cur, inner, seenAt, wordCount});
    ...
    // ^addrFirst(%seenAt, %wordCount): read the byte right after `@`: requires
    // at least one hex digit with no intervening white sapce (EOF here ->
    // ^error)
    rewriter.setInsertionPointToEnd(addrFirstBlock);
    {
      Value seenAt = addrFirstBlock->getArgument(0);
      Value wordCount = addrFirstBlock->getArgument(1);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(rewriter, loc, eof, errorBlock, ValueRange{},
                               addrFirstByteBlock,
                               ValueRange{byte, seenAt, wordCount});
    }

    // ^addrFirstByte(%b, %seenAt, %wordCount): first address digit; non-hex ->
    // ^error.
    rewriter.setInsertionPointToEnd(addrFirstByteBlock);
    {
      Value b = addrFirstByteBlock->getArgument(0);
      Value seenAt = addrFirstByteBlock->getArgument(1);
      Value wordCount = addrFirstByteBlock->getArgument(2);
      auto [digit, valid] = decodeHex(b);
      cf::CondBranchOp::create(rewriter, loc, valid, addrAccBlock,
                               ValueRange{digit, seenAt, wordCount}, errorBlock,
                               ValueRange{});
    }

    // ^readAddr(%acc, %seenAt, %wordCount): read next hex digit
    rewriter.setInsertionPointToEnd(readAddrBlock);
    {
      Value acc = readAddrBlock->getArgument(0);
      Value seenAt = readAddrBlock->getArgument(1);
      Value wordCount = readAddrBlock->getArgument(2);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(rewriter, loc, eof, validateJmpBlock,
                               ValueRange{acc, seenAt, wordCount},
                               addrByteBlock,
                               ValueRange{acc, byte, seenAt, wordCount});
    }

    // ^addrByte(%acc, %b, %seenAt, %wordCount): accumulate a hex digit, or end
    // the address and re-dispatch the delimiter byte via ^validateRedisp
    rewriter.setInsertionPointToEnd(addrByteBlock);
    {
      Value acc = addrByteBlock->getArgument(0);
      Value b = addrByteBlock->getArgument(1);
      Value seenAt = addrByteBlock->getArgument(2);
      Value wordCount = addrByteBlock->getArgument(3);

      auto [digit, valid] = decodeHex(b);
      Value newAcc = arith::OrIOp::create(
          rewriter, loc, arith::ShLIOp::create(rewriter, loc, acc, cst64(4)),
          digit);
      cf::CondBranchOp::create(rewriter, loc, valid, addrAccBlock,
                               ValueRange{newAcc, seenAt, wordCount},
                               validateRedispBlock,
                               ValueRange{acc, b, seenAt, wordCount});
    }

    // ^addrAcc(%newAcc, %seenAt, %wordCount): reject addresses beyond the
    // window early, before the accumulator can overflow i64.
    rewriter.setInsertionPointToEnd(addrAccBlock);
    {
      Value newAcc = addrAccBlock->getArgument(0);
      Value seenAt = addrAccBlock->getArgument(1);
      Value wordCount = addrAccBlock->getArgument(2);
      Value tooBig = arith::CmpIOp::create(
          rewriter, loc, arith::CmpIPredicate::sgt, newAcc, winHi);
      cf::CondBranchOp::create(rewriter, loc, tooBig, errorBlock, ValueRange{},
                               readAddrBlock,
                               ValueRange{newAcc, seenAt, wordCount});
    }

    auto emitAddrCheck = [&](Value fileAddr) -> Value {
      Value loB = arith::SelectOp::create(rewriter, loc, up, startSv, finishSv);
      Value hiB = arith::SelectOp::create(rewriter, loc, up, finishSv, startSv);
      return inRange(fileAddr, loB, hiB);
    };

    // ^validateJmp(%fileAddr, %seenAt, %wordCount): EOF ended the address;
    // check it against the range implied by the task arguments, then resume at
    // ^start.
    rewriter.setInsertionPointToEnd(validateJmpBlock);
    {
      Value fileAddr = validateJmpBlock->getArgument(0);
      Value seenAt = validateJmpBlock->getArgument(1);
      Value wordCount = validateJmpBlock->getArgument(2);

      cf::CondBranchOp::create(
          rewriter, loc, emitAddrCheck(fileAddr), startBlock,
          ValueRange{fileAddr, cst64(0), seenAt, wordCount}, errorBlock,
          ValueRange{});
    }

    // ^validateRedisp(%fileAddr, %b, %seenAt, %wordCount): a delimiter ended
    // the address; validate it, the re-dispatch the delimiter byte through
    // ^chkSlash.
    rewriter.setInsertionPointToEnd(validateRedispBlock);
    {
      Value fileAddr = validateRedispBlock->getArgument(0);
      Value b = validateRedispBlock->getArgument(1);
      Value seenAt = validateRedispBlock->getArgument(2);
      Value wordCount = validateRedispBlock->getArgument(3);
      cf::CondBranchOp::create(
          rewriter, loc, emitAddrCheck(fileAddr), chkSlashBlock,
          ValueRange{fileAddr, cst64(0), b, seenAt, wordCount}, errorBlock,
          ValueRange{});
    }

    // Value parsing
    // ^readValue(%cur, %inner, %acc, %seenAt, %wordCount): read next digit byte
    rewriter.setInsertionPointToEnd(readValueBlock);
    {
      Value cur = readValueBlock->getArgument(0);
      Value inner = readValueBlock->getArgument(1);
      Value acc = readValueBlock->getArgument(2);
      Value seenAt = readValueBlock->getArgument(3);
      Value wordCount = readValueBlock->getArgument(4);
      auto [byte, eof] = readByte();
      cf::CondBranchOp::create(
          rewriter, loc, eof, commitBlock,
          ValueRange{cur, inner, acc, seenAt, wordCount}, valueByteBlock,
          ValueRange{cur, inner, acc, byte, seenAt, wordCount});
    }

    // ^valueByte(%cur, %inner, %acc, %b, %seenAt, %wordCount): digit -> shift
    // acc and insert; x/z -> shift only (loads as 0); '_' -> keep acc; any
    // other byte ends the token with the previous acc -> ^commitRedisp.
    rewriter.setInsertionPointToEnd(valueByteBlock);
    {
      Value cur = valueByteBlock->getArgument(0);
      Value inner = valueByteBlock->getArgument(1);
      Value acc = valueByteBlock->getArgument(2);
      Value b = valueByteBlock->getArgument(3);
      Value seenAt = valueByteBlock->getArgument(4);
      Value wordCount = valueByteBlock->getArgument(5);

      auto [digit64, validDigit] = decodeDigit(b);
      Value digitAcc =
          accW == 64 ? digit64
                     : arith::ExtUIOp::create(rewriter, loc, accTy, digit64)
                           .getResult();
      Value shiftAcc = cstAcc(shift);
      Value shifted = arith::ShLIOp::create(rewriter, loc, acc, shiftAcc);
      Value withDig = arith::OrIOp::create(rewriter, loc, shifted, digitAcc);

      Value isXZv = isXZ(b);
      Value isUnd = eqChar(b, '_');
      Value isCont = arith::OrIOp::create(
          rewriter, loc, arith::OrIOp::create(rewriter, loc, validDigit, isXZv),
          isUnd);
      Value newAcc = arith::SelectOp::create(
          rewriter, loc, isUnd, acc,
          arith::SelectOp::create(rewriter, loc, isXZv, shifted, withDig));

      cf::CondBranchOp::create(
          rewriter, loc, isCont, readValueBlock,
          ValueRange{cur, inner, newAcc, seenAt, wordCount}, commitRedispBlock,
          ValueRange{cur, inner, acc, b, seenAt, wordCount});
    }

    // Shared commit logic: count the word, stop (-> ^tooMany) once `cur` hast
    // left the requeste window, otherwise check enum membership, store into
    // dest[cur][inner...] (queue.set for queues, sig.array_get chain with
    // row-major decomposition of `inner` for arrays), and advance (inner, cur).
    // `continueTo` emits the terminator.
    auto emitGuardedCommit =
        [&](Value cur, Value inner, Value val, Value seenAt, Value wordCount,
            function_ref<void(Value nextCur, Value nextInner, Value newCount)>
                continueTo) {
          Value newCount =
              arith::AddIOp::create(rewriter, loc, wordCount, cst64(1));
          Value pAsc = arith::CmpIOp::create(
              rewriter, loc, arith::CmpIPredicate::sgt, cur, finishSv);
          Value pDesc = arith::CmpIOp::create(
              rewriter, loc, arith::CmpIPredicate::slt, cur, finishSv);
          Value past = arith::SelectOp::create(rewriter, loc, up, pAsc, pDesc);

          Block *writeBlk;
          {
            OpBuilder::InsertionGuard g(rewriter);
            writeBlk = rewriter.createBlock(contBlock);
          }
          cf::CondBranchOp::create(rewriter, loc, past, tooManyBlock,
                                   ValueRange{}, writeBlk, ValueRange{});
          rewriter.setInsertionPointToEnd(writeBlk);

          Value elemInt =
              accW == (unsigned)elemW
                  ? val
                  : arith::TruncIOp::create(rewriter, loc, iNTy, val)
                        .getResult();

          if (auto enumVals = op.getEnumValues()) {
            Value member = cstI(i1Ty, 0);
            for (int64_t v : *enumVals)
              member = arith::OrIOp::create(
                  rewriter, loc, member,
            ...
            {
              OpBuilder::InsertionGuard g(rewriter);
              storeBlk = rewriter.createBlock(contBlock);
            }
            cf::CondBranchOp::create(rewriter, loc, member, storeBlk,
                                     ValueRange{}, errorBlock, ValueRange{});
            rewriter.setInsertionPointToEnd(storeBlk);
          }

          Value elemVal =
              elemTy == iNTy
                  ? elemInt
                  : hw::BitcastOp::create(rewriter, loc, elemTy, elemInt)
                        .getResult();
          Value delay = llhd::ConstantTimeOp::create(
              rewriter, loc, llhd::TimeAttr::get(ctx, 0U, "ns", 0, 1));
          Value hw0 = svToHw(cur);

          if (isQueue) {
            Value q = llhd::ProbeOp::create(rewriter, loc, adaptor.getDest());
            Value idx = arith::TruncIOp::create(rewriter, loc, i32Ty, hw0);
            Value newQ =
                sim::QueueSetOp::create(rewriter, loc, q, idx, elemVal);
            llhd::DriveOp::create(rewriter, loc, adaptor.getDest(), newQ, delay,
                                  Value{});
          } else {
            auto idxFor = [&](Value idx64, uint64_t size) -> Value {
              unsigned w = llvm::Log2_64_Ceil(size);
              if (w == 0)
                return cstI(rewriter.getIntegerType(0), 0);
              return arith::TruncIOp::create(rewriter, loc,
                                             rewriter.getIntegerType(w), idx64);
            };
            Value elemRef = llhd::SigArrayGetOp::create(
                rewriter, loc, adaptor.getDest(), idxFor(hw0, dims[0]));
            Value rem = inner;
            for (unsigned k = 1; k < dims.size(); ++k) {
              uint64_t stride = 1;
              for (unsigned j = k + 1; j < dims.size(); ++j)
                stride *= dims[j];
              Value fileIdx = arith::DivUIOp::create(rewriter, loc, rem,
                                                     cst64((int64_t)stride));
              Value hwIdx =
                  dimDescs[k]
                      ? fileIdx
                      : arith::SubIOp::create(
                            rewriter, loc, cst64((int64_t)dims[k] - 1), fileIdx)
                            .getResult();
              elemRef = llhd::SigArrayGetOp::create(rewriter, loc, elemRef,
                                                    idxFor(hwIdx, dims[k]));
            }
            llhd::DriveOp::create(rewriter, loc, elemRef, elemVal, delay,
                                  Value{});
          }

          Value nextInner =
              arith::AddIOp::create(rewriter, loc, inner, cst64(1));
          Value wrap =
              arith::CmpIOp::create(rewriter, loc, arith::CmpIPredicate::eq,
                                    nextInner, cst64((int64_t)wordsPerEntry));
          Value curStep = arith::SelectOp::create(
              rewriter, loc, up,
              arith::AddIOp::create(rewriter, loc, cur, cst64(1)),
              arith::SubIOp::create(rewriter, loc, cur, cst64(1)));
          Value nextCur =
              arith::SelectOp::create(rewriter, loc, wrap, curStep, cur);
          Value nextInner2 =
              arith::SelectOp::create(rewriter, loc, wrap, cst64(0), nextInner);
          continueTo(nextCur, nextInner2, newCount);
        };

    // ^commit(%cur, %inner, %val, %seenAt, %wordCount): write val to dest[cur],
    // advance address.
    rewriter.setInsertionPointToEnd(commitBlock);
    {
      Value cur = commitBlock->getArgument(0);
      Value inner = commitBlock->getArgument(1);
      Value val = commitBlock->getArgument(2);
      Value seenAt = commitBlock->getArgument(3);
      Value wordCount = commitBlock->getArgument(4);
      emitGuardedCommit(
          cur, inner, val, seenAt, wordCount,
          [&](Value /*nextCur*/, Value /*nextInner*/, Value newCount) {
            cf::BranchOp::create(rewriter, loc, epilogueBlock,
                                 ValueRange{seenAt, newCount});
          });
    }

    // ^commitRedisp(%cur, %inner, %val, %byte, %seenAt, %wordCount): a
    // delimiter ended the token; store the word, the re-dispatch the delimiter
    // byte through ^chkSlash.
    rewriter.setInsertionPointToEnd(commitRedispBlock);
    {
      Value cur = commitRedispBlock->getArgument(0);
      Value inner = commitRedispBlock->getArgument(1);
      Value val = commitRedispBlock->getArgument(2);
      Value byte = commitRedispBlock->getArgument(3);
      Value seenAt = commitRedispBlock->getArgument(4);
      Value wordCount = commitRedispBlock->getArgument(5);

      emitGuardedCommit(cur, inner, val, seenAt, wordCount,
                        [&](Value nextCur, Value nextInner, Value newCount) {
                          cf::BranchOp::create(rewriter, loc, chkSlashBlock,
                                               ValueRange{nextCur, nextInner,
                                                          byte, seenAt,
                                                          newCount});
                        });
    }

    // ^warn: file held fewer words than implied range.
    rewriter.setInsertionPointToEnd(warnBlock);
    {
      emitMsg("$readmem(\"",
              "\"): warning: fewer words in file than addresses in range\n");
      cf::BranchOp::create(rewriter, loc, fcloseBlock);
    }

    // ^tooMany: the file holds more words than the requested range; report and
    // close without the epilogue's fewer-words checks.
    rewriter.setInsertionPointToEnd(tooManyBlock);
    {
      emitMsg("$readmem(\"",
              "\"): warning: more words in file than addresses in range\n");
      cf::BranchOp::create(rewriter, loc, fcloseBlock);
    }

    // ^fcloseBlock: fclose
    rewriter.setInsertionPointToEnd(fcloseBlock);
    sim::SVFCloseOp::create(rewriter, loc, fd.getResult());
    cf::BranchOp::create(rewriter, loc, contBlock);

    // ^error: report malformed input or out-of-range address, then close.
    rewriter.setInsertionPointToEnd(errorBlock);
    {
      emitMsg("$readmem(\"",
              "\"): error: address out of range or malformed input\n");
      cf::BranchOp::create(rewriter, loc, fcloseBlock);
    }

    // ^epilogue(%seenAt, %wordCount): warn when file held fewer words than the
    // implied range and no @addr appeared; then close
    rewriter.setInsertionPointToEnd(epilogueBlock);
    {
      Value seenAt = epilogueBlock->getArgument(0);
      Value wordCount = epilogueBlock->getArgument(1);
      Value diff = arith::SelectOp::create(
          rewriter, loc, up,
          arith::SubIOp::create(rewriter, loc, finishSv, startSv),
          arith::SubIOp::create(rewriter, loc, startSv, finishSv));
      Value expectedCount =
          arith::AddIOp::create(rewriter, loc, diff, cst64(1));
      if (wordsPerEntry > 1)
        expectedCount = arith::MulIOp::create(rewriter, loc, expectedCount,
                                              cst64((int64_t)wordsPerEntry));
      Value noAt = arith::XOrIOp::create(rewriter, loc, seenAt, cstI(i1Ty, 1));
      Value cntMismatch = arith::CmpIOp::create(
          rewriter, loc, arith::CmpIPredicate::ne, wordCount, expectedCount);
      Value shouldWarn =
          arith::AndIOp::create(rewriter, loc, noAt, cntMismatch);
      cf::CondBranchOp::create(rewriter, loc, shouldWarn, warnBlock,
                               ValueRange{}, fcloseBlock, ValueRange{});
    }
