...
class ModOpIterator
  ...
  SCCNode sccOp;
  HWModuleOp module;
  Region::OpIterator modOpsIterator, modOpsEnd;

public:
  explicit ModOpIterator(SCCNode op, bool end = false)
      : sccOp(op), module(cast<HWModuleOp>(sccOp.op)),
        modOpsIterator(module.getOps<seq::FirRegOp>().begin()),
        modOpsEnd(module.getOps<seq::FirRegOp>().end()) {
    if (end)
      modOpsIterator = modOpsEnd;
  }
