...
struct SimEmitValueOpLowering
  ...
  matchAndRewrite(arc::SimEmitValueOp op, OpAdaptor adaptor,
    ...
    SmallVector<std::string> printfFormatStr;
    ...
    if (valueType.getWidth() < intType.getWidth()) {
      ...
      printfFormatStr.push_back(llvm::formatv("%0{0}{1}", width, intFormatter));
      ...
      for (int i = 0; remainingBits > 0; ++i) {
        int width = llvm::divideCeil(
            std::min(remainingBits, static_cast<int>(intType.getWidth())), 4);
        ...
        printfFormatStr.push_back(
            llvm::formatv("%0{0}{1}", width, intFormatter));
    ...
    // Reverse the format string. The arguments itself are reversed later.
    std::reverse(printfFormatStr.begin(), printfFormatStr.end());
    ...
    if (!(formatStrGlobal =
      ...
      formatStr.append(" = ");
      formatStr.append(llvm::join(printfFormatStr, ""));
      formatStr.append("\n");
