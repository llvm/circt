...
bool HandshakeBuilder::visitHandshake(MemoryOp op) {
  ...
  for (size_t i = 0; i < numLoads; ++i) {
    ...
    auto memClock = rewriter.create<SubfieldOp>(insertLoc, memBundle, memType.getElementIndex("clk"));
    rewriter.create<ConnectOp>(insertLoc, memClock, clock);

    // Get the load address out of the bundle.
    auto memAddr = rewriter.create<SubfieldOp>(insertLoc, memBundle, memType.getElementIndex("addr"));
    ...
    auto memData = rewriter.create<SubfieldOp>(insertLoc, memBundle, memType.getElementIndex("data"));
    ...
    auto memEnable = rewriter.create<SubfieldOp>(insertLoc, memBundle, memType.getElementIndex("en"));
  ...
  for (size_t i = 0; i < numStores; ++i) {
    ...
    auto memMask = rewriter.create<SubfieldOp>(insertLoc, memBundle, memType.getElementIndex("mask"));
