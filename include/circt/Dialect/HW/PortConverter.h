...
/// Controls whether a newly created port inherits the original port's
/// attributes.
enum class PortAttrPolicy { Drop, Preserve };
...
public:
  ...
  /// and is recorded internally. `attrPolicy` controls whether the new port
  /// inherits the original port's attributes. Any changes to 'newPort' after
  /// calling this will not be reflected in the modules new port list. Will also
  /// add the new input to the block arguments of the body of the module.
