...
struct AttributeValue : EvaluatorValue {
  ...
  AttributeValue(Attribute attr, Location loc)
    : EvaluatorValue(attr.getContext(), Kind::Attr, loc), attr(attr), type(cast<TypedAttr>(attr).getType()) {
    markFullyEvaluated();
  }

  // Constructors for partially evaluated AttributeValue.
  AttributeValue(Type type) : AttributeValue(mlir::UnknownLoc::get(type.getContext())) {}
  AttributeValue(Type type, Location loc) : EvaluatorValue(type.getContext(), Kind::Attr, loc), type(type) {}
