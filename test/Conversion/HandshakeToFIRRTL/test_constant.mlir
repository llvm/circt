...
handshake.func @test_constant(%arg0: none, ...) -> (index, none) {
  ...
  // CHECK: %0:3 = firrtl.instance @handshake_lazy_fork_1ins_2outs_ctrl {name = "", portNames = ["arg0", "arg1", "arg2"]} : !firrtl.bundle<valid: flip<uint<1>>, ready: uint<1>>, !firrtl.bundle<valid: uint<1>, ready: flip<uint<1>>>, !firrtl.bundle<valid: uint<1>, ready: flip<uint<1>>>
  %0:2 = "handshake.lazy_fork"(%arg0) {control = true} : (none) -> (none, none)
  
  // CHECK: %1:2 = firrtl.instance @handshake_constant_1ins_1outs {name = "", portNames = ["arg0", "arg1"]} : !firrtl.bundle<valid: flip<uint<1>>, ready: uint<1>>, !firrtl.bundle<valid: uint<1>, ready: flip<uint<1>>, data: uint<64>>
