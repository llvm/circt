...
// CHECK:   firrtl.strictconnect %[[IN_READY:.+]], %[[VAL_5:.+]] : !firrtl.uint<1>
...
// CHECK:   firrtl.strictconnect %validReg0, %[[VAL_6:.+]] : !firrtl.uint<1>
...
// CHECK:   firrtl.strictconnect %ctrlValidWire0, %[[SUCC_VALID0]]

// pred_ready = !readyReg
// CHECK:   %[[NOT_READY0:.+]] = firrtl.not %readyReg
// CHECK:   firrtl.strictconnect %readyWire0, %[[NOT_READY0]]
...
// CHECK:   firrtl.strictconnect %ctrlReadyWire0, %[[VAL_8:.+]] : !firrtl.uint<1>

// CHECK:   %[[VAL_9:.+]] = firrtl.mux(%[[VAL_8:.+]], %ctrlValidWire0, %validReg1) : (!firrtl.uint<1>, !firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK:   firrtl.strictconnect %validReg1, %[[VAL_9:.+]] : !firrtl.uint<1>
...
// CHECK:   firrtl.strictconnect %ctrlValidWire1, %[[SUCC_VALID1]]

// pred_ready = !readyReg
// CHECK:   %[[NOT_READY1:.+]] = firrtl.not %[[READY_REG1]]
// CHECK:   firrtl.strictconnect %readyWire1, %[[NOT_READY1]]
...
// CHECK:   firrtl.strictconnect %ctrlReadyWire1, %[[VAL_11:.+]] : !firrtl.uint<1>

// CHECK:   %[[VAL_12:.+]] = firrtl.mux(%[[VAL_11:.+]], %ctrlValidWire1, %validReg2) : (!firrtl.uint<1>, !firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK:   firrtl.strictconnect %validReg2, %[[VAL_12:.+]] : !firrtl.uint<1>

// Connet to output ports.
// CHECK:   firrtl.strictconnect %[[OUT_VALID:.+]], %ctrlValidWire2 : !firrtl.uint<1>
// CHECK:   firrtl.strictconnect %ctrlReadyWire2, %[[OUT_READY:.+]] : !firrtl.uint<1>
// CHECK: }
...
// CHECK:   firrtl.strictconnect %[[VAL_66]], %[[VAL_60]] : !firrtl.bundle<valid: uint<1>, ready flip: uint<1>>
// CHECK:   firrtl.strictconnect %[[VAL_68]], %[[VAL_64]] : !firrtl.clock
// CHECK:   firrtl.strictconnect %[[VAL_69]], %[[VAL_65]] : !firrtl.uint<1>
// CHECK:   firrtl.strictconnect %[[VAL_62]], %[[VAL_67]] : !firrtl.bundle<valid: uint<1>, ready flip: uint<1>>
// CHECK:   firrtl.strictconnect %[[VAL_63]], %[[VAL_61]] : !firrtl.bundle<valid: uint<1>, ready flip: uint<1>>
// CHECK: }
...
// CHECK:   firrtl.strictconnect %ctrlDataRegWire{{.*}}, %[[VAL_13:.+]] : firrtl.uint<64>


// CHECK:   firrtl.strictconnect %[[OUT_DATA:.+]], %ctrlDataWire1 : firrtl.uint<64>
// CHECK: }
...
// CHECK:   firrtl.strictconnect %[[VAL_70]], %[[VAL_64]] : firrtl.bundle<valid: uint<1>, ready flip: uint<1>, data: uint<64>>, !firrtl.bundle<valid: uint<1>, ready flip: uint<1>
// CHECK:   firrtl.strictconnect %[[VAL_72]], %[[VAL_68]] : firrtl.clock
// CHECK:   firrtl.strictconnect %[[VAL_73]], %[[VAL_69]] : firrtl.uint<1>
// CHECK:   firrtl.strictconnect %[[VAL_66]], %[[VAL_71]] : firrtl.bundle<valid: uint<1>, ready flip: uint<1>, data: uint<64>>, !firrtl.bundle<valid: uint<1>, ready flip: uint<1>
// CHECK:   firrtl.strictconnect %[[VAL_67]], %[[VAL_65]] : firrtl.bundle<valid: uint<1>, ready flip: uint<1>>, !firrtl.bundle<valid: uint<1>
// CHECK: }
