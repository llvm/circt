...
// CHECK:   %17 = firrtl.and %16, %0 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK:   firrtl.connect %4, %17 : !firrtl.flip<uint<1>>, !firrtl.uint<1>
// CHECK:   %18 = firrtl.and %16, %12 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK:   firrtl.connect %1, %18 : !firrtl.flip<uint<1>>, !firrtl.uint<1>
// CHECK: }
...
handshake.func @test_conditional_branch(%arg0: i1, %arg1: index, %arg2: none, ...) -> (index, index, none) {
  ...
  // CHECK: %0 = firrtl.instance @handshake_conditional_branch_2ins_2outs {name = ""} : !firrtl.bundle<arg0: bundle<valid: flip<uint<1>>, ready: uint<1>, data: flip<uint<1>>>, arg1: bundle<valid: flip<uint<1>>, ready: uint<1>, data: flip<uint<64>>>, arg2: bundle<valid: uint<1>, ready: flip<uint<1>>, data: uint<64>>, arg3: bundle<valid: uint<1>, ready: flip<uint<1>>, data: uint<64>>>
