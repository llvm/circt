...
// CHECK:   %[[MDATA_READY:.+]] = firrtl.subfield %arg1("ready") : (!firrtl.bundle<valid: uint<1>, ready flip: uint<1>>, data: uint<8>>) -> !firrtl.flip<uint<1>>
...
// CHECK:   %[[CTRL_READY:.+]] = firrtl.subfield %arg2("ready") : (!firrtl.bundle<valid: uint<1>, ready flip: uint<1>>>) -> !firrtl.flip<uint<1>>
// CHECK:   %[[ODATA_VALID:.+]] = firrtl.subfield %arg3("valid") : (!firrtl.bundle<valid flip: uint<1>>, ready: uint<1>, data: flip<uint<8>>>) -> !firrtl.flip<uint<1>>
// CHECK:   %[[ODATA_READY:.+]] = firrtl.subfield %arg3("ready") : (!firrtl.bundle<valid flip: uint<1>>, ready: uint<1>, data: flip<uint<8>>>) -> !firrtl.uint<1>
// CHECK:   %[[ODATA_DATA:.+]] = firrtl.subfield %arg3("data") : (!firrtl.bundle<valid flip: uint<1>>, ready: uint<1>, data: flip<uint<8>>>) -> !firrtl.flip<uint<8>>
// CHECK:   %[[MADDR_VALID:.+]] = firrtl.subfield %arg4("valid") : (!firrtl.bundle<valid flip: uint<1>>, ready: uint<1>, data: flip<uint<64>>>) -> !firrtl.flip<uint<1>>
...
// CHECK:   %[[MADDR_DATA:.+]] = firrtl.subfield %arg4("data") : (!firrtl.bundle<valid flip: uint<1>>, ready: uint<1>, data: flip<uint<64>>>) -> !firrtl.flip<uint<64>>

// CHECK:   firrtl.connect %[[MADDR_DATA:.+]], %[[IADDR_DATA:.+]] : !firrtl.flip<uint<64>>, !firrtl.uint<64>
// CHECK:   firrtl.connect %[[ODATA_DATA:.+]], %[[MDATA_DATA:.+]] : !firrtl.flip<uint<8>>, !firrtl.uint<8>

// CHECK:   %14 = firrtl.and %[[IADDR_VALID:.+]], %[[CTRL_VALID:.+]] : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK:   firrtl.connect %[[MADDR_VALID:.+]], %14 : !firrtl.flip<uint<1>>, !firrtl.uint<1>

// CHECK:   %15 = firrtl.and %14, %[[MADDR_READY:.+]] : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK:   firrtl.connect %[[IADDR_READY:.+]], %15 : !firrtl.flip<uint<1>>, !firrtl.uint<1>
// CHECK:   firrtl.connect %[[CTRL_READY:.+]], %15 : !firrtl.flip<uint<1>>, !firrtl.uint<1>

// CHECK:   firrtl.connect %[[ODATA_VALID:.+]], %[[MDATA_VALID:.+]] : !firrtl.flip<uint<1>>, !firrtl.uint<1>
// CHECK:   firrtl.connect %[[MDATA_READY:.+]], %[[ODATA_READY:.+]] : !firrtl.flip<uint<1>>, !firrtl.uint<1>
// CHECK: }
