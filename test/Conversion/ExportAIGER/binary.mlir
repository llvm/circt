...
hw.module @mixed_logic(in %a: i1, in %b: i1, in %c: i1, out x: i1, out y: i1, in %clk: !seq.clock) {
  %comb_logic = synth.aig.and_inv %a, %b : i1
  %next_x = synth.aig.and_inv %c, %comb_logic : i1
  %next_y = synth.aig.and_inv %reg_x, %reg_y : i1
  %reg_x = seq.compreg %next_x, %clk : i1
  %reg_y = seq.compreg %next_y, %clk : i1
  %final_out = synth.aig.and_inv %next_x, %next_y : i1
...
// CHECK-NEXT:    %[[AND_INV_1:.+]] = synth.aig.and_inv %b, %a : i1
// CHECK-NEXT:    %[[AND_INV_2]] = synth.aig.and_inv %[[AND_INV_1]], %c : i1
// CHECK-NEXT:    %[[AND_INV_3:.+]] = synth.aig.and_inv %reg_y, %reg_x : i1
// CHECK-NEXT:    %[[AND_INV_4:.+]] = synth.aig.and_inv %[[AND_INV_3]], %[[AND_INV_2]] : i1
