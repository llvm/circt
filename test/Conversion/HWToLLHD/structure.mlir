...
module {
  ...
  hw.module @FeedThrough(input %in: i1, output out: i1) {
  ...
  // CHECK-SAME: (%[[IN:.+]] : !llhd.sig<i1>) ->
  // CHECK-SAME: (%[[OUT:.+]] : !llhd.sig<i1>)
  hw.module @CombOp(input %in : i1, output out: i1) {
  ...
  // CHECK-SAME: ({{.+}} : !llhd.sig<i1>)
  hw.module @sub(input %in: i1, output out: i1) {
    hw.output %in : i1
  }
...
// CHECK-LABEL: llhd.entity @SimpleInstance
  // CHECK-SAME: (%[[IN:.+]] : !llhd.sig<i1>) ->
  // CHECK-SAME: (%[[OUT:.+]] : !llhd.sig<i1>)
  hw.module @SimpleInstance(input %in: i1, output out: i1) {
  ...
  // CHECK-SAME: (%[[IN:.+]] : !llhd.sig<i1>) ->
  // CHECK-SAME: (%[[OUT:.+]] : !llhd.sig<i1>)
  hw.module @InstanceWithLocalOps(input %in: i1, output out: i1) {
    // CHECK: %[[PRB:.+]] = llhd.prb %[[IN]]
  ...
  hw.module @InstanceDirectlyDrivingMultipleOutputs(input %in: i1, output out1: i1, output out2: i1) {
  ...
  // CHECK-SAME: %[[OUT1:.+]] : !llhd.sig<i1>,
  // CHECK-SAME: %[[OUT2:.+]] : !llhd.sig<i1>)
  hw.module @InstanceIndirectlyDrivingMultipleOutputs(input %in: i1, output out1: i1, output out2: i1) {
    // CHECK: %[[RES_SIG:.+]] = llhd.sig
    // CHECK: %[[RES:.+]] = llhd.prb %[[RES_SIG]]
