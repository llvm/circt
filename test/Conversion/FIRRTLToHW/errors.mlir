...
firrtl.circuit "zero_width_mem" {
  firrtl.module @zero_width_mem(in %clock: !firrtl.clock, in %reset: !firrtl.uint<1>, in %r0en: !firrtl.uint<1>) {
    ...
    firrtl.strictconnect %1, %r0en : !firrtl.uint<1>
    ...
    firrtl.strictconnect %4, %r0en : !firrtl.uint<1>
    ...
    firrtl.strictconnect %6, %c0_ui1 : !firrtl.uint<1>
...
// connects are dead
firrtl.circuit "noConnects" {
  firrtl.module @noConnects(in %clock: !firrtl.clock, out %out: !firrtl.clock) {
    %w = firrtl.wire : !firrtl.clock
    // expected-error @+2 {{(ICE) Connect should have been lowered to strictconnect}}
    // expected-error @+1 {{'firrtl.connect' op LowerToHW couldn't handle this operation}}
    firrtl.connect %w, %clock : !firrtl.clock, !firrtl.clock
    firrtl.strictconnect %out, %w : !firrtl.clock
  }
}
