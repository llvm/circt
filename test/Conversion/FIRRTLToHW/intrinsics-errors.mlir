...
firrtl.circuit "Foo" {
  firrtl.strictmodule @Foo(in %a: !firrtl.uint<1>, in %b: !firrtl.uint<1>) {
...
firrtl.circuit "Foo" {
  firrtl.strictmodule @Foo(in %clock: !firrtl.clock, in %reset: !firrtl.reset, out %hbr: !firrtl.uint<1>) {
