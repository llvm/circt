...
firrtl.circuit "Foo" {
  firrtl.module @Foo(in %clk: !firrtl.clock, in %a: !firrtl.uint<1>, in %b: !firrtl.uint<1>) {
...
firrtl.circuit "Foo" {
  firrtl.module @Foo(in %clk: !firrtl.clock, in %a: !firrtl.uint<1>, in %b: !firrtl.uint<1>) {
    ...
    %2 = firrtl.int.ltl.clocked_delay %a, posedge %clk, 42 : (!firrtl.uint<1>, !firrtl.clock) -> !firrtl.uint<1>
