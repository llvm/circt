...
//===----------------------------------------------------------------------===//
// Test FIRRTL Probe Operation Lowering to HW
//===----------------------------------------------------------------------===//

firrtl.circuit "ProbeOps" {
  // CHECK-LABEL: hw.module @ProbeOps
  firrtl.module @ProbeOps() {
  }

  // CHECK-LABEL: hw.module @RefTypeLowering
  firrtl.module @RefTypeLowering(
      in %in_data: !firrtl.uint<32>,
      out %out_probe: !firrtl.probe<uint<32>>) {

    // Test read-only probe creation
    // CHECK: %[[PROBE:.+]] = hw.probe.send %in_data : i32
    %probe = firrtl.ref.send %in_data : !firrtl.uint<32>
    // CHECK: hw.probe.define %out_probe, %[[PROBE]] : !hw.probe<i32>
    firrtl.ref.define %out_probe, %probe : !firrtl.probe<uint<32>>
  }

  // CHECK-LABEL: hw.module @RefSendLowering
  firrtl.module @RefSendLowering(in %data: !firrtl.uint<8>) {
    // CHECK: %{{.+}} = hw.probe.send %data : i8
    %ref = firrtl.ref.send %data : !firrtl.uint<8>
  }

  // CHECK-LABEL: hw.module @RefSendForceable
  firrtl.module @RefSendForceable(in %data: !firrtl.uint<16>) {
    // CHECK: %{{.+}} = hw.probe.send forceable %data : i16
    %rwref = firrtl.ref.send %data : !firrtl.uint<16>
  }

  // CHECK-LABEL: hw.module @RefResolveLowering
  firrtl.module @RefResolveLowering(in %in_data: !firrtl.uint<32>, out %data: !firrtl.uint<32>) {
    // Create a probe from input
    %ref = firrtl.ref.send %in_data : !firrtl.uint<32>
    // CHECK: %[[PROBE:.+]] = hw.probe.send %in_data : i32
    // CHECK: %[[RESOLVED:.+]] = hw.probe.resolve %[[PROBE]] : !hw.probe<i32>
    // CHECK: hw.output %[[RESOLVED]]
    %resolved = firrtl.ref.resolve %ref : !firrtl.probe<uint<32>>
    firrtl.matchingconnect %data, %resolved : !firrtl.uint<32>
  }

  // CHECK-LABEL: hw.module @RefSubArray
  firrtl.module @RefSubArray(in %arr: !firrtl.vector<uint<32>, 4>, out %elem_ref: !firrtl.probe<uint<32>>) {
    %ref = firrtl.ref.send %arr : !firrtl.vector<uint<32>, 4>
    // CHECK: %[[PROBE:.+]] = hw.probe.send %arr : !hw.array<4xi32>
    // CHECK: %[[SUB:.+]] = hw.probe.sub %[[PROBE]][2 : i32] : !hw.probe<!hw.array<4xi32>>
    %elem = firrtl.ref.sub %ref[2] : !firrtl.probe<vector<uint<32>, 4>>
    firrtl.ref.define %elem_ref, %elem : !firrtl.probe<uint<32>>
  }

  // CHECK-LABEL: hw.module @RefSubStruct
  firrtl.module @RefSubStruct(in %s: !firrtl.bundle<a: uint<8>, b: uint<16>>, out %field_ref: !firrtl.probe<uint<16>>) {
    %ref = firrtl.ref.send %s : !firrtl.bundle<a: uint<8>, b: uint<16>>
    // CHECK: %[[PROBE:.+]] = hw.probe.send %s : !hw.struct<a: i8, b: i16>
    // CHECK: %[[SUB:.+]] = hw.probe.sub %[[PROBE]][1 : i32] : !hw.probe<!hw.struct<a: i8, b: i16>>
    %field = firrtl.ref.sub %ref[1] : !firrtl.probe<bundle<a: uint<8>, b: uint<16>>>
    firrtl.ref.define %field_ref, %field : !firrtl.probe<uint<16>>
  }

  // CHECK-LABEL: hw.module @RefSubNested
  firrtl.module @RefSubNested(in %arr: !firrtl.vector<bundle<x: uint<4>, y: uint<8>>, 2>, out %field_ref: !firrtl.probe<uint<8>>) {
    %ref = firrtl.ref.send %arr : !firrtl.vector<bundle<x: uint<4>, y: uint<8>>, 2>
    // CHECK: %[[PROBE:.+]] = hw.probe.send %arr : !hw.array<2xstruct<x: i4, y: i8>>
    // CHECK: %[[ELEM:.+]] = hw.probe.sub %[[PROBE]][0 : i32] : !hw.probe<!hw.array<2xstruct<x: i4, y: i8>>>
    %elem = firrtl.ref.sub %ref[0] : !firrtl.probe<vector<bundle<x: uint<4>, y: uint<8>>, 2>>

    // CHECK: %[[FIELD:.+]] = hw.probe.sub %[[ELEM]][1 : i32] : !hw.probe<!hw.struct<x: i4, y: i8>>
    %field = firrtl.ref.sub %elem[1] : !firrtl.probe<bundle<x: uint<4>, y: uint<8>>>
    firrtl.ref.define %field_ref, %field : !firrtl.probe<uint<8>>
  }

  // For RefCast and RefDefine, we need to skip since they need input probes
  // which FIRRTL doesn't allow. These operations are tested implicitly in other tests.

  // CHECK-LABEL: hw.module @CombinedProbeOps
  firrtl.module @CombinedProbeOps(in %data: !firrtl.uint<32>, out %result: !firrtl.uint<32>) {
    // Send a probe directly from input
    // CHECK: %[[PROBE:.+]] = hw.probe.send %data : i32
    %probe = firrtl.ref.send %data : !firrtl.uint<32>

    // Resolve the probe
    // CHECK: %[[RESOLVED:.+]] = hw.probe.resolve %[[PROBE]] : !hw.probe<i32>
    // CHECK: hw.output %[[RESOLVED]]
    %resolved = firrtl.ref.resolve %probe : !firrtl.probe<uint<32>>
    firrtl.matchingconnect %result, %resolved : !firrtl.uint<32>
  }

  // CHECK-LABEL: hw.module @ProbeWithArrayAccess
  firrtl.module @ProbeWithArrayAccess(in %arr: !firrtl.vector<uint<16>, 8>, out %elem: !firrtl.uint<16>) {
    // CHECK: %[[PROBE:.+]] = hw.probe.send %arr : !hw.array<8xi16>
    %probe = firrtl.ref.send %arr : !firrtl.vector<uint<16>, 8>

    // CHECK: %[[SUB:.+]] = hw.probe.sub %[[PROBE]][3 : i32] : !hw.probe<!hw.array<8xi16>>
    %sub = firrtl.ref.sub %probe[3] : !firrtl.probe<vector<uint<16>, 8>>

    // CHECK: %[[RESOLVED:.+]] = hw.probe.resolve %[[SUB]] : !hw.probe<i16>
    // CHECK: hw.output %[[RESOLVED]]
    %resolved = firrtl.ref.resolve %sub : !firrtl.probe<uint<16>>
    firrtl.matchingconnect %elem, %resolved : !firrtl.uint<16>
  }
}
