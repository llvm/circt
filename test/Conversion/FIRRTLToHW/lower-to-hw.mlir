...
firrtl.circuit "RegInitial" {
  ...
  // Signed register with a negative initial value. The bit pattern is preserved
  // as an unsigned preset on the lowered seq.firreg (si8 -1 -> i8 255).
  // CHECK-LABEL: hw.module @RegInitialSigned
  firrtl.module @RegInitialSigned(in %clock: !firrtl.clock,
                                  in %d: !firrtl.sint<8>, out %q: !firrtl.sint<8>) {
    // CHECK: seq.firreg %d clock %clock preset 255 : i8
    %t = firrtl.reg %clock {initial = -1 : si8} : !firrtl.clock, !firrtl.sint<8>
    firrtl.matchingconnect %t, %d : !firrtl.sint<8>
    firrtl.matchingconnect %q, %t : !firrtl.sint<8>
  }
}
