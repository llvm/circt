...
firrtl.circuit "Simple" {
  ...
  firrtl.module private @PortMadness(in %inA: !firrtl.uint<4>,
    ...
    // CHECK: %.outB.output = sv.wire sym @__PortMadness__.outB.output {chisel_name} : !hw.inout<i4>
    // CHECK: [[OUTBR:%.+]] = sv.read_inout %.outB.output
    // CHECK: [[OUTC:%.+]] = sv.wire sym @__PortMadness__.outC.output {chisel_name} : !hw.inout<i4>
    // CHECK: [[OUTCR:%.+]] = sv.read_inout %.outC.output
    // CHECK: [[OUTD:%.+]] = sv.wire sym @__PortMadness__.outD.output {chisel_name} : !hw.inout<i4>
