...
module Expressions;
  ...
  initial begin
    ...
    // CHECK: [[TMP11:%.+]] = moore.constant 0 : i24
    // CHECK: [[TMP12:%.+]] = moore.concat [[TMP10]], [[TMP11]] : (!moore.i8, !moore.i24) -> i32
    // CHECK: moore.blocking_assign %a, [[TMP12]] : i32
    a = { << { 8'b0011_0101 }};
    // CHECK: [[TMP1:%.+]] = moore.constant -11 : i6
    // CHECK: [[TMP2:%.+]] = moore.extract [[TMP1]] from 0 : i6 -> i4
    // CHECK: [[TMP3:%.+]] = moore.extract [[TMP1]] from 4 : i6 -> i2
    // CHECK: [[TMP4:%.+]] = moore.concat [[TMP2]], [[TMP3]] : (!moore.i4, !moore.i2) -> i6
    // CHECK: [[TMP5:%.+]] = moore.constant 0 : i26
    // CHECK: [[TMP6:%.+]] = moore.concat [[TMP4]], [[TMP5]] : (!moore.i6, !moore.i26) -> i32
    // CHECK: moore.blocking_assign %a, [[TMP6]] : i32
    a = { << 4 { 6'b11_0101 }};
    // CHECK: [[TMP1:%.+]] = moore.constant -11 : i6
    // CHECK: [[TMP2:%.+]] = moore.constant 0 : i26
    // CHECK: [[TMP3:%.+]] = moore.concat [[TMP1]], [[TMP2]] : (!moore.i6, !moore.i26) -> i32
    // CHECK: moore.blocking_assign %a, [[TMP3]] : i32
    a = { >> 4 { 6'b11_0101 }};
    ...
    // CHECK: [[TMP10:%.+]] = moore.constant 0 : i28
    // CHECK: [[TMP11:%.+]] = moore.concat [[TMP9]], [[TMP10]] : (!moore.i4, !moore.i28) -> i32
    ...
    // CHECK: [[TMP3:%.+]] = moore.read %c : <i32>
    // CHECK: [[TMP4:%.+]] = moore.concat [[TMP1]], [[TMP2]], [[TMP3]] : (!moore.i32, !moore.i32, !moore.i32) -> i96
    // CHECK: [[TMP5:%.+]] = moore.constant 0 : i4
    // CHECK: [[TMP6:%.+]] = moore.concat [[TMP4]], [[TMP5]] : (!moore.i96, !moore.i4) -> i100
    // CHECK: moore.blocking_assign %dd, [[TMP6]] : i100
    dd = {>>{ a, b, c }};
    ...
    // CHECK: [[TMP3:%.+]] = moore.extract [[TMP2]] from 4 : i100 -> i96
...
// CHECK-LABEL: moore.module @StreamingAssignmentSizing
module StreamingAssignmentSizing(
    input logic [7:0] narrow_source,
    input logic [11:0] wide_source,
    output wire logic [11:0] wide_target,
    output wire logic [3:0] a,
    output wire logic [3:0] b
  );
  // CHECK: [[NARROW:%.+]] = moore.read %narrow_source_0 : <l8>
  // CHECK: [[ZERO:%.+]] = moore.constant 0 : l4
  // CHECK: [[PADDED:%.+]] = moore.concat [[NARROW]], [[ZERO]] : (!moore.l8, !moore.l4) -> l12
  // CHECK: [[WIDE:%.+]] = moore.read %wide_source_1 : <l12>
  // CHECK: [[MSBS:%.+]] = moore.extract [[WIDE]] from 4 : l12 -> l8
  assign wide_target = {>>{narrow_source}};
  assign {>>{a, b}} = wide_source;
endmodule

// CHECK-LABEL: moore.module @StreamingInitializerSizing
module StreamingInitializerSizing(
    input logic [7:0] source,
    output wire logic [11:0] result
  );
  // CHECK: %target = moore.variable [[PADDED:%.+]] : <l12>
  // CHECK: %net_target = moore.net wire [[NET_PADDED:%.+]] : <l12>
  // CHECK: [[SOURCE:%.+]] = moore.read %source_0 : <l8>
  // CHECK: [[ZERO:%.+]] = moore.constant 0 : l4
  // CHECK: [[PADDED]] = moore.concat [[SOURCE]], [[ZERO]] : (!moore.l8, !moore.l4) -> l12
  // CHECK: [[NET_SOURCE:%.+]] = moore.read %source_0 : <l8>
  // CHECK: [[NET_ZERO:%.+]] = moore.constant 0 : l4
  // CHECK: [[NET_PADDED]] = moore.concat [[NET_SOURCE]], [[NET_ZERO]] : (!moore.l8, !moore.l4) -> l12
  logic [11:0] target = {>>{source}};
  // Exercise the same conversion through a net initializer.
  wire logic [11:0] net_target = {>>{source}};
  assign result = target ^ net_target;
endmodule
...
// CHECK-LABEL: moore.module @StreamingNestedAggregates
module StreamingNestedAggregates;
  typedef struct packed {
    logic [2:0] a;
    logic [4:0] b;
  } packed_inner_t;
  typedef struct packed {
    packed_inner_t nested;
  } packed_t;
  typedef struct {
    packed_t packed_fields;
    logic [3:0] unpacked_fields [0:1];
  } mixed_t;
  // CHECK: %data = moore.variable : <uarray<2 x ustruct<{packed_fields: struct<{nested: struct<{a: l3, b: l5}>}>, unpacked_fields: uarray<2 x l4>}>>>
  mixed_t data [0:1];
  // CHECK: %bits = moore.variable : <l32>
  logic [31:0] bits;
  // CHECK: [[DATA:%.+]] = moore.read %data
  // CHECK: moore.blocking_assign %bits, {{%.+}} : l32
  // CHECK: [[ELEM1:%.+]] = moore.extract_ref %data from 1
  // CHECK: [[PACKED1:%.+]] = moore.struct_extract_ref [[ELEM1]], "packed_fields"
  // CHECK: [[UNPACKED1:%.+]] = moore.struct_extract_ref [[ELEM1]], "unpacked_fields"
  // CHECK: moore.extract_ref [[UNPACKED1]] from 1
  // CHECK: moore.extract_ref [[UNPACKED1]] from 0
  // CHECK: [[ELEM0:%.+]] = moore.extract_ref %data from 0
  // CHECK: [[PACKED0:%.+]] = moore.struct_extract_ref [[ELEM0]], "packed_fields"
  // CHECK: [[UNPACKED0:%.+]] = moore.struct_extract_ref [[ELEM0]], "unpacked_fields"
  // CHECK: moore.extract_ref [[UNPACKED0]] from 1
  // CHECK: moore.extract_ref [[UNPACKED0]] from 0
  // CHECK: [[STREAM:%.+]] = moore.concat_ref {{.*}} : ({{.*}}) -> <l32>
  // CHECK: [[RHS:%.+]] = moore.read %bits : <l32>
  // CHECK: moore.blocking_assign [[STREAM]], [[RHS]] : l32
  initial begin
    bits = {<<7{data}};
    {<<5{data}} = bits;
  end
...
// CHECK-LABEL: moore.module @StreamingAggregateRanges
module StreamingAggregateRanges(input logic [15:0] bits);
  logic [3:0] ascending [0:1];
  logic [3:0] descending [1:0];
  // CHECK: moore.concat_ref {{.*}} : ({{.*}}) -> <l16>
  // CHECK: [[BITS:%.+]] = moore.read %bits_0 : <l16>
  // CHECK: moore.nonblocking_assign {{%.+}}, [[BITS]] : l16
  always_comb {<<3{ascending, descending}} <= bits;
endmodule

// CHECK-LABEL: moore.module @StreamingDifferentWidthDomainsUnion
module StreamingDifferentWidthDomainsUnion;
  union {
    logic [7:0] first;
    bit [3:0] middle_two_state;
    logic [15:0] wider;
  } data;
  logic [7:0] bits;
  initial begin
    // CHECK: [[UNION:%.+]] = moore.read %data : <uunion<{first: l8, middle_two_state: i4, wider: l16}>>
    // CHECK: [[FIRST:%.+]] = moore.union_extract [[UNION]], "first" : uunion<{first: l8, middle_two_state: i4, wider: l16}> -> l8
    // CHECK: moore.blocking_assign %bits, [[FIRST]] : l8
    bits = {>>{data}};
    // CHECK: [[FIRST_REF:%.+]] = moore.union_extract_ref %data, "first" : <uunion<{first: l8, middle_two_state: i4, wider: l16}>> -> <l8>
    // CHECK: [[LOW:%.+]] = moore.extract_ref [[FIRST_REF]] from 0 : <l8> -> <l4>
    // CHECK: [[HIGH:%.+]] = moore.extract_ref [[FIRST_REF]] from 4 : <l8> -> <l4>
    // CHECK: [[STREAM:%.+]] = moore.concat_ref [[LOW]], [[HIGH]] : (!moore.ref<l4>, !moore.ref<l4>) -> <l8>
    // CHECK: [[RHS:%.+]] = moore.read %bits : <l8>
    // CHECK: moore.blocking_assign [[STREAM]], [[RHS]] : l8
    {<<4{data}} = bits;
  end
endmodule

// CHECK-LABEL: moore.module @StreamingMixedDomainPackedUnion
module StreamingMixedDomainPackedUnion;
  typedef struct packed {
    bit [3:0] two_state;
  ...
  union packed {
    mixed_t mixed_first;
    logic [7:0] full_four_state;
  } data;
  logic [7:0] bits;
  initial begin
    // CHECK: [[LOW:%.+]] = moore.extract_ref %data from 0 : <union<{mixed_first: struct<{two_state: i4, four_state: l4}>, full_four_state: l8}>> -> <l4>
    // CHECK: [[HIGH:%.+]] = moore.extract_ref %data from 4 : <union<{mixed_first: struct<{two_state: i4, four_state: l4}>, full_four_state: l8}>> -> <l4>
    // CHECK: [[STREAM:%.+]] = moore.concat_ref [[LOW]], [[HIGH]] : (!moore.ref<l4>, !moore.ref<l4>) -> <l8>
    // CHECK: [[RHS:%.+]] = moore.read %bits : <l8>
