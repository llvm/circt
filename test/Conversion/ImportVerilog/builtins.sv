...
// CHECK-LABEL: func.func private @TimeFormatBuiltins(
// CHECK-SAME: [[UNITVAL:%[^ ,]+]]: !moore.i32
function void TimeFormatBuiltins(int unitVal);
  // CHECK: moore.builtin.timeformat
  $timeformat();

  // CHECK: [[C9:%.+]] = moore.constant 9 : i32
  // CHECK-NEXT: [[U:%.+]] = moore.neg [[C9]] : i32
  // CHECK-NEXT: moore.builtin.timeformat unit [[U]]
  $timeformat(-9);

  // CHECK: [[C9:%.+]] = moore.constant 9 : i32
  // CHECK-NEXT: [[U:%.+]] = moore.neg [[C9]] : i32
  // CHECK-NEXT: [[P:%.+]] = moore.constant 5 : i32
  // CHECK-NEXT: [[SI:%.+]] = moore.constant_string " ns" : i24
  // CHECK-NEXT: [[S:%.+]] = moore.int_to_string [[SI]] : i24
  // CHECK-NEXT: [[W:%.+]] = moore.constant 10 : i32
  // CHECK-NEXT: moore.builtin.timeformat unit [[U]] precision [[P]] suffix [[S]] min_width [[W]]
  ...
  // CHECK-NEXT: [[S:%.+]] = moore.int_to_string [[SI]] : i24
  // CHECK-NEXT: [[W:%.+]] = moore.constant 10 : i32
  // CHECK-NEXT: moore.builtin.timeformat unit [[UNITVAL]] precision [[P]] suffix [[S]] min_width [[W]]
  $timeformat(unitVal, 5, " ns", 10);
endfunction

// CHECK-LABEL: func.func private @TimeFormatAllRuntime(
// CHECK-SAME: [[U:%[^ ,]+]]: !moore.i32
// CHECK-SAME: [[P:%[^ ,]+]]: !moore.i32
// CHECK-SAME: [[S:%[^ ,]+]]: !moore.string
// CHECK-SAME: [[W:%[^ ,]+]]: !moore.i32
function void TimeFormatAllRuntime(int u, int p, string suf, int w);
  // CHECK: moore.builtin.timeformat unit [[U]] precision [[P]] suffix [[S]] min_width [[W]]
  $timeformat(u, p, suf, w);
endfunction
