...
module Foo;
  ...
  SubA subA();
  // CHECK: [[RD_SA_SB_Y:%.+]] = moore.read %subA.subB.y : !moore.ref<i32> -> i32
  int s = subA.subB.y;
  // CHECK: [[RD_R:%.+]] = moore.read %r : !moore.ref<i32> -> i32
  // CHECK: moore.assign %subA.subB.x, [[RD_R]] : !moore.ref<i32>, i32
...
module SubA;
  ...
  SubB subB();
  // CHECK: [[RD_SB_Y:%.+]] = moore.read %subB.y : !moore.ref<i32> -> i32
  // CHECK: moore.assign %a, [[RD_SB_Y]] : !moore.ref<i32>, i32
  assign a = subB.y;
...
module SubB;
  int x, y, z;
  // CHECK: [[RD_FOO_R:%.+]] = moore.read %Foo.r : !moore.ref<i32> -> i32
  // CHECK: moore.assign %y, [[RD_FOO_R]] : !moore.ref<i32>, i32
  // CHECK: [[RD_SA_A:%.+]] = moore.read %subA.a : !moore.ref<i32> -> i32
  // CHECK: moore.assign %z, [[RD_SA_A]] : !moore.ref<i32>, i32
  assign y = Foo.r;
  assign z = Foo.subA.a;
...
module Bar(input a, b,
  ...
  SubC subC2(a, b, c);
  // CHECK: [[RD_SC1_SD_Z:%.+]] = moore.read %subC1.subD.z : !moore.ref<i32> -> i32
...
module SubD;
  int z;
  int w;
  // CHECK: [[RD_Z:%.+]] = moore.read %z : !moore.ref<i32> -> i32
  // CHECK: moore.assign %w, [[RD_Z]] : !moore.ref<i32>, i32
...
// CHECK: moore.nonblocking_assign [[INSTRES]], [[C1]] : !moore.ref<l1>, l1
// CHECK: moore.return
module HasInitial;
   SubE subE1();
   initial
...
// CHECK: [[READ:%.+]] = moore.read [[INSTRES]] : !moore.ref<l8> -> l8
// CHECK: [[C1:%.+]] = moore.constant 1 : l8
// CHECK: [[ADD:%.+]] = moore.add [[READ]], [[C1]] : l8
// CHECK: moore.assign %a, [[ADD]] : !moore.ref<l8>, l8
