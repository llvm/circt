...
module MonitorBasic;
  ...
  // CHECK:   moore.blocking_assign [[ACTIVE_REF]], [[ID]] : !moore.ref<i32>, i32
  // CHECK:   moore.return
  // CHECK: }
  initial begin
    $monitor("a=%d", a);
  ...
  // CHECK:   [[A_VAL:%.+]] = moore.read [[A]] : !moore.ref<i32> -> i32
...
module MonitorFormats;
  ...
  // CHECK: moore.procedure always_comb {
  // CHECK:   [[X_VAL_0:%.+]] = moore.read [[X]] : !moore.ref<i32> -> i32
  ...
  // CHECK: moore.procedure always_comb {
  // CHECK:   [[X_VAL_1:%.+]] = moore.read [[X]] : !moore.ref<i32> -> i32
  ...
  // CHECK: moore.procedure always_comb {
  // CHECK:   [[X_VAL_2:%.+]] = moore.read [[X]] : !moore.ref<i32> -> i32
  ...
  // CHECK: moore.procedure always_comb {
  // CHECK:   [[X_VAL_3:%.+]] = moore.read [[X]] : !moore.ref<i32> -> i32
...
module MonitorControl;
  ...
  // CHECK: moore.blocking_assign [[ENABLED_REF_OFF]], [[FALSE]] : !moore.ref<i1>, i1
  // CHECK: [[ENABLED_REF_ON:%.+]] = moore.get_global_variable @__monitor_enabled : <i1>
  // CHECK: [[TRUE:%.+]] = moore.constant 1 : i1
  // CHECK: moore.blocking_assign [[ENABLED_REF_ON]], [[TRUE]] : !moore.ref<i1>, i1
  // CHECK: moore.return
  // CHECK: }
  initial begin
    $monitor("e=%d", e);
...
module MultipleMonitors;
  ...
  // CHECK:   [[F_VAL:%.+]] = moore.read [[F]] : !moore.ref<i32> -> i32
  ...
  // CHECK:   [[G_VAL:%.+]] = moore.read [[G]] : !moore.ref<i32> -> i32
