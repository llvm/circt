...
hw.module.extern @E(input %a: i1, input %b: i1, input %c: i1)
hw.module.extern @Array(input %a: !hw.array<2xi4>)

hw.module @TESTSIMPLE(input %a: i4, input %b: i4, input %c: i2, input %cond: i1,
                        input %array2d: !hw.array<12 x array<10xi4>>,
...
hw.module @i0Module(input %arg1: i0) {}
...
//hw.module @issue316(input %inp_0: i0) {
//  hw.output
//}
...
hw.module @signs(input %in1: i4, input %in2: i4, input %in3: i4, input %in4: i4)  {
...
// CHECK-NEXT:     memory_r_en_pipe[1'h0] = 1'h0;
// CHECK-NEXT: endmodule
hw.module @ArrayLHS(input %clock: i1) {
...
hw.module @noTemporaryIfReadInOutIsAfterUse(input %clock: i1, input %x: i1) {
...
hw.module @NoExtraTemporaryWireForAssign(input %a: i2, input %b: i4) {
...
hw.module @parameterizedTypes<param: i32 = 1, wire: i32 = 2>
  ...
  // CHECK: input [$clog2($unsigned(param)) - 1:0]{{ *}}c
   input %c: !hw.int<#hw.param.expr.clog2<#hw.param.decl.ref<"param">>>) {
...
// CHECK-EMPTY:
// CHECK-NEXT:  endmodule
hw.module @Issue4485(input %in: i4) {
