...
hw.module @namehint_variadic(input %a: i3, output b: i3) {
...
hw.module @outOfOrderInoutOperations(input %a: i4, output c: i4) {
  // CHECK: %wire = sv.wire
...
hw.module @twoState_variadic(input %a: i1, input %b: i1, input %c: i1, output d:i1){
...
hw.module @carryOverWireAttrs(input %a: i1, output b: i1){
...
module {
  ...
  hw.module @SpillTemporaryInProceduralRegion(input %a: i4, input %b: i4, input %fd: i32) {
    // CHECK-NEXT: %r = sv.reg
...
module attributes {circt.loweringOptions = "disallowLocalVariables"} {
  // CHECK: @test_hoist
  hw.module @test_hoist(input %a: i3) {
  ...
  hw.module @SpillTemporary(input %a: i4, input %b: i4, output c: i1) {
  ...
  hw.module @SpillTemporaryWireForMultipleUseExpression(input %a: i4, input %b: i4, output c: i4, output d: i4) {
    // CHECK-NEXT: %[[VAL:.+]] = comb.add %a, %b
...
module attributes {circt.loweringOptions =
  ...
  hw.module @namehints(input %a: i8, output b: i8) {
...
module attributes {circt.loweringOptions =
                  "disallowMuxInlining"} {
  // CHECK-LABEL: mux
  hw.module @mux(input %c: i1, input %b: i8, input %a: i8, output d: i8, output e: i8) {
...
module attributes {circt.loweringOptions =
  ...
  hw.module @combine(input %c: i1, input %b: i8, input %a: i8, output d: i8) {
...
module attributes {circt.loweringOptions = "maximumNumberOfTermsPerExpression=2"} {
  // CHECK-NOT: sv.wire
  hw.module @Foo(input %in_0: i4, input %in_1: i4, input %in_2: i4, input %in_3: i4, output out: !hw.array<4xi4>) {
...
// CHECK:   hw.module @packed_struct_assignment(input %[[VAL_0:.*]] : i32, output out : !hw.struct<a: i32>, output out2 : !hw.struct<a: i32>, output out3 : !hw.struct<a: i32>) {
// CHECK:           %[[VAL_1:.*]] = sv.wire
...
module attributes { circt.loweringOptions = "disallowPackedStructAssignments"} {
  hw.module @packed_struct_assignment(input %in : i32, output out: !T, output out2: !T, output out3: !T)  {
...
hw.module @Issue5613(input %a: i1, input %b: i1) {
...
hw.module @Issue5605(input %a: i1, input %b: i1, input %clock: i1, input %reset: i1) {
