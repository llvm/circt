...
// RUN: circt-opt --externalize-registers --lower-to-bmc="top-module=two_clk_design bound=10 sync-clocks=true" %s | FileCheck %s --check-prefix=CHECKSYNC
...
// CHECKSYNC: func.func @two_clk_design() {
// CHECKSYNC:   [[BMC:%.+]] = verif.bmc bound 20 num_regs 2 initial_values [unit, unit] init {
// CHECKSYNC:   } loop {
// CHECKSYNC:   ^bb0([[CLK0:%.+]]: !seq.clock, [[CLK1:%.+]]: !seq.clock):
// CHECKSYNC:     [[FROM0:%.+]] = seq.from_clock [[CLK0]]
// CHECKSYNC:     [[TRUE0:%.+]] = hw.constant true
// CHECKSYNC:     [[NCLK0:%.+]] = comb.xor [[FROM0]], [[TRUE0]]
// CHECKSYNC:     [[NEW_CLK0:%.+]] = seq.to_clock [[NCLK0]]
// CHECKSYNC:     [[FROM1:%.+]] = seq.from_clock [[CLK1]]
// CHECKSYNC:     [[TRUE1:%.+]] = hw.constant true
// CHECKSYNC:     [[NCLK1:%.+]] = comb.xor [[FROM1]], [[TRUE1]]
// CHECKSYNC:     [[NEW_CLK1:%.+]] = seq.to_clock [[NCLK1]]
// CHECKSYNC:     verif.yield [[NEW_CLK0]], [[NEW_CLK1]]
// CHECKSYNC:   } circuit {
