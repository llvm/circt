...
rtg.test @immediates() {
  // CHECK-NEXT: [[V0:%.+]] = rtg.constant #rtg.isa.immediate<2, 3>
...
rtg.test @arrays() {
  // CHECK-NEXT: [[V0:%.+]] = rtg.array_create [] : !rtg.array<0 x index>
  // CHECK-NEXT: func.call @dummy7([[V0]]) : (!rtg.array<0 x index>) -> ()
  %0 = rtg.array_create [] : !rtg.array<0 x index>
  func.call @dummy7(%0) : (!rtg.array<0 x index>) -> ()
