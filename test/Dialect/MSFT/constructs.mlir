...
// LOWER-LABEL: hw.module @SAExample1(input %clk : !seq.clock, output out : !hw.array<2xarray<3xi8>>) {
...
hw.module @SAExample1 (input %clk: !seq.clock, output out: !hw.array<2 x array<3 x i8>>) {
...
hw.module @PE(input %clk: !seq.clock, input %a: i8, input %b: i8, output sum: i8) {
  %sum = comb.add %a, %b: i8
  %sumDelay1 = seq.compreg %sum, %clk : i8
  hw.output %sumDelay1 : i8
}

// CHECK-LABEL: hw.module @ChannelExample(input %clk : !seq.clock, input %a : i8, output out : i8) {
// CHECK:         [[REG0:%.+]] = msft.constructs.channel %a %clk "chEx"(2) : i8
// CHECK:         hw.output [[REG0]] : i8
// LOWER-LABEL: hw.module @ChannelExample(input %clk : !seq.clock, input %a : i8, output out : i8) {
// LOWER:         %chEx_0 = seq.compreg sym @chEx_0 %a, %clk : i8
// LOWER:         %chEx_1 = seq.compreg sym @chEx_1 %chEx_0, %clk : i8
// LOWER:         hw.output %chEx_1 : i8
hw.module @ChannelExample (input %clk: !seq.clock, input %a : i8, output out: i8) {
  %out = msft.constructs.channel %a %clk "chEx" (2) : i8
  hw.output %out : i8
}

// CHECK-LABEL: hw.module @foo(input %in0 : i32, input %in1 : i32, input %in2 : i32, input %clk : !seq.clock, output out : i32) {
...
hw.module @foo (input %in0 : i32, input %in1 : i32, input %in2 : i32, input %clk: !seq.clock, output out: i32) {
