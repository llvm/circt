...
// CHECK-LABEL: hw.module @test1(%arg0: i3, %arg1: i1, %arg2:
// !hw.array<1000xi8>) -> (result: i50) {
hw.module @test1(% arg0
                 : i3, % arg1
                 : i1, % arg2
                 : !hw.array<1000xi8>)
    ->(result
      ...
      % a = hw.constant 42 : i12 % b = comb.add % a,
      % a : i12 % c = comb.mul % a,
      %
          b : i12
      ...
      i1
          ...
          % parity1 =
              ...
              // CHECK-NEXT:    [[RES8:%[0-9]+]] = comb.concat [[RES4]],
              // [[RES0]], [[RES1]], [[RES2]], [[RES2]] : i12, i12, i12, i7, i7
              % result = comb.concat % conc1,
      % b,
      % c,
      % d,
      % d : i12,
      ...
      i7
          ...
          // CHECK-NEXT: [[RES9:%[0-9]+]] = comb.extract [[RES8]] from 4 : (i50)
          // -> i19
          % small1 = comb.extract %
                     result from 4 : (i50)->i19

                     // CHECK-NEXT: [[RES10:%[0-9]+]] = comb.extract [[RES8]]
                     // from 31 : (i50) -> i19
                     %
                     small2 = comb.extract %
                              result from 31 : (i50)->i19
      ...
      %
          ...
          % before1 =
              sv.wire name "after1" : !hw.inout<i4>
              ...
              % read_before1 =
                  ...
                  // CHECK-NEXT: %after2_conflict_0 = sv.wire name
                  // "after2_conflict" : !hw.inout<i4>
                  % before2_0 =
                      sv.wire name "after2_conflict" : !hw.inout<i4> %
                      before2_1 =
                          sv.wire name "after2_conflict" : !hw.inout<i4>

                          // CHECK-NEXT: %after3 = sv.wire {someAttr = "foo"} :
                          // !hw.inout<i4>
                          % before3 =
                              sv.wire name "after3" {someAttr = "foo"} :
                                  !hw.inout<i4>
                              ...
                              % w2 =
                                  ...
                                  % before4 =
                                      ...
                                      // CHECK-NEXT: %after5_conflict = hw.wire
                                      // [[RES2]] : i7
                                      // CHECK-NEXT: %after5_conflict_1 =
                                      // hw.wire [[RES2]] name "after5_conflict"
                                      // : i7
                                      % before5_0 =
                                          hw.wire %
                                          d name "after5_conflict" :
                                              i7 % before5_1 =
                                                  hw.wire %
                                                  d name "after5_conflict" : i7

                                                  // CHECK-NEXT: %after6 =
                                                  // hw.wire [[RES2]] {someAttr
                                                  // = "foo"} : i7
                                                  % before6 =
                                                      hw.wire %
                                                      d name "after6" {
      ...
      %
          ...
          // CHECK-NEXT: [[STR:%[0-9]+]] = hw.struct_create ({{.*}}, {{.*}}) :
          // !hw.struct<foo: i19, bar: i7>
          % s0 = hw.struct_create(% small1, % mux) : !hw.struct < foo : i19,
      bar : i7 >

          // CHECK-NEXT: %foo = hw.struct_extract [[STR]]["foo"] :
          // !hw.struct<foo: i19, bar: i7>
          % foo = hw.struct_extract % s0["foo"] : !hw.struct < foo : i19,
      bar : i7 >

          // CHECK-NEXT: = hw.struct_inject [[STR]]["foo"], {{.*}} :
          // !hw.struct<foo: i19, bar: i7>
          % s1 = hw.struct_inject % s0["foo"],
      % foo : !hw.struct < foo : i19,
      bar : i7 >

          // CHECK-NEXT:  %foo_2, %bar = hw.struct_explode [[STR]] :
          // !hw.struct<foo: i19, bar: i7>
          % foo_2,
      % bar = hw.struct_explode % s0 : !hw.struct < foo : i19,
      bar : i7 >
          ...
          % idx = arith.constant 13 : i10
                  // CHECK-NEXT: = hw.array_slice %arg2[%c13_i10] :
                  // (!hw.array<1000xi8>) -> !hw.array<24xi8>
                  % subArray = hw.array_slice %
                               arg2[% idx] :
                                   (!hw.array<1000xi8>)->!hw.array<24xi8>
                               // CHECK-NEXT: [[ARR1:%.+]] = hw.array_create
                               // [[RES9]], [[RES10]] : i19
                               % arrCreated = hw.array_create % small1,
      %
          small2 : i19
          // CHECK-NEXT: [[ARR2:%.+]] = hw.array_create [[RES9]], [[RES10]],
          // {{.+}} : i19
          % arr2 = hw.array_create % small1,
      % small2,
      %
          add : i19
          // CHECK-NEXT: = hw.array_concat [[ARR1]], [[ARR2]] :
          // !hw.array<2xi19>, !hw.array<3xi19>
          % bigArray = hw.array_concat % arrCreated,
      % arr2 : !hw.array<2 x i19>,
      !hw.array<3 x i19>
          ...
          % A_enum = hw.enum.constant A : !hw.enum<A, B, C>
                     ...
                     % B_enum = hw.enum.constant B : !hw.enum<A, B, C>
                                // CHECK-NEXT: = hw.enum.cmp %A, %B :
                                // !hw.enum<A, B, C>, !hw.enum<A, B, C>
                                % enumcmp = hw.enum.cmp % A_enum,
      % B_enum : !hw.enum<A, B, C>,
      !hw.enum<A, B, C>

              // CHECK-NEXT: hw.aggregate_constant [false, true] : !hw.struct<a:
              // i1, b: i1>
                  hw.aggregate_constant[false, true] : !hw.struct
          < a : i1,
      b : i1 >
          // hw.enum.constant A : !hw.enum<A, B>
          // CHECK-NEXT: hw.aggregate_constant [0 : i2, 1 : i2, -2 : i2, -1 :
          // i2] : !hw.array<4xi2>
          hw.aggregate_constant [0:i2, 1:i2, -2:i2, -1:i2] :
              !hw.array<4xi2>
                  // CHECK-NEXT: hw.aggregate_constant [false] :
                  // !hw.uarray<1xi1>
                      hw.aggregate_constant[false] :
                          !hw.uarray<1xi1>
                              // CHECK-NEXT{LITERAL}: hw.aggregate_constant
                              // [[false]] : !hw.struct<a: !hw.array<1xi1>>
                                  hw.aggregate_constant[[false]] : !hw.struct
          < a : !hw.array < 1xi1 >>
          // CHECK-NEXT: hw.aggregate_constant ["A"] : !hw.struct<a: !hw.enum<A,
          // B, C>>
          hw.aggregate_constant["A"] : !hw.struct < a : !hw.enum < A,
      B,
      C >>
          // CHECK-NEXT: hw.aggregate_constant ["A"] : !hw.array<1xenum<A, B,
          // C>>
          hw.aggregate_constant["A"] :
                  !hw.array<1 x !hw.enum<A, B, C>>
...
hw.module @UnionOps(% a
    ...
    ->(x
       ...
       : i3, baz
       : i8 >){
      % x = hw.union_extract % a["bar"] : !hw.union < foo : i1,
      bar : i3 > % z = hw.union_create "bar",
      % x : !hw.union < bar : i3,
      baz : i8 > hw.output % x,
      % z : i3,
      !hw.union < bar : i3,
      baz : i8 >
    } // CHECK-LABEL: hw.module @UnionOps(%a: !hw.union<foo: i1, bar: i3>) ->
      // (x: i3, z: !hw.union<bar: i3, baz: i8>) {
// CHECK-NEXT:    [[I3REG:%.+]] = hw.union_extract %a["bar"] : !hw.union<foo:
// i1, bar: i3>
// CHECK-NEXT:    [[UREG:%.+]] = hw.union_create "bar", [[I3REG]] :
// !hw.union<bar: i3, baz: i8>
// CHECK-NEXT:    hw.output [[I3REG]], [[UREG]] : i3, !hw.union<bar: i3, baz:
// i8>

// https://github.com/llvm/circt/issues/863
// CHECK-LABEL: hw.module @signed_arrays
hw.module @signed_arrays(% arg0
                         : si8)
    ->(out
       : !hw.array<2xsi8>){
      ...
      % 0 : !hw.array<2xsi8>
          ...
          % result = sv.read_inout %
                     wireArray : !hw.inout<!hw.array<2xsi8>> hw.output %
                     result : !hw.array<2xsi8>
    }
...
hw.module @fileListTest(% arg1
                        : i32) attributes{
    output_filelist = #hw.output_filelist<"foo.f">} {}
...
module {
  ...
  hw.module @D()->() {
    hw.instance "M1" sym @memInst @FIRRTLMem()->() {
      circt.globalRef = [#hw.globalNameRef<@glbl_D_M1>]
    }
    % c0 = hw.constant 0 : i1 % 2 = hw.instance "ab" sym @SF @F(in
                                                                : % c0
                                                                : i1)
                                        ->(out
                                           : i1) {
      circt.globalRef =
          [ #hw.globalNameRef<@glbl_D_M2>, #hw.globalNameRef<@glbl_D_M3> ]
    }
  }
  hw.module @B()->() {
    hw.instance "M1" sym @memInst @FIRRTLMem()->() {
      circt.globalRef = [#hw.globalNameRef<@glbl_B_M1>]
    }
  }
  hw.module @C()->() {
    hw.instance "m" sym @inst @D()->() {
      circt.globalRef = [
        #hw.globalNameRef<@glbl_D_M1>, #hw.globalNameRef<@glbl_D_M2>,
        #hw.globalNameRef<@glbl_D_M3>
      ]
    }
  }
  hw.module @A()->() {
    hw.instance "h1" sym @inst_1 @B()->() {
      circt.globalRef = [#hw.globalNameRef<@glbl_B_M1>]
    }
    hw.instance "h2" sym @inst_0 @C()->() {
      circt.globalRef = [
        #hw.globalNameRef<@glbl_D_M1>, #hw.globalNameRef<@glbl_D_M2>,
        #hw.globalNameRef<@glbl_D_M3>
      ]
