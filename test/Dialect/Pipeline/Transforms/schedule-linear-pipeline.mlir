...
// CHECK:             %[[VAL_9:.*]] = comb.add %[[VAL_5]], %[[VAL_6]] {operator_type = @add1} : i32
// CHECK:             %[[VAL_10:.*]] = comb.add %[[VAL_6]], %[[VAL_5]] {operator_type = @add1} : i32
// CHECK:             %[[VAL_11:.*]] = pipeline.stage when %[[VAL_7]]
// CHECK:             %[[VAL_12:.*]] = pipeline.stage when %[[VAL_11]]
// CHECK:             %[[VAL_13:.*]] = comb.mul %[[VAL_5]], %[[VAL_9]] {operator_type = @mul2} : i32
// CHECK:             %[[VAL_14:.*]] = pipeline.stage when %[[VAL_12]]
// CHECK:             %[[VAL_15:.*]] = pipeline.stage when %[[VAL_14]]
// CHECK:             %[[VAL_16:.*]] = pipeline.stage when %[[VAL_15]]
// CHECK:             %[[VAL_17:.*]] = comb.add %[[VAL_13]], %[[VAL_10]] {operator_type = @add1} : i32
...
module {
  ...
  hw.module @pipeline(%arg0 : i32, %arg1 : i32, %clk : i1, %rst : i1) -> (out: i32) {
    ...
    : (i32, i32) -> (i32) {
    ^bb0(%a0 : i32, %a1: i32):
      %0 = comb.add %a0, %a1 {operator_type = @add1} : i32
      %1 = comb.mul %a0, %0 {operator_type = @mul2} : i32
      %2 = comb.add %a1, %a0 {operator_type = @add1} : i32
      %3 = comb.add %1, %2 {operator_type = @add1} : i32
