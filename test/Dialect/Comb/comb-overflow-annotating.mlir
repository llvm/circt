...
hw.module @basic_add(in %a : i4, in %b : i4, in %c : i4, out add1 : i4, out add2 : i5) {
  // CHECK-NEXT: %false = hw.constant false
  // CHECK-NEXT: comb.add %a, %b {comb.upper_bit_trunc = true} : i4
  // CHECK-NEXT: %[[AEXT:.+]] = comb.concat %false, %a : i1, i4
  // CHECK-NEXT: %[[BEXT:.+]] = comb.concat %false, %b : i1, i4
  // CHECK-NEXT: comb.add %[[AEXT]], %[[BEXT]] {comb.upper_bit_trunc = false} : i5
...
hw.module @basic_mul(in %a : i4, in %b : i4, out mul7 : i7, out mul8 : i8) {
  ...
  // CHECK-NEXT: comb.mul %[[AEXT7]], %[[BEXT7]] {comb.upper_bit_trunc = true} : i7
  // CHECK-NEXT: %[[AEXT8:.+]] = comb.concat %c0_i4, %a : i4, i4
  // CHECK-NEXT: %[[BEXT8:.+]] = comb.concat %c0_i4, %b : i4, i4
  // CHECK-NEXT: comb.mul %[[AEXT8]], %[[BEXT8]] {comb.upper_bit_trunc = false} : i8
...
hw.module @basic_fma(in %a : i4, in %b : i4, in %c : i4, in %d : i5, out fma1 : i8, out fma2 : i8) {
  ...
  // CHECK-NEXT: %[[MUL:.+]] = comb.mul %0, %1 {comb.upper_bit_trunc = false} : i8
  // Should not overflow when adding 4-bit c
  // CHECK-NEXT: comb.add %4, %2 {comb.upper_bit_trunc = false} : i8
  // Should overflow when adding 5-bit d
  // CHECK-NEXT: comb.add %4, %3 {comb.upper_bit_trunc = true} : i8
  %c0_i3 = hw.constant 0 : i3
  %c0_i4 = hw.constant 0 : i4
  %4 = comb.concat %c0_i4, %a : i4, i4
  %5 = comb.concat %c0_i4, %b : i4, i4
