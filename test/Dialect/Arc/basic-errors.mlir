...
hw.module @Foo(input %clock: !seq.clock) {
...
hw.module @Foo(input %clock: !seq.clock, input %enable: i1) {
...
hw.module @Foo(input %clock: !seq.clock, input %reset: i1) {
...
hw.module @clockDomainNumOutputs(input %clk: !seq.clock) {
...
hw.module @clockDomainNumInputs(input %clk: !seq.clock) {
...
hw.module @clockDomainInputTypes(input %clk: !seq.clock, input %arg0: i16) {
...
hw.module @clockDomainOutputTypes(input %clk: !seq.clock) {
  %0 = arc.clock_domain () clock %clk : () -> (i32) {
  ^bb0:
...
hw.module @clockDomainIsolatedFromAbove(input %clk: !seq.clock, input %arg0: i32) {
  // expected-note @+1 {{required by region isolation constraints}}
  %0 = arc.clock_domain () clock %clk : () -> (i32) {
    // expected-error @+1 {{using value defined outside the region}}
    arc.output %arg0 : i32
...
hw.module @stateOpInsideClockDomain(input %clk: !seq.clock) {
...
hw.module @memoryWritePortOpInsideClockDomain(input %clk: !seq.clock) {
  arc.clock_domain (%clk) clock %clk : (!seq.clock) -> () {
  ^bb0(%arg0: !seq.clock):
...
hw.module @memoryWritePortOpOutsideClockDomain(input %clock: !seq.clock, input %en: i1) {
  %mem = arc.memory <4 x i32, i32>
  %c0_i32 = hw.constant 0 : i32
...
hw.module @memoryWritePortOpLatZero(input %clock: !seq.clock, input %en: i1) {
  %mem = arc.memory <4 x i32, i32>
  %c0_i32 = hw.constant 0 : i32
...
hw.module @at_least_one_result(input %in0: i1, input %in1: i1, input %in2: i1, input %in3: i1) {
