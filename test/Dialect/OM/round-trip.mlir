...
om.class @NestedField2() {
  ...
  om.class.fields(
    @bar %0 : !om.class.type<@NestedField1>
  )
}
...
om.class @NestedField3() {
  ...
  om.class.fields(
    @foo %0 : !om.class.type<@NestedField2>
  )
}
...
om.class @ReferenceParameter(%arg0: !om.ref, %arg1: !om.sym_ref) {
  om.class.fields(
    // CHECK: @myref %arg0 : !om.ref,
    @myref %arg0 : !om.ref,
    // CHECK: @sym %arg1 : !om.sym_ref
    @sym %arg1 : !om.sym_ref
  )
}
...
om.class @ReferenceConstant() {
  ...
  om.class.fields(
    // CHECK: @myref %[[const1]] : !om.ref,
    @myref %0 : !om.ref,
    // CHECK: @sym %[[const2]] : !om.sym_ref
    @sym %1 : !om.sym_ref
  )
}
...
om.class @ListConstant() {
  ...
  om.class.fields(
    // CHECK: @list_i64 %[[const1]] : !om.list<i64>,
    @list_i64 %0 : !om.list<i64>,
    // CHECK: @list_i32 %[[const2]] : !om.list<i32>
    @list_i32 %1 : !om.list<i32>
  )
}
...
om.class @ListCreate() {
  ...
  om.class.fields(
    // CHECK: @list_field [[list]] : !om.list<!om.class.type<@Widget>>
    @list_field %list : !om.list<!om.class.type<@Widget>>
  )
}
...
om.class @LinkedList(%prev: !om.class.type<@LinkedList>) {
  om.class.fields(
    @prev %prev : !om.class.type<@LinkedList>
  )
}
...
om.class @BoolConstant(%b0 : i1) {
  ...
  om.class.fields(
    // CHECK: @bool %b0 : i1,
    @bool %b0 : i1,
    // CHECK: @bool2 %[[const1]] : i1,
    @bool2 %1 : i1,
    // CHECK: @bool3 %[[const2]] : i1
    @bool3 %2 : i1
  )
}
...
om.class @Map(%map: !om.map<!om.string, !om.string>) {
  om.class.fields(
    @field %map : !om.map<!om.string, !om.string>
  )
}
...
om.class @Tuple(%int: i1, %str: !om.string) {
  ...
  om.class.fields(
    // CHECK-NEXT: @tuple %[[tuple]] : tuple<i1, !om.string>,
    @tuple %tuple : tuple<i1, !om.string>,
    // CHECK-NEXT: @val %[[tuple_get]] : !om.string
    @val %val : !om.string
  )
}
...
om.class @MapConstant() {
  ...
  om.class.fields(
    // CHECK: @map_i64 %[[const1]] : !om.map<!om.string, i64>
    @map_i64 %0 : !om.map<!om.string, i64>
  )
}
...
om.class @MapCreate(%e1: tuple<!om.string, !om.class.type<@Empty>>, %e2: tuple<!om.string, !om.class.type<@Empty>>) {
  ...
  // CHECK-NEXT: om.class.fields(
  om.class.fields(
    // CHECK-NEXT: @map_field %[[map]] : !om.map<!om.string, !om.class.type<@Empty>>
    @map_field %map : !om.map<!om.string, !om.class.type<@Empty>>
  )
}
...
om.class @Path(%basepath: !om.basepath) {
  ...
  %3 = om.path_empty
  om.class.fields(
    // CHECK: @path_empty %[[v3]] : !om.path
    @path_empty %3 : !om.path
  )
}
