// RUN: circt-opt -fsm-print-state-graph %s -o %t 2>&1 | FileCheck %s

// CHECK: digraph "axi_read_target" {
// CHECK:   label="axi_read_target";
// CHECK:   [[IDLE:.*]] [shape=record,label="{IDLE}"];
// CHECK:   [[IDLE]] -> [[MID:.*]];
// CHECK:   [[IDLE]] -> [[END:.*]];
// CHECK:   [[MID]] [shape=record,label="{MID}"];
// CHECK:   [[MID]] -> [[MID]];
// CHECK:   [[MID]] -> [[END]];
// CHECK:   [[END]] [shape=record,label="{END}"];
// CHECK:   [[END]] -> [[MID]];
// CHECK:   [[END]] -> [[END]];
// CHECK:   [[END]] -> [[HOLD:.*]];
// CHECK:   [[HOLD]] [shape=record,label="{HOLD}"];
// CHECK:   [[HOLD]] -> [[MID]];
// CHECK:   [[HOLD]] -> [[END]];
// CHECK: }
