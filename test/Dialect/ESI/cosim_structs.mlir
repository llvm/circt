...
// RUN: circt-opt %s --lower-esi-ports --lower-esi-to-rtl --rtl-declare-typedefs | circt-translate --export-verilog | FileCheck --check-prefix=SV %s
...
// Test only a few, critical lines in the systemverilog output. Anything else would be too fragile.
// SV: typedef struct packed {{.+}} DataPkt;
// SV: interface IValidReady_DataPkt;
