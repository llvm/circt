...
hw.module @Reciever(input %a: !esi.channel<i1, ValidReady>) {
  %rdy = arith.constant 1 : i1
  // Recieve bits.
  %data, %valid = esi.unwrap.vr %a, %rdy : i1
}
!FooStruct = !hw.struct<a: si4, b: !hw.array<3 x ui4>>
hw.module @StructRcvr(input %a: !esi.channel<!FooStruct>) {
  %rdy = arith.constant 1 : i1
  // Recieve bits.
...
// CHECK-LABEL: hw.module @StructRcvr(input %a : !esi.channel<!hw.struct<a: si4, b: !hw.array<3xui4>>>)
...
hw.module.extern @IFaceSender(input %x: !sv.modport<@IData::@Source>)
hw.module.extern @IFaceRcvr(input %x: !sv.modport<@IData::@Sink>)
sv.interface @IData {
