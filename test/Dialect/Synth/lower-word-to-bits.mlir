...
hw.module @Basic_Comb(in %cond: i1, in %a: i2, in %b: i2, out f: i2, out h: i2, out i: i2) {
  %0 = synth.aig.and_inv %a, %b : i2
  %1 = synth.xor_inv %a, %b : i2
  %2 = comb.mux %cond, %a, %b : i2
  ...
  // CHECK-DAG: %[[AND0:.+]] = synth.aig.and_inv %[[A0]], %[[B0]] : i1
  // CHECK-DAG: %[[AND1:.+]] = synth.aig.and_inv %[[A1]], %[[B1]] : i1
  // CHECK-DAG: %[[AND:.+]] = comb.concat %[[AND1]], %[[AND0]] : i1, i1
  // CHECK-DAG: %[[XOR0:.+]] = synth.xor_inv %[[A0]], %[[B0]] : i1
  // CHECK-DAG: %[[XOR1:.+]] = synth.xor_inv %[[A1]], %[[B1]] : i1
  ...
  // CHECK-DAG: hw.output %[[AND]], %[[XOR]], %[[MUX]] : i2, i2, i2
