...
// RUN: circt-opt %s --pass-pipeline='builtin.module(hw.module(synth-aiger-runner{solver-path="cp" solver-args="<inputFile>" solver-args="<outputFile>"}))' | FileCheck %s

// CHECK-LABEL: hw.module @test(in %a : i1, in %b : i1, out out : i1)
// CHECK-NEXT: %0 = synth.aig.and_inv %b, %a : i1
// CHECK-NEXT: hw.output %0 : i1 
hw.module @test(in %a: i1, in %b: i1, out out: i1) {
  %0 = synth.aig.and_inv %b, %a : i1
  hw.output %0 : i1
}
...
// CHECK-NEXT: %[[AND_INV_0:.+]] = synth.aig.and_inv %[[B_0]], %[[A_0]] : i1
// CHECK-NEXT: %[[AND_INV_1:.+]] = synth.aig.and_inv %[[B_1]], %[[A_1]] : i1
// CHECK-NEXT: %[[AND_INV_2:.+]] = synth.aig.and_inv %[[ADD_0]], %[[A_0]] : i1
// CHECK-NEXT: %[[AND_INV_3:.+]] = synth.aig.and_inv %[[ADD_1]], %[[A_1]] : i1
// CHECK-NEXT: hw.output %[[CONCAT:.+]] : i2
hw.module @unknownOperation(in %a: i2, in %b: i2, out out: i2) {
  %0 = synth.aig.and_inv %b, %a : i2
  dbg.variable "test", %0 : i2
  %1 = comb.add %a, %b : i2
  %2 = synth.aig.and_inv %1, %a : i2
  hw.output %2 : i2
}
