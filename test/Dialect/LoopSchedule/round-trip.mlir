...
func.func @test1(%arg0: memref<?xi32>) -> i32 {
  ...
  // CHECK: loopschedule.pipeline_while
  // CHECK-SAME: II = 1
  ...
  // CHECK: } do {
    ...
    // CHECK: loopschedule.pipeline_stage start = 0 {
...
func.func @test2(%arg0: memref<?xi32>, %arg1: memref<?xi32>) {
  ...
  // CHECK-SAME: (index) -> ()
  loopschedule.pipeline_while II = 1 iter_args(%arg2 = %c0) : (index) -> () {
    %0 = arith.cmpi ult, %arg2, %c10 : index
    loopschedule.register %0 : i1
  } do {
    %0:4 = loopschedule.pipeline_stage start = 0 {
    ...
    // CHECK: loopschedule.pipeline_stage start = 1 when %0#3
    %1:3 = loopschedule.pipeline_stage start = 1 when %0#3  {
      %4 = arith.subi %0#0, %c3 : index
      loopschedule.register %0#2, %0#3, %4 : i32, i1, index
    } : i32, i1, index
    %2:4 = loopschedule.pipeline_stage start = 2 when %1#1  {
      %4 = memref.load %arg0[%1#2] : memref<?xi32>
      loopschedule.register %1#0, %1#1, %1#2, %4 : i32, i1, index, i32
    } : i32, i1, index, i32
    %3:3 = loopschedule.pipeline_stage start = 3 when %2#1  {
      %4 = arith.addi %2#0, %2#3 : i32
      loopschedule.register %2#1, %2#2, %4 : i1, index, i32
    } : i1, index, i32
    loopschedule.pipeline_stage start = 5 when %3#0  {
...
func.func @test3(%arg0: memref<?xi32>) {
  ...
  // CHECK-SAME: (index) -> ()
  loopschedule.pipeline_while II = 1 iter_args(%arg1 = %c0) : (index) -> () {
    %3 = arith.cmpi ult, %arg1, %c10 : index
    loopschedule.register %3 : i1
  } do {
    %3:5 = loopschedule.pipeline_stage start = 0 {
...
func.func @test4(%arg0: memref<?xi32>, %arg1: memref<?xi32>) {
  ...
  loopschedule.pipeline_while II = 1 iter_args(%arg2 = %c0) : (index) -> () {
    ...
    %1:2 = loopschedule.pipeline_stage start = 1 {
      %3 = memref.load %arg0[%0#1] : memref<?xi32>
      loopschedule.register %0#1, %3 : index, i32
    } : index, i32
    %2:2 = loopschedule.pipeline_stage start = 2 {
...
func.func @test5(%arg0: memref<?xi32>) {
  ...
  // CHECK-SAME: (index) -> ()
  loopschedule.pipeline_while II = 1 iter_args(%arg1 = %c2) : (index) -> () {
...
func.func @trip_count_attr() {
  ...
  // CHECK: loopschedule.pipeline_while II = 1 trip_count = 3
  loopschedule.pipeline_while II = 1 trip_count = 3 iter_args(%arg0 = %false) : (i1) -> () {
