...
firrtl.module @declarations(in %clock : !firrtl.clock, in %u8 : !firrtl.uint<8>, in %reset : !firrtl.asyncreset) attributes {annotations = [{class = "firrtl.passes.InlineAnnotation"}]} {
  ...
  // CHECK: %myinst_memoryport = firrtl.memoryport Infer %myinst_cmem, %myinst_u8, %myinst_clock : (!firrtl.cmemory<uint<8>, 8>, !firrtl.uint<8>, !firrtl.clock) -> !firrtl.uint<8>
  %memoryport = firrtl.memoryport Infer %cmem, %u8, %clock : (!firrtl.cmemory<uint<8>, 8>, !firrtl.uint<8>, !firrtl.clock) -> !firrtl.uint<8>
