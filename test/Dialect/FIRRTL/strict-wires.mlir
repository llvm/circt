// RUN: circt-opt -pass-pipeline='builtin.module(firrtl.circuit(firrtl.strictmodule(firrtl-strict-wires)))' %s | FileCheck %s

firrtl.circuit "TopLevel" {

  // CHECK-LABEL: @TopLevel
  firrtl.strictmodule @TopLevel(in %source: !firrtl.bundle<valid: uint<1>, ready: uint<1>, data: uint<64>>,
    ...
    firrtl.strictconnect %sink, %w : !firrtl.bundle<valid: uint<1>, ready: uint<1>, data: uint<64>>
...
// CHECK: firrtl.strictconnect %sink, %w : !firrtl.bundle<valid: uint<1>, ready: uint<1>, data: uint<64>> 
  }

  // CHECK-LABEL: @Subfield
  firrtl.strictmodule @Subfield(in %source: !firrtl.uint<1>,
    ...
    firrtl.strictconnect %sink, %w_valid : !firrtl.uint<1>
...
// CHECK: firrtl.strictconnect %sink, %1 : !firrtl.uint<1>
  }

  // CHECK-LABEL: @Subindex
  firrtl.strictmodule @Subindex(in %source: !firrtl.uint<1>,
                             out %sink: !firrtl.uint<1>) {
...
// CHECK: firrtl.strictconnect %sink, %1 : !firrtl.uint<1>
    ...
    // CHECK-LABEL: @Inst
  firrtl.strictmodule @Inst(in %source: !firrtl.uint<1>,
                             out %sink: !firrtl.uint<1>) {
    ...
    firrtl.strictconnect %sink, %s_sink : !firrtl.uint<1>
...
// CHECK: firrtl.strictconnect %sink, %s_sink : !firrtl.uint<1> 
  }
