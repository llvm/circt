...
firrtl.module @ConstPortModule(
  ...
  %a2_write = firrtl.wrapSink %a2 : !firrtl.const.uint<1>
  firrtl.strictconnect %a2_write, %a : !firrtl.const.uint<1>
  %b2_write = firrtl.wrapSink %b2 : !firrtl.const.bundle<a: uint<1>>
  firrtl.strictconnect %b2_write, %b : !firrtl.const.bundle<a: uint<1>>
  %c2_write = firrtl.wrapSink %c2 : !firrtl.bundle<a: const.uint<1>>
  firrtl.strictconnect %c2_write, %c : !firrtl.bundle<a: const.uint<1>>
  %d2_write = firrtl.wrapSink %d2 : !firrtl.const.vector<uint<1>, 3>
  firrtl.strictconnect %d2_write, %d : !firrtl.const.vector<uint<1>, 3>
  %e2_write = firrtl.wrapSink %e2 : !firrtl.vector<const.uint<1>,3>
  firrtl.strictconnect %e2_write, %e : !firrtl.vector<const.uint<1>, 3>
  %f2_write = firrtl.wrapSink %f2 : !firrtl.const.enum<a: uint<2>, b: uint<1>>
