...
firrtl.circuit "NLALowering" {
  ...
  firrtl.hierpath @nla_b [@fallBackName::@test, @Aardvark::@test, @NLALowering]
  // CHECK:      firrtl.hierpath @nla_b [@fallBackName::@test, @Aardvark::@test, @NLALowering]
  ...
  firrtl.hierpath @nla_b2 [@fallBackName::@test, @Aardvark::@test, @NLALowering]
  // CHECK-NEXT: firrtl.hierpath @nla_b2 [@fallBackName::@test, @Aardvark::@test, @NLALowering]
  ...
  firrtl.module @NLALowering() attributes {annotations = [{circt.nonlocal = @nla, class = "circt.nonlocal"}]}{
    ...
    // CHECK:   %bundle_valid = firrtl.wire : !firrtl.uint<1>
    // CHECK-NEXT:   %bundle_ready = firrtl.wire
    // CHECK-SAME:   {annotations = [{circt.nonlocal = @nla_b, class = "test"}, {circt.nonlocal = @nla_b, class = "firrtl.transforms.DontTouchAnnotation"}]} : !firrtl.uint<1>
    // Note the new NLA added here.
    // CHECK-NEXT:   %bundle_data = firrtl.wire
    // CHECK-SAME: {annotations = [{B, circt.nonlocal = @nla_b}, {A, circt.nonlocal = @nla_b}, {circt.nonlocal = @nla_b, class = "firrtl.transforms.DontTouchAnnotation"}]}
    // CHECK:   %bundle2_valid = firrtl.wire : !firrtl.uint<1>
    // CHECK:   %bundle2_ready = firrtl.wire {annotations = [{circt.nonlocal = @nla_b2, class = "firrtl.transforms.DontTouchAnnotation"}]}
    // CHECK:   %bundle2_data = firrtl.wire {annotations = [{circt.nonlocal = @nla_b2, class = "firrtl.transforms.DontTouchAnnotation"}]}
  }
}

// Test the update of NLA when a new symbol is added after lowering of bundle fields.
firrtl.circuit "NLALoweringNewSymbol" {
  firrtl.hierpath @lowernla_2 [@NLALoweringNewSymbol::@testBundle_Bar, @testBundle_Bar]
  // CHECK: firrtl.hierpath @lowernla_2 [@NLALoweringNewSymbol::@testBundle_Bar, @testBundle_Bar]
  firrtl.hierpath @lowernla_1 [@NLALoweringNewSymbol::@testBundle_Bar, @testBundle_Bar]
  // CHECK: firrtl.hierpath @lowernla_1 [@NLALoweringNewSymbol::@testBundle_Bar, @testBundle_Bar]
  firrtl.module private @testBundle_Bar(
    in %a: !firrtl.uint<1>,
    out %b: !firrtl.bundle<baz: uint<1>, qux: uint<1>, data: uint<2>> [
    ...
    // CHECK-SAME: out %b_baz: !firrtl.uint<1> [{A, circt.nonlocal = @lowernla_1}, {B, circt.nonlocal = @lowernla_1}]
    // CHECK-SAME: out %b_qux: !firrtl.uint<1> [{C, circt.nonlocal = @lowernla_1}]
    // CHECK-SAME: out %b_data: !firrtl.uint<2> [{circt.nonlocal = @lowernla_1, class = "firrtl.transforms.DontTouchAnnotation"}]
    %d = firrtl.wire {
      annotations = [
    ...
    // CHECK: %d_baz = firrtl.wire {annotations = [{A, circt.nonlocal = @lowernla_2}, {C, circt.nonlocal = @lowernla_2}, {D, circt.nonlocal = @lowernla_2}]}
    // CHECK: %d_qux = firrtl.wire {annotations = [{A, circt.nonlocal = @lowernla_2}, {B, circt.nonlocal = @lowernla_2}, {C, circt.nonlocal = @lowernla_2}, {D, circt.nonlocal = @lowernla_2}]} : !firrtl.uint<1>
  }
...
firrtl.circuit "CorrectSymbolsOnly" {
  firrtl.hierpath @path_0 [@CorrectSymbolsOnly::@foo, @Foo]
  firrtl.hierpath @path_1 [@CorrectSymbolsOnly::@foo, @Foo]
  firrtl.module @Foo() {
    // CHECK:      firrtl.wire
