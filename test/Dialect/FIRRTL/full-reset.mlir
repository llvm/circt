// RUN: circt-opt --pass-pipeline='builtin.module(firrtl.circuit(firrtl-full-reset))' --verify-diagnostics --split-input-file %s | FileCheck %s

// Full reset (FART) tests. Extracted from InferResets when full-reset was split
// into its own pass. Prefer chaining firrtl-infer-resets,firrtl-full-reset when
// inputs still contain abstract resets.

//===----------------------------------------------------------------------===//
// Full Reset
//===----------------------------------------------------------------------===//

firrtl.circuit "ConsumeIgnoreAnno" {
