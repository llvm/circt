...
firrtl.circuit "StructCast" {
  firrtl.module @StructCast() {
    ...
    %b = firrtl.rtlStructCast %a : (!firrtl.bundle<valid: uint<1>, ready: uint<1>, data: uint<64>>) -> (!hw.struct<valid: i1, ready: i1>)
  }
}
...
firrtl.circuit "StructCast2" {
  firrtl.module @StructCast2() {
    ...
    %b = firrtl.rtlStructCast %a : (!firrtl.bundle<valid: uint<1>>) -> (!hw.struct<yovalid: i1>)
  }
}
...
firrtl.circuit "StructCast3" {
  firrtl.module @StructCast3() {
    ...
    %b = firrtl.rtlStructCast %a : (!firrtl.bundle<valid: uint<1>>) -> (!hw.struct<valid: i2>)
  }
}
