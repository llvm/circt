...
firrtl.circuit "NoMask" {
  firrtl.module @NoMask(in %en: !firrtl.uint<1>, in %mask: !firrtl.uint<1>) {
    ...
    // Enable:
    %0 = firrtl.subfield %MemSimple_write[1] : !firrtl.bundle<addr: uint<4>, en: uint<1>, clk: clock, data: uint<42>, mask: uint<1>>
    firrtl.connect %0, %en : !firrtl.uint<1>, !firrtl.uint<1>

    // Mask:
    %1 = firrtl.subfield %MemSimple_write[4] : !firrtl.bundle<addr: uint<4>, en: uint<1>, clk: clock, data: uint<42>, mask: uint<1>>
...
firrtl.circuit "YesMask" {
  firrtl.module @YesMask(in %en: !firrtl.uint<1>, in %mask: !firrtl.uint<4>) {
    ...
    // Enable:
    %0 = firrtl.subfield %MemSimple_write[1] : !firrtl.bundle<addr: uint<10>, en: uint<1>, clk: clock, data: uint<40>, mask: uint<4>>
    firrtl.connect %0, %en : !firrtl.uint<1>, !firrtl.uint<1>

    // Mask:
    %1 = firrtl.subfield %MemSimple_write[4] : !firrtl.bundle<addr: uint<10>, en: uint<1>, clk: clock, data: uint<40>, mask: uint<4>>
...
firrtl.circuit "MemDepth1" {
  firrtl.module @MemDepth1(in %clock: !firrtl.clock, in %en: !firrtl.uint<1>,
    ...
    %1 = firrtl.subfield %mem0_write[0] : !firrtl.bundle<addr: uint<1>, en: uint<1>, clk: clock, data: uint<32>, mask: uint<4>>
    firrtl.connect %1, %addr : !firrtl.uint<1>, !firrtl.uint<1>
    %3 = firrtl.subfield %mem0_write[1] : !firrtl.bundle<addr: uint<1>, en: uint<1>, clk: clock, data: uint<32>, mask: uint<4>>
    firrtl.connect %3, %en : !firrtl.uint<1>, !firrtl.uint<1>
    %0 = firrtl.subfield %mem0_write[2] : !firrtl.bundle<addr: uint<1>, en: uint<1>, clk: clock, data: uint<32>, mask: uint<4>>
    firrtl.connect %0, %clock : !firrtl.clock, !firrtl.clock
    %2 = firrtl.subfield %mem0_write[3] : !firrtl.bundle<addr: uint<1>, en: uint<1>, clk: clock, data: uint<32>, mask: uint<4>>
...
firrtl.circuit "inferUnmaskedMemory" {
  firrtl.module @inferUnmaskedMemory(in %clock: !firrtl.clock, in %rAddr: !firrtl.uint<4>, in %rEn: !firrtl.uint<1>, out %rData: !firrtl.uint<8>, in %wMode: !firrtl.uint<1>, in %wMask: !firrtl.uint<1>, in %wData: !firrtl.uint<8>) {
    ...
    %MReadWrite_readwrite = firrtl.mem Undefined {depth = 12 : i64, name = "MReadWrite", portNames = ["readwrite"], readLatency = 1 : i32, writeLatency = 1 : i32} : !firrtl.bundle<addr: uint<4>, en: uint<1>, clk: clock, rdata flip: uint<42>, wmode: uint<1>, wdata: uint<42>, wmask: uint<1>>
    %rw_en = firrtl.subfield %MReadWrite_readwrite[1] : !firrtl.bundle<addr: uint<4>, en: uint<1>, clk: clock, rdata flip: uint<42>, wmode: uint<1>, wdata: uint<42>, wmask: uint<1>>
    %rw_wmode = firrtl.subfield %MReadWrite_readwrite[4] : !firrtl.bundle<addr: uint<4>, en: uint<1>, clk: clock, rdata flip: uint<42>, wmode: uint<1>, wdata: uint<42>, wmask: uint<1>>
    %rw_mask = firrtl.subfield %MReadWrite_readwrite[6] : !firrtl.bundle<addr: uint<4>, en: uint<1>, clk: clock, rdata flip: uint<42>, wmode: uint<1>, wdata: uint<42>, wmask: uint<1>>
