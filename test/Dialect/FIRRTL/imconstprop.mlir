...
firrtl.circuit "testDontTouch"  {
  ...
  firrtl.module private @blockProp1(in %clock: !firrtl.clock, in %a: !firrtl.uint<1> [{class = "firrtl.transforms.DontTouchAnnotation"}], out %b: !firrtl.uint<1>) {
    //CHECK: %c = firrtl.reg
...
firrtl.circuit "OutPortTop" {
    firrtl.module private @OutPortChild1(out %out: !firrtl.uint<1> [{class = "firrtl.transforms.DontTouchAnnotation"}]) {
      %c0_ui1 = firrtl.constant 0 : !firrtl.uint<1>
...
firrtl.circuit "InputPortTop"   {
  ...
  firrtl.module private @InputPortChild(in %in0: !firrtl.uint<1>, in %in1: !firrtl.uint<1> [{class = "firrtl.transforms.DontTouchAnnotation"}], out %out: !firrtl.uint<1>) {
...
firrtl.circuit "dntOutput" {
  ...
  firrtl.module private @foo(out %b: !firrtl.uint<3> [{class = "firrtl.transforms.DontTouchAnnotation"}]) {
