...
firrtl.module @reset0(in %a : !firrtl.uint<1>, out %b : !firrtl.reset) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.reset, !firrtl.uint<1>
}

firrtl.module @reset1(in %a : !firrtl.asyncreset, out %b : !firrtl.reset) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.reset, !firrtl.asyncreset
}
...
firrtl.module @sint0(in %a : !firrtl.sint<1>, out %b : !firrtl.sint<1>) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.sint<1>, !firrtl.sint<1>
}

// UInt<> source.
firrtl.module @uint0(in %a : !firrtl.uint<1>, out %b : !firrtl.uint<1>) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.uint<1>, !firrtl.uint<1>
}
firrtl.module @uint1(in %a : !firrtl.uint<1>, out %b : !firrtl.uint<2>) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.uint<2>, !firrtl.uint<1>
}
...
firrtl.module @vect0(in %a : !firrtl.vector<uint<1>, 3>, out %b : !firrtl.vector<uint<1>, 3>) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.vector<uint<1>, 3>, !firrtl.vector<uint<1>, 3>
}
...
firrtl.module @bundle0(in %a : !firrtl.bundle<f1: uint<1>, f2 flip: sint<1>>, out %b : !firrtl.bundle<f1: uint<1>, f2 flip: sint<1>>) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.bundle<f1: uint<1>, f2 flip: sint<1>>, !firrtl.bundle<f1: uint<1>, f2 flip: sint<1>>
}

firrtl.module @bundle1(in %a : !firrtl.bundle<f1: uint<1>, f2 flip: sint<2>>, out %b : !firrtl.bundle<f1: uint<2>, f2 flip: sint<1>>) {
  // CHECK: chirrtl.connect %b, %a
  chirrtl.connect %b, %a : !firrtl.bundle<f1: uint<2>, f2 flip: sint<1>>, !firrtl.bundle<f1: uint<1>, f2 flip: sint<2>>
}
...
firrtl.module @wires0(in %in : !firrtl.uint<1>, out %out : !firrtl.uint<1>) {
  ...
  // CHECK: chirrtl.connect %w, %in : !firrtl.uint<1>, !firrtl.uint<1>
  // CHECK: chirrtl.connect %out, %w : !firrtl.uint<1>, !firrtl.uint<1>
  chirrtl.connect %w, %in : !firrtl.uint<1>, !firrtl.uint<1>
  chirrtl.connect %out, %w : !firrtl.uint<1>, !firrtl.uint<1>
}

firrtl.module @wires1(in %in : !firrtl.uint<1>, out %out : !firrtl.uint<1>) {
  %wf = firrtl.wire : !firrtl.uint<1>
  // CHECK: chirrtl.connect %wf, %in : !firrtl.uint<1>, !firrtl.uint<1>
  // CHECK: chirrtl.connect %out, %wf : !firrtl.uint<1>, !firrtl.uint<1>
  chirrtl.connect %wf, %in : !firrtl.uint<1>, !firrtl.uint<1>
  chirrtl.connect %out, %wf : !firrtl.uint<1>, !firrtl.uint<1>
}

firrtl.module @wires2() {
  %w0 = firrtl.wire : !firrtl.uint<1>
  %w1 = firrtl.wire : !firrtl.uint<1>
  // CHECK: chirrtl.connect %w0, %w1
  chirrtl.connect %w0, %w1 : !firrtl.uint<1>, !firrtl.uint<1>
}

firrtl.module @wires3(out %out : !firrtl.uint<1>) {
  %wf = firrtl.wire : !firrtl.uint<1>
  // check that we can read from an output port
  // CHECK: chirrtl.connect %wf, %out
  chirrtl.connect %wf, %out : !firrtl.uint<1>, !firrtl.uint<1>
}
...
firrtl.module @wires4(in %in : !firrtl.uint<1>, out %out : !firrtl.uint<1>) {
  ...
  // CHECK: chirrtl.connect %0, %in : !firrtl.uint<1>, !firrtl.uint<1>
  // CHECK: chirrtl.connect %out, %0 : !firrtl.uint<1>, !firrtl.uint<1>
  chirrtl.connect %0, %in : !firrtl.uint<1>, !firrtl.uint<1>
  chirrtl.connect %out, %0 : !firrtl.uint<1>, !firrtl.uint<1>
}
...
firrtl.module @registers1(in %clock : !firrtl.clock) {
  ...
  // CHECK: chirrtl.connect %0, %1
  chirrtl.connect %0, %1 : !firrtl.uint<1>, !firrtl.uint<1>
}

firrtl.module @ConstClock(in %in : !firrtl.const.clock, out %out : !firrtl.const.clock) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.clock, !firrtl.const.clock
  chirrtl.connect %out, %in : !firrtl.const.clock, !firrtl.const.clock
}

firrtl.module @ConstReset(in %in : !firrtl.const.reset, out %out : !firrtl.const.reset) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.reset, !firrtl.const.reset
  chirrtl.connect %out, %in : !firrtl.const.reset, !firrtl.const.reset
}

firrtl.module @ConstAsyncReset(in %in : !firrtl.const.asyncreset, out %out : !firrtl.const.asyncreset) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.asyncreset, !firrtl.const.asyncreset
  chirrtl.connect %out, %in : !firrtl.const.asyncreset, !firrtl.const.asyncreset
}

firrtl.module @ConstUInt(in %in : !firrtl.const.uint<2>, out %out : !firrtl.const.uint<2>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.uint<2>, !firrtl.const.uint<2>
  chirrtl.connect %out, %in : !firrtl.const.uint<2>, !firrtl.const.uint<2>
}

firrtl.module @ConstSInt(in %in : !firrtl.const.sint<2>, out %out : !firrtl.const.sint<2>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.sint<2>, !firrtl.const.sint<2>
  chirrtl.connect %out, %in : !firrtl.const.sint<2>, !firrtl.const.sint<2>
}

firrtl.module @ConstVec(in %in : !firrtl.const.vector<uint<1>, 3>, out %out : !firrtl.const.vector<uint<1>, 3>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.vector<uint<1>, 3>, !firrtl.const.vector<uint<1>, 3>
  chirrtl.connect %out, %in : !firrtl.const.vector<uint<1>, 3>, !firrtl.const.vector<uint<1>, 3>
}

firrtl.module @ConstBundle(in %in : !firrtl.const.bundle<a: uint<1>, b: sint<2>>, out %out : !firrtl.const.bundle<a: uint<1>, b: sint<2>>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.bundle<a: uint<1>, b: sint<2>>, !firrtl.const.bundle<a: uint<1>, b: sint<2>>
  chirrtl.connect %out, %in : !firrtl.const.bundle<a: uint<1>, b: sint<2>>, !firrtl.const.bundle<a: uint<1>, b: sint<2>>
}

firrtl.module @MixedConstBundle(in %in : !firrtl.bundle<a: uint<1>, b: const.sint<2>>, out %out : !firrtl.bundle<a: uint<1>, b: const.sint<2>>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.bundle<a: uint<1>, b: const.sint<2>>, !firrtl.bundle<a: uint<1>, b: const.sint<2>>
  chirrtl.connect %out, %in : !firrtl.bundle<a: uint<1>, b: const.sint<2>>, !firrtl.bundle<a: uint<1>, b: const.sint<2>>
}
...
firrtl.module @ConstToExplicitConstElementsBundle(in %in : !firrtl.const.bundle<a: uint<1>, b: sint<2>>, out %out : !firrtl.const.bundle<a: const.uint<1>, b: const.sint<2>>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.bundle<a: const.uint<1>, b: const.sint<2>>, !firrtl.const.bundle<a: uint<1>, b: sint<2>>
  chirrtl.connect %out, %in : !firrtl.const.bundle<a: const.uint<1>, b: const.sint<2>>, !firrtl.const.bundle<a: uint<1>, b: sint<2>>
}

firrtl.module @ConstToNonConstClock(in %in : !firrtl.const.clock, out %out : !firrtl.clock) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.clock, !firrtl.const.clock
  chirrtl.connect %out, %in : !firrtl.clock, !firrtl.const.clock
}

firrtl.module @ConstToNonConstReset(in %in : !firrtl.const.reset, out %out : !firrtl.reset) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.reset, !firrtl.const.reset
  chirrtl.connect %out, %in : !firrtl.reset, !firrtl.const.reset
}

firrtl.module @ConstToNonConstAsyncReset(in %in : !firrtl.const.asyncreset, out %out : !firrtl.asyncreset) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.asyncreset, !firrtl.const.asyncreset
  chirrtl.connect %out, %in : !firrtl.asyncreset, !firrtl.const.asyncreset
}

firrtl.module @ConstToNonConstUInt(in %in : !firrtl.const.uint<2>, out %out : !firrtl.uint<2>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.uint<2>, !firrtl.const.uint<2>
  chirrtl.connect %out, %in : !firrtl.uint<2>, !firrtl.const.uint<2>
}

firrtl.module @ConstToNonConstBundle(in %in : !firrtl.const.bundle<a: uint<1>, b: sint<2>>, out %out : !firrtl.bundle<a: uint<1>, b: sint<2>>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.bundle<a: uint<1>, b: sint<2>>, !firrtl.const.bundle<a: uint<1>, b: sint<2>>
  chirrtl.connect %out, %in : !firrtl.bundle<a: uint<1>, b: sint<2>>, !firrtl.const.bundle<a: uint<1>, b: sint<2>>
}
...
firrtl.module @MixedConstToNonConstBundle(in %in : !firrtl.bundle<a: uint<1>, b: const.sint<2>>, out %out : !firrtl.bundle<a: uint<1>, b: sint<2>>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.bundle<a: uint<1>, b: sint<2>>, !firrtl.bundle<a: uint<1>, b: const.sint<2>>
  chirrtl.connect %out, %in : !firrtl.bundle<a: uint<1>, b: sint<2>>, !firrtl.bundle<a: uint<1>, b: const.sint<2>>
}

firrtl.module @ConstToExplicitConstElementsVec(in %in : !firrtl.const.vector<uint<1>, 3>, out %out : !firrtl.vector<const.uint<1>, 3>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.vector<const.uint<1>, 3>, !firrtl.const.vector<uint<1>, 3>
  chirrtl.connect %out, %in : !firrtl.vector<const.uint<1>, 3>, !firrtl.const.vector<uint<1>, 3>
}

firrtl.module @ConstToNonConstVec(in %in : !firrtl.const.vector<uint<1>, 3>, out %out : !firrtl.vector<uint<1>, 3>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.vector<uint<1>, 3>, !firrtl.const.vector<uint<1>, 3>
  chirrtl.connect %out, %in : !firrtl.vector<uint<1>, 3>, !firrtl.const.vector<uint<1>, 3>
}

firrtl.module @NonConstToConstFlip(in %in   : !firrtl.bundle<a flip: uint<1>>,
                                   out %out : !firrtl.const.bundle<a flip: uint<1>>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.const.bundle<a flip: uint<1>>, !firrtl.bundle<a flip: uint<1>>
  chirrtl.connect %out, %in : !firrtl.const.bundle<a flip: uint<1>>, !firrtl.bundle<a flip: uint<1>>
}

firrtl.module @NonConstToNestedConstFlip(in %in   : !firrtl.bundle<a flip: uint<1>>,
                                         out %out : !firrtl.bundle<a flip: const.uint<1>>) {
  // CHECK: chirrtl.connect %out, %in : !firrtl.bundle<a flip: const.uint<1>>, !firrtl.bundle<a flip: uint<1>>
  chirrtl.connect %out, %in : !firrtl.bundle<a flip: const.uint<1>>, !firrtl.bundle<a flip: uint<1>>
}
...
firrtl.module @NonConstToNonConstFlipFromConstSubaccess(in %in    : !firrtl.bundle<a flip: uint<1>>,
  ...
  // CHECK: chirrtl.connect %0, %in : !firrtl.bundle<a flip: uint<1>>, !firrtl.bundle<a flip: uint<1>>
  chirrtl.connect %0, %in : !firrtl.bundle<a flip: uint<1>>, !firrtl.bundle<a flip: uint<1>>
}
...
firrtl.module @NonConstWhenConstFlipAssign(in %p: !firrtl.uint<1>, in %in: !firrtl.bundle<a flip: uint<2>>, out %out: !firrtl.const.bundle<a flip: uint<2>>) {
  firrtl.when %p : !firrtl.uint<1> {
    chirrtl.connect %out, %in : !firrtl.const.bundle<a flip: uint<2>>, !firrtl.bundle<a flip: uint<2>>
  }
}
...
firrtl.module @NonConstWhenNestedConstFlipAssign(in %p: !firrtl.uint<1>, in %in: !firrtl.bundle<a flip: uint<2>>, out %out: !firrtl.bundle<a flip: const.uint<2>>) {
  firrtl.when %p : !firrtl.uint<1> {
    chirrtl.connect %out, %in : !firrtl.bundle<a flip: const.uint<2>>, !firrtl.bundle<a flip: uint<2>>
  }
}
...
firrtl.module @NonConstWhenLocalConstFlipAssign(in %cond: !firrtl.uint<1>, out %out : !firrtl.const.bundle<a flip: uint<2>>) {
  firrtl.when %cond : !firrtl.uint<1> {
    ...
    chirrtl.connect %out, %w : !firrtl.const.bundle<a flip: uint<2>>, !firrtl.const.bundle<a flip: uint<2>>
  }
}
...
firrtl.module @NonConstWhenLocalNestedConstFlipAssign(in %cond: !firrtl.uint<1>, out %out : !firrtl.bundle<a flip: const.uint<2>>) {
  firrtl.when %cond : !firrtl.uint<1> {
    ...
    chirrtl.connect %out, %w : !firrtl.bundle<a flip: const.uint<2>>, !firrtl.bundle<a flip: const.uint<2>>
  }
}
}
