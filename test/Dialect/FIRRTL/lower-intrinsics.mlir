...
firrtl.circuit "Foo" {
  ...
  firrtl.module @LTL(in %in0 : !firrtl.uint<1>, in %in1 : !firrtl.uint<1>, in %clk : !firrtl.clock) {
    ...
    // CHECK-NEXT: firrtl.int.ltl.clocked_past %in0, 42, %clk :
    firrtl.int.generic "circt_ltl_clocked_past" <delay: i64 = 42> %in0, %clk : (!firrtl.uint<1>, !firrtl.clock) -> !firrtl.uint<1>
