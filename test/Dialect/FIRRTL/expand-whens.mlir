...
firrtl.module @WhenCForce(in %c: !firrtl.uint<1>, in %clock : !firrtl.clock, in %x: !firrtl.uint<4>) {
  ...
  firrtl.when %c : !firrtl.uint<1> {
    ...
    firrtl.ref.force %clock, %c1_ui1, %n_ref, %x : !firrtl.clock, !firrtl.const.uint<1>, !firrtl.uint<4>
    // CHECK: firrtl.ref.force_initial %c, %n_ref, %x :
    firrtl.ref.force_initial %c1_ui1, %n_ref, %x : !firrtl.const.uint<1>, !firrtl.uint<4>
    // CHECK: firrtl.ref.release %clock, %c, %n_ref :
    firrtl.ref.release %clock, %c1_ui1, %n_ref : !firrtl.clock, !firrtl.const.uint<1>, !firrtl.rwprobe<uint<4>>
    // CHECK: firrtl.ref.release_initial %c, %n_ref :
    firrtl.ref.release_initial %c1_ui1, %n_ref : !firrtl.const.uint<1>, !firrtl.rwprobe<uint<4>>
  }
}
