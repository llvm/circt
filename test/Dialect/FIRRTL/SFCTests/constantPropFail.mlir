...
// These depend on more than constant prop.  They need to move.
  ...
  firrtl.module @padZeroReg(in %clock: !firrtl.clock, out %z: !firrtl.uint<16>) {
      ...
      %_n = firrtl.node droppable_name %c171_ui8  : !firrtl.const.uint<8>
      %1 = firrtl.cat %_n, %_r : (!firrtl.const.uint<8>, !firrtl.uint<8>) -> !firrtl.uint<16>
      firrtl.strictconnect %z, %1 : !firrtl.uint<16>
    // CHECK: %[[TMP:.+]] = firrtl.constant 43776 : !firrtl.const.uint<16>
    // CHECK-NEXT: firrtl.strictconnect %z, %[[TMP]] : !firrtl.uint<16>, !firrtl.const.uint<16>
  }
