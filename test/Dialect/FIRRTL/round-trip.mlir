...
firrtl.module @DomainTypesWithFields(
  ...
  // CHECK: %my_domain = firrtl.domain.create (%[[name]], %[[voltage]], %[[alwaysOn]]) : (!firrtl.string, !firrtl.integer, !firrtl.bool) -> !firrtl.domain<@PowerDomain(name: !firrtl.string, voltage: !firrtl.integer, alwaysOn: !firrtl.bool)>
  %name = firrtl.string "VDD"
  %voltage = firrtl.integer 1800
  %alwaysOn = firrtl.bool true
  %my_domain = firrtl.domain.create(%name, %voltage, %alwaysOn) : (!firrtl.string, !firrtl.integer, !firrtl.bool) -> !firrtl.domain<@PowerDomain(name: !firrtl.string, voltage: !firrtl.integer, alwaysOn: !firrtl.bool)>
