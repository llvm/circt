...
firrtl.module @TypeAlias(in %c: !firrtl.uint<1>, in %in: !firrtl.alias<bar, uint<1>>, out %out: !firrtl.alias<foo, uint<1>>) {
  ...
  // CHECK:      %0 = firrtl.mux(%c, %out, %in) : (!firrtl.uint<1>, !firrtl.alias<foo, uint<1>>, !firrtl.alias<bar, uint<1>>) -> !firrtl.uint<1>
  // CHECK-NEXT: %1 = firrtl.mux(%c, %out, %in) : (!firrtl.uint<1>, !firrtl.alias<foo, uint<1>>, !firrtl.alias<bar, uint<1>>) -> !firrtl.alias<foo, uint<1>>
  // CHECK-NEXT: %2 = firrtl.mux(%c, %out, %in) : (!firrtl.uint<1>, !firrtl.alias<foo, uint<1>>, !firrtl.alias<bar, uint<1>>) -> !firrtl.alias<bar, uint<1>>
