...
// RUN: circt-opt -pass-pipeline='builtin.module(firrtl-imdeadcodeelim{remove-ports-only ignore-dont-touch})' %s -split-input-file | FileCheck %s --check-prefix=IGNORE
firrtl.circuit "Top"   {
...
// Test ignoreDontTouch option - removes unused ports even with symbols/annotations

// IGNORE-LABEL: firrtl.circuit "IgnoreDontTouch"
firrtl.circuit "IgnoreDontTouch" {
  firrtl.module @IgnoreDontTouch() {
    // IGNORE: firrtl.instance bar @Bar()
    %bar_sym, %bar_anno = firrtl.instance bar @Bar(out sym: !firrtl.uint<1>, out anno: !firrtl.uint<1>)
  }

  // IGNORE-LABEL: firrtl.module private @Bar() {
  // IGNORE-NEXT:  }
