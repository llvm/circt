...
// RUN: circt-reduce %s --test /bin/sh --test-arg -c --test-arg 'firtool "$0" 2>&1 | grep -q "error: Instance input \`x\` is not initialized"' --keep-best=0 -exclude=module-name-sanitizer -exclude=module-internal-name-sanitizer | FileCheck %s
