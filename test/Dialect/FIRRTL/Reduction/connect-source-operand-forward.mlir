...
firrtl.circuit "Foo" {
  ...
  firrtl.module @Foo(in %clock: !firrtl.clock, in %reset: !firrtl.uint<1>, in %val: !firrtl.uint<2>) {
    ...
    %b = firrtl.reg %clock {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<1>
    %rst = firrtl.asReset %reset : (!firrtl.uint<1>) -> !firrtl.reset
    %c = firrtl.regreset %clock, %rst, %reset {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 0 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<1>, !firrtl.uint<1>
    ...
    // CHECK-NEXT:   %b = firrtl.reg %clock {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<2>
    // CHECK-NEXT:   %0 = firrtl.asReset %reset : (!firrtl.uint<1>) -> !firrtl.reset
    // CHECK-NEXT:   %c = firrtl.reg %clock {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<2>
