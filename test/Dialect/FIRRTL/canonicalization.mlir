...
firrtl.module @namedrop(in %clock: !firrtl.clock, in %reset: !firrtl.uint<1>, in %in: !firrtl.uint<1>, out %out: !firrtl.uint<1>) {
  // CHECK-NOT: _T_
  %c1_ui1 = firrtl.constant 1 : !firrtl.uint<1>
  %_T_0 = firrtl.node droppable_name %in : !firrtl.uint<1>
  %_T_1 = firrtl.wire droppable_name : !firrtl.uint<1>
  %_T_2 = firrtl.reg droppable_name %clock : !firrtl.uint<1>
  %_T_3 = firrtl.regreset droppable_name %clock, %reset, %c1_ui1 : !firrtl.uint<1>, !firrtl.uint<1>, !firrtl.uint<1>
  %a = firrtl.mem droppable_name Undefined  {depth = 8 : i64, name = "_T_5", portNames = ["a"], readLatency = 0 : i32, writeLatency = 1 : i32} : !firrtl.bundle<addr: uint<3>, en: uint<1>, clk: clock, data flip: uint<1>>
