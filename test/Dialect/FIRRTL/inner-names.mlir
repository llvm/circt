...
firrtl.circuit "Foo" {
  ...
  // CHECK-SAME: in %reset: !firrtl.reset sym @symReset
  firrtl.module @Foo(
    ...
    // CHECK: %regName = firrtl.reg sym @regSym %clock {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<42>
    %regName = firrtl.reg sym @regSym %clock {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<42>
    // CHECK: %regResetName = firrtl.regreset sym @regResetSym %clock, %reset, %value {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<42>, !firrtl.uint<42>
    %regResetName = firrtl.regreset sym @regResetSym %clock, %reset, %value {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<42>, !firrtl.uint<42>
