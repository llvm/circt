...
// RUN: circt-opt -pass-pipeline='builtin.module(firrtl.circuit(firrtl-lower-signatures,firrtl.module(firrtl-lower-types{preserve-aggregate=1d-vec})))' %s | FileCheck --check-prefixes=1DVEC,COMMON --implicit-check-not=mux %s
// RUN: circt-opt -pass-pipeline='builtin.module(firrtl.circuit(firrtl-lower-signatures,firrtl.module(firrtl-lower-types{preserve-aggregate=all})))' %s | FileCheck --check-prefixes=ALL,COMMON --implicit-check-not=mux %s
