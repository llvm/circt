...
// CHECK:  firrtl.strictconnect %[[readAddr]], %[[index2:.+]] : firrtl.uint<4>
// CHECK:  firrtl.strictconnect %[[readEnable]], %mem_MPORT_en : firrtl.uint<1>
// CHECK:  firrtl.strictconnect %[[writeAddr]], %index : firrtl.uint<4>
// CHECK:  firrtl.strictconnect %[[writeEnable]], %wen : firrtl.uint<1>
// CHECK:  %[[v10:.+]] = firrtl.not %wen : (!firrtl.uint<1>) -> !firrtl.uint<1>
// CHECK:  firrtl.strictconnect %mem_MPORT_en, %[[v10]] : firrtl.uint<1>
...
// CHECK:    %mem_rw = firrtl.mem Undefined  {depth = 2048 : i64, name = "mem", portNames = ["rw"], readLatency = 1 : i32, writeLatency = 1 : i32} : !firrtl.bundle<addr: uint<11>, en: uint<1>, clk: clock, rdata flip: uint<32>, wmode: uint<1>, wdata: uint<32>, wmask: uint<1>>
    ...
    firrtl.strictconnect %0, %io_addr : firrtl.uint<11>
    firrtl.strictconnect %1, %io_wen : firrtl.uint<1>
    firrtl.strictconnect %2, %clock : firrtl.clock
    firrtl.strictconnect %4, %c1_ui1 : firrtl.uint<1>
    firrtl.strictconnect %3, %io_dataIn : firrtl.uint<32>
    ...
    firrtl.strictconnect %6, %10 : firrtl.uint<1>
    firrtl.strictconnect %5, %io_addr : firrtl.uint<11>
    firrtl.strictconnect %7, %clock : firrtl.clock
    firrtl.strictconnect %io_dataOut, %8 : firrtl.uint<32>
...
// CHECK:   %[[v8:.+]] = firrtl.or %readEnable, %writeEnable : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
    ...
    firrtl.strictconnect %0, %io_addr : firrtl.uint<11>
    firrtl.strictconnect %1, %9 : firrtl.uint<1>
    firrtl.strictconnect %2, %clock : firrtl.clock
    firrtl.strictconnect %4, %c1_ui1 : firrtl.uint<1>
    firrtl.strictconnect %3, %io_dataIn : firrtl.uint<32>
    ...
    firrtl.strictconnect %6, %13 : firrtl.uint<1>
    firrtl.strictconnect %5, %io_addr : firrtl.uint<11>
    firrtl.strictconnect %7, %clock : firrtl.clock
    firrtl.strictconnect %io_dataOut, %8 : firrtl.uint<32>
...
// CHECK-LABEL: firrtl.module @memTest5t
  firrtl.module @memTest5t(in %clk1: !firrtl.clock, in %clk2: !firrtl.clock, in %reset: !firrtl.uint<1>, in %io_en: !firrtl.uint<1>, in %io_wen: !firrtl.uint<1>, in %io_waddr: !firrtl.uint<8>, in %io_wdata: !firrtl.uint<32>, in %io_raddr: !firrtl.uint<8>, out %io_rdata: !firrtl.uint<32>) {
    ...
    firrtl.strictconnect %0, %11 : firrtl.uint<7>
    firrtl.strictconnect %2, %clk1 : firrtl.clock
    firrtl.strictconnect %io_rdata, %3 : firrtl.uint<32>
    ...
    firrtl.strictconnect %4, %13 : firrtl.uint<7>
    firrtl.strictconnect %5, %12 : firrtl.uint<1>
    firrtl.strictconnect %6, %clk2 : firrtl.clock
    firrtl.strictconnect %8, %c1_ui1 : firrtl.uint<1>
    firrtl.strictconnect %7, %io_wdata : firrtl.uint<32>
  }
...
// CHECK:    %mem_rw = firrtl.mem Undefined  {depth = 2048 : i64, name = "mem", portNames = ["rw"], readLatency = 1 : i32, writeLatency = 1 : i32} : !firrtl.bundle<addr: uint<11>, en: uint<1>, clk: clock, rdata flip: uint<32>, wmode: uint<1>, wdata: uint<32>, wmask: uint<1>>
    ...
    firrtl.strictconnect %2, %xc : firrtl.clock
    firrtl.strictconnect %xc, %clock : firrtl.clock
    firrtl.strictconnect %4, %c1_ui1 : firrtl.uint<1>
    firrtl.strictconnect %3, %io_dataIn : firrtl.uint<32>
    %9 = firrtl.not %io_wen : (!firrtl.uint<1>) -> !firrtl.uint<1>
    %10 = firrtl.mux(%9, %io_ren, %c0_ui1) : (!firrtl.uint<1>, !firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
    ...
    firrtl.module @constMask(in %clock: !firrtl.clock, in %reset: !firrtl.asyncreset, in %index: !firrtl.uint<4>, in %index2: !firrtl.uint<4>, in %data_0: !firrtl.uint<8>, in %wen: !firrtl.uint<1>, in %_T_29: !firrtl.uint<1>, out %auto_0: !firrtl.uint<8>) {
      ...
      firrtl.strictconnect %8, %c1 : firrtl.uint<2>
      %9 = firrtl.not %wen : (!firrtl.uint<1>) -> !firrtl.uint<1>
      firrtl.strictconnect %mem_MPORT_en, %9 : firrtl.uint<1>
      %REG = firrtl.reg %clock  : !firrtl.clock, !firrtl.uint<1>
      firrtl.strictconnect %REG, %9 : firrtl.uint<1>
