...
firrtl.circuit "DPI" {
  // CHECK-NEXT: sim.func.dpi private @uncloked_result(in %in_0 : i2, in %in_1 : i2, out out_0 : i2) attributes {verilogName = "uncloked_result"}
  // CHECK-NEXT: sim.func.dpi private @cloked_void(in %in_0 : i2, in %in_1 : i2) attributes {verilogName = "cloked_void"}
  ...
  firrtl.module @DPI(in %clock: !firrtl.clock, in %enable: !firrtl.uint<1>, in %in_0: !firrtl.uint<2>, in %in_1: !firrtl.uint<2>, out %out_0: !firrtl.uint<2>, out %out_1: !firrtl.uint<2>) attributes {convention = #firrtl<convention scalarized>} {
    ...
    // CHECK-NEXT: sim.func.dpi.call @cloked_void(%8, %9) clock %6 enable %7 : (i2, i2) -> ()
    ...
    // CHECK-NEXT: %13 = sim.func.dpi.call @uncloked_result(%11, %12) enable %10 : (i2, i2) -> i2
    ...
    firrtl.int.dpi.call "cloked_void"(%in_0, %in_1) clock %clock enable %enable : (!firrtl.uint<2>, !firrtl.uint<2>) -> ()
    %1 = firrtl.int.dpi.call "uncloked_result"(%in_0, %in_1) enable %enable {name = "result2"} : (!firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<2>
