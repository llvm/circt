...
firrtl.circuit "Foo" {
  ...
  firrtl.module @CastOp() {
    ...
    %7 = firrtl.asUInt %3 : (!firrtl.reset) -> !firrtl.uint<1>
  ...
  firrtl.module @RegSimple(in %clk: !firrtl.clock, in %x: !firrtl.uint<6>) {
    // CHECK: %0 = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<6>
    // CHECK: %1 = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<6>
    %0 = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint
    %1 = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint
    %2 = firrtl.wire : !firrtl.uint
  ...
  firrtl.module @RegShl(in %clk: !firrtl.clock, in %x: !firrtl.uint<6>) {
    ...
    %0 = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint
    %1 = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint
    %2 = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint
  ...
  firrtl.module @RegResetSimple(
    ...
    // CHECK: %0 = firrtl.regreset %clk, %rst, %c0_ui1 {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<1>, !firrtl.uint<6>
    // CHECK: %1 = firrtl.regreset %clk, %rst, %c0_ui1 {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<1>, !firrtl.uint<6>
    // CHECK: %2:2 = firrtl.regreset %clk, %rst, %c0_ui17 forceable {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<17>, !firrtl.uint<17>, !firrtl.rwprobe<uint<17>>
    // CHECK: %3 = firrtl.regreset %clk, %rst, %c0_ui17 {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<17>, !firrtl.uint<17>
    %c0_ui = firrtl.constant 0 : !firrtl.uint
    %c0_ui17 = firrtl.constant 0 : !firrtl.uint<17>
    %0 = firrtl.regreset %clk, %rst, %c0_ui {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint, !firrtl.uint
    %1 = firrtl.regreset %clk, %rst, %c0_ui {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint, !firrtl.uint
    %2:2 = firrtl.regreset %clk, %rst, %c0_ui17 forceable {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<17>, !firrtl.uint, !firrtl.rwprobe<uint>
    %3 = firrtl.regreset %clk, %rst, %c0_ui17 {clockEdge = 0 : i32, resetPolarity = 0 : i32, resetType = 1 : i32} : !firrtl.clock, !firrtl.reset, !firrtl.uint<17>, !firrtl.uint
  ...
  firrtl.module @InferBundle(in %in : !firrtl.uint<3>, in %clk : !firrtl.clock) {
    ...
    // CHECK: firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.bundle<a: uint<3>>
    %w = firrtl.wire : !firrtl.bundle<a: uint>
    %r = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.bundle<a: uint>
  ...
  firrtl.module @InferVectorSubindex(in %in : !firrtl.uint<4>, in %clk : !firrtl.clock) {
    ...
    // CHECK: firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.vector<uint<4>, 10>
    %w = firrtl.wire : !firrtl.vector<uint, 10>
    %r = firrtl.reg %clk {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.vector<uint, 10>
  ...
  firrtl.module @Issue1271(in %clock: !firrtl.clock, in %cond: !firrtl.uint<1>) {
    // CHECK: %a = firrtl.reg %clock {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint<2>
    // CHECK: %b = firrtl.node %0  : !firrtl.uint<3>
    // CHECK: %c = firrtl.node %1  : !firrtl.uint<2>
    %a = firrtl.reg %clock {clockEdge = 0 : i32} : !firrtl.clock, !firrtl.uint
    %c0_ui1 = firrtl.constant 0 : !firrtl.uint<1>
