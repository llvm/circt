...
firrtl.module @InferRead(in %cond: !firrtl.uint<1>, in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, out %out : !firrtl.uint<1>, in %vec : !firrtl.vector<uint<1>, 2>) {
  ...
  // CHECK: firrtl.strictconnect [[ADDR_W]], %invalid_ui8
  // CHECK: [[EN:%.*]] = firrtl.subfield %ram_ramport[en]
  // CHECK: [[EN_W:%.*]] = firrtl.wrapSink [[EN]]
  // CHECK: firrtl.strictconnect [[EN_W]], %c0_ui1
  // CHECK: [[CLOCK:%.*]] = firrtl.subfield %ram_ramport[clk]
  // CHECK: [[CLOCK_W:%.*]] = firrtl.wrapSink [[CLOCK]]
  // CHECK: firrtl.strictconnect [[CLOCK_W]], %invalid_clock
  ...
  // CHECK:   [[ADDR_W:%.*]] = firrtl.wrapSink [[ADDR]]
  // CHECK:   firrtl.strictconnect [[ADDR_W]], %addr
  // CHECK:   [[EN_W:%.*]] = firrtl.wrapSink [[EN]]
  // CHECK:   firrtl.strictconnect [[EN_W]], %c1_ui1
  // CHECK:   [[CLOCK_W:%.*]] = firrtl.wrapSink [[CLOCK]]
  // CHECK:   firrtl.strictconnect [[CLOCK_W]], %clock
  // CHECK: }
...
firrtl.module @InferWrite(in %cond: !firrtl.uint<1>, in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in : !firrtl.uint<1>) {
  ...
  // CHECK: firrtl.strictconnect [[DATA_W]], %invalid_ui1
  // CHECK: [[MASK:%.*]] = firrtl.subfield %ram_ramport[mask]
  // CHECK: [[MASK_W:%.*]] = firrtl.wrapSink [[MASK]]
  // CHECK: firrtl.strictconnect [[MASK_W]], %invalid_ui1
  ...
  // CHECK:   firrtl.strictconnect [[CLOCK_W]], %clock
  // CHECK:   [[MASK_W:%.*]] = firrtl.wrapSink [[MASK]]
  // CHECK:   firrtl.strictconnect [[MASK_W]], %c0_ui1
  // CHECK: }
  firrtl.when %cond : !firrtl.uint<1> {
    chirrtl.memoryport.access %ramport_port[%addr], %clock : !chirrtl.cmemoryport, !firrtl.uint<8>, !firrtl.clock
  }
  ...
  // CHECK: [[MASK_W:%.*]] = firrtl.wrapSink [[MASK]]
  // CHECK: firrtl.strictconnect [[MASK_W]], %c1_ui1
...
firrtl.module @InferReadWrite(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in : !firrtl.uint<1>, out %out : !firrtl.uint<1>) {
  ...
  // CHECK: [[ADDR:%.*]] = firrtl.subfield %ram_ramport[addr]
  // CHECK: [[ADDR_W:%.*]] = firrtl.wrapSink [[ADDR]]
  // CHECK: firrtl.strictconnect [[ADDR_W]], %invalid
  // CHECK: [[EN:%.*]] = firrtl.subfield %ram_ramport[en]
  // CHECK: [[EN_W:%.*]] = firrtl.wrapSink [[EN]]
  // CHECK: firrtl.strictconnect [[EN_W]], %c0_ui1
  // CHECK: [[CLOCK:%.*]] = firrtl.subfield %ram_ramport[clk]
  ...
  // CHECK: [[WMODE_W:%.*]] = firrtl.wrapSink [[WMODE]]
  // CHECK: firrtl.strictconnect [[WMODE_W]], %c0_ui1
  // CHECK: [[WDATA:%.*]] = firrtl.subfield %ram_ramport[wdata]
  // CHECK: [[WDATA_W:%.*]] = firrtl.wrapSink [[WDATA]]
  // CHECK: firrtl.strictconnect [[WDATA_W]], %invalid_ui1
  // CHECK: [[WMASK:%.*]] = firrtl.subfield %ram_ramport[wmask]
  // CHECK: [[WMASK_W:%.*]] = firrtl.wrapSink [[WMASK]]
  // CHECK: firrtl.strictconnect [[WMASK_W]], %invalid
  %ram = chirrtl.combmem : !chirrtl.cmemory<uint<1>, 256>

  // CHECK: [[ADDR_W:%.*]] = firrtl.wrapSink [[ADDR]]
  // CHECK: firrtl.strictconnect [[ADDR_W]], %addr : !firrtl.uint<8>
  // CHECK: [[EN_W:%.*]] = firrtl.wrapSink [[EN]]
  // CHECK: firrtl.strictconnect [[EN_W]], %c1_ui1
  // CHECK: [[CLOCK_W:%.*]] = firrtl.wrapSink [[CLOCK]]
  // CHECK: firrtl.strictconnect [[CLOCK_W]], %clock
  // CHECK: [[WMASK_W:%.*]] = firrtl.wrapSink [[WMASK]]
  // CHECK: firrtl.strictconnect [[WMASK_W]], %c0_ui1
  ...
  // CHECK: [[WMASK_W:%.*]] = firrtl.wrapSink [[WMASK]]
  // CHECK: firrtl.strictconnect [[WMASK_W]], %c1_ui1
  // CHECK: [[WMODE_W:%.*]] = firrtl.wrapSink [[WMODE]]
  // CHECK: firrtl.strictconnect [[WMODE_W]], %c1_ui1
...
firrtl.module @WriteToSubfield(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %value: !firrtl.uint<1>) {
  ...
  // CHECK: [[MASK_B_W:%.*]] = firrtl.wrapSink [[MASK_B]]
  // CHECK: firrtl.strictconnect [[MASK_B_W]], %c1_ui1
...
firrtl.module @ReadAndWriteToSubfield(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in: !firrtl.uint<1>, out %out: !firrtl.uint<1>) {
  ...
  // CHECK: [[WMASK_A_W:%.*]] = firrtl.wrapSink [[WMASK_A]]
  // CHECK: firrtl.strictconnect [[WMASK_A_W]], %c1_ui1
  // CHECK: [[WMODE_W:%.*]] = firrtl.wrapSink [[WMODE]]
  // CHECK: firrtl.strictconnect [[WMODE_W]], %c1_ui1
...
firrtl.module @ReadAndWriteToSubindex(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in: !firrtl.uint<1>, out %out: !firrtl.uint<1>) {
  ...
  // CHECK: [[WMASK_0_W:%.*]] = firrtl.wrapSink [[WMASK_0]]
  // CHECK: firrtl.strictconnect [[WMASK_0_W]], %c1_ui1 : !firrtl.uint<1>
  // CHECK: [[WMODE_W:%.*]] = firrtl.wrapSink [[WMODE]]
  // CHECK: firrtl.strictconnect [[WMODE_W]], %c1_ui1 : !firrtl.uint<1>
...
firrtl.module @EnableInference0(in %p: !firrtl.uint<1>, in %addr: !firrtl.uint<4>, in %clock: !firrtl.clock, out %v: !firrtl.uint<32>) {
  ...
  firrtl.when %p : !firrtl.uint<1> {
    // CHECK: [[EN_W:%.*]] = firrtl.wrapSink [[EN]]
    // CHECK-NEXT: firrtl.strictconnect [[EN_W]], %c1_ui1
...
firrtl.module @EnableInference1(in %p: !firrtl.uint<1>, in %addr: !firrtl.uint<4>, in %clock: !firrtl.clock, out %v: !firrtl.uint<32>) {
  ...
  // CHECK: [[ADDR:%.*]] = firrtl.subfield %ram_ramport[addr]
  // CHECK: [[EN:%.*]] = firrtl.subfield %ram_ramport[en]
  // CHECK: [[CLK:%.*]] = firrtl.subfield %ram_ramport[clk]
  // CHECK: [[DATA:%.*]] = firrtl.subfield %ram_ramport[data]
  // CHECK: firrtl.when %p : !firrtl.uint<1>
  firrtl.when %p : !firrtl.uint<1> {
   // CHECK: [[EN_W:%.*]] = firrtl.wrapSink [[EN]]
   ...
   // CHECK: [[ADDR_W:%.*]] = firrtl.wrapSink [[ADDR]]
   // CHECK-NEXT: firrtl.strictconnect [[ADDR_W]], %n
   // CHECK: [[CLK_W:%.*]] = firrtl.wrapSink [[CLK]]
   // CHECK-NEXT: firrtl.strictconnect [[CLK_W]], %clock
   // CHECK-NEXT: firrtl.connect %v, [[DATA]]
...
firrtl.module @AddressLargerThanPort(in %clock: !firrtl.clock, in %addr: !firrtl.uint<3>, out %out: !firrtl.uint<1>) {
  ...
  // CHECK: [[ADDR_W:%.*]] = firrtl.wrapSink [[ADDR]]
  // CHECK: firrtl.strictconnect [[ADDR_W]], [[TRUNC]]
