...
// expected-error @below {{by-reference DPI ports must not have 'in' direction}}
sim.func.dpi @dpi_bad_byref(in %arg : !llvm.ptr {sim.func.dpi.byref})

// -----

// expected-error @below {{by-reference DPI ports must use !llvm.ptr type}}
sim.func.dpi @dpi_bad_byref_type(out arg : i32 {sim.func.dpi.byref})

// -----

// expected-error @below {{explicitly returned port must have 'out' direction}}
sim.func.dpi @dpi_bad_return_dir(in %arg: i1 {sim.func.explicitly_returned})

// -----

// expected-error @below {{explicitly returned port must be the final port}}
sim.func.dpi @dpi_bad_return(out ret: i1 {sim.func.explicitly_returned}, out other: i1)

// -----

sim.func.dpi @dpi_sig(in %a: i1, out ret: i1 {sim.func.explicitly_returned})
