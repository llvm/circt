...
// CHECK-DAG:      %[[FD:.*]] = sim.fmt.dec %[[ARG]], width 10, signed false : i32
// CHECK-DAG:      %[[FH:.*]] = sim.fmt.hex %[[ARG]], width 8, isUpper false : i32
...
// CHECK-DAG:      %[[H0:.*]] = sim.fmt.hex %[[ARG0]], width 2, isUpper false : i8
// CHECK-DAG:      %[[O0:.*]] = sim.fmt.oct %[[ARG0]], width 3 : i8
// CHECK-DAG:      %[[B1:.*]] = sim.fmt.bin %[[ARG1]], width 8 : i8
// CHECK-DAG:      %[[H1:.*]] = sim.fmt.hex %[[ARG1]], width 2, isUpper false : i8
// CHECK-DAG:      %[[O1:.*]] = sim.fmt.oct %[[ARG1]], width 3 : i8
// CHECK-DAG:      %[[B2:.*]] = sim.fmt.bin %[[ARG2]], width 8 : i8
// CHECK-DAG:      %[[H2:.*]] = sim.fmt.hex %[[ARG2]], width 2, isUpper false : i8
...
hw.module @multi_args(in %clk : !seq.clock, in %a: i8, in %b: i8, in %c: i8) {
  ...
  %bina = sim.fmt.bin %a, width 8 : i8
  %binb = sim.fmt.bin %b, width 8 : i8
  %binc = sim.fmt.bin %c, width 8 : i8

  %hexa = sim.fmt.hex %a, width 2, isUpper false : i8
  %hexb = sim.fmt.hex %b, width 2, isUpper false : i8
  %hexc = sim.fmt.hex %c, width 2, isUpper false : i8

  %octa = sim.fmt.oct %a, width 3 : i8
  %octb = sim.fmt.oct %b, width 3 : i8
  %octc = sim.fmt.oct %c, width 3 : i8
...
// CHECK-DAG:      %[[FA:.*]] = sim.fmt.hex %[[ARGA]], width 8, isUpper false : i32
...
// CHECK-DAG:      %[[FB:.*]] = sim.fmt.hex %[[ARGB]], width 8, isUpper false : i32
...
// CHECK-DAG:      %[[FC:.*]] = sim.fmt.hex %[[ARGC]], width 8, isUpper false : i32
...
hw.module @sequence(in %clk: !seq.clock, in %conda: i1, in %condb: i1, in %val : i32) {
  ...
  %cen = sim.fmt.bin %val, width 32 : i32
...
hw.module @condition_as_val(in %clk: !seq.clock, in %condval: i1) {
    %bin = sim.fmt.bin %condval, width 1 : i1
