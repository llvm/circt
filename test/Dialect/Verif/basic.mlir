...
verif.formal sym @formal1(k : 20 type : bmc) {
  ...
  %clk_i = seq.from_clock %clk 
  // CHECK: %[[CLK_U:.*]] = comb.xor %[[CLK_I]], %true : i1
  %clk_update = comb.xor %clk_i, %true : i1
  // CHECK: %[[CLK:.*]] = verif.concrete_input %true, %[[CLK_U]] : !seq.clock
  %clk = verif.concrete_input %true, %clk_update : (i1, i1) -> !seq.clock
  // CHECK:  hw.instance "foo" @Foo(" d*": %foo: i1, "": %foo: i1) -> ("": i1, "1": i1)
  %0, %1 = hw.instance "foo" @Foo("0": %sym: i1, "clk": %clk : !seq.clock)  -> ("" : i1, "1" : i1)
}
