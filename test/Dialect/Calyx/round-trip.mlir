...
module attributes {calyx.entrypoint = "A"} {
  // CHECK: hw.module.extern @prim(input %in : i32, output out : i32) attributes {filename = "test.v"}
  hw.module.extern @prim(input %in : i32, output out : i32) attributes {filename = "test.v"}

  // CHECK: hw.module.extern @params<WIDTH: i32>(input %in : !hw.int<#hw.param.decl.ref<"WIDTH">>, output out : !hw.int<#hw.param.decl.ref<"WIDTH">>) attributes {filename = "test.v"}
...
// CHECK: module attributes {calyx.entrypoint = "A"} {
module attributes {calyx.entrypoint = "A"} {
  // CHECK: hw.module.extern @params<WIDTH: i32>(input %in : !hw.int<#hw.param.decl.ref<"WIDTH">>, input %clk : i1 {calyx.clk}, input %go : i1 {calyx.go}, output out : !hw.int<#hw.param.decl.ref<"WIDTH">>, output done : i1 {calyx.done}) attributes {filename = "test.v"}
