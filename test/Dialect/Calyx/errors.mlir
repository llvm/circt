...
module attributes {calyx.entrypoint = "A"} {
  hw.module.extern @params<WIDTH: i32>(input %in: !hw.int<#hw.param.decl.ref<"WIDTH">>, input %clk: i1 {calyx.clk}, input %go: i1 {calyx.go = 1}, output %out: !hw.int<#hw.param.decl.ref<"WIDTH">>, output %done: i1 {calyx.done}) attributes {filename = "test.v"}
...
// -----
module attributes {calyx.entrypoint = "A"} {
  hw.module.extern @params<WIDTH: i32>(input %in: !hw.int<#hw.param.decl.ref<"TEST">>, input %clk: i1 {calyx.clk}, input %go: i1 {calyx.go = 1}, output %out: !hw.int<#hw.param.decl.ref<"WIDTH">>, output %done: i1 {calyx.done}) attributes {filename = "test.v"}
