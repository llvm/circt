...
hir.def @MatmulKernel at %t(
  %A : !hir.memref<128*128*i32, packing=[1], r>, 
  %B : !hir.memref<128*128*i32, packing=[], r>, 
  %C : !hir.memref<128*128*i32, packing=[1], w>) -> (i32) {
  ...
  hir.for %i : i32 = %0 : !hir.const<i32> to %128 : !hir.const<i32> step %1 : !hir.const<i32> iter_time(%ti = %t){
    ...
    hir.unroll_for %j = 0 to 128 step 1 iter_time(%tj = %ti){
      ...
      %C_bus = hir.alloc() : !hir.wire<128*i32>
      hir.wire_write %0 to %C_bus[%0] at %tj offset %3 : !hir.const<i32> : (!hir.const<i32>, !hir.wire<128*i32>[!hir.const<i32>])
      ...
      %tk_end=hir.unroll_for %k = 0 to 128 step 1 iter_time(%tk = %tj tstep 1){
        ...
        %a = hir.mem_read %A[%i_delayed, %k] at %ti offset %k : !hir.const<i32> : !hir.memref<128*128*i32, packing=[1], r>[i32, !hir.const<i32>] -> i32 // hoist from j-loop
        %a_delayed = hir.delay %a by %j : !hir.const<i32> at %ti offset %k : !hir.const<i32> : i32 -> i32
        %b = hir.mem_read %B[%j, %k] at %tk : !hir.memref<128*128*i32, packing=[], r>[!hir.const<i32>, !hir.const<i32>] -> i32
        %ab = hir.call @mult(%a_delayed, %b) at %tk : (i32, i32) -> (i32)
        %c_prev = hir.wire_read %C_bus[%k] at %tk offset %3 : !hir.const<i32> : !hir.wire<128*i32>[!hir.const<i32>] -> i32
        ...
        hir.wire_write %c to %C_bus[%kPlus1] at %tk offset %4 : !hir.const<i32> : (i32, !hir.wire<128*i32>[!hir.const<i32>])
      }
