...
// RUN: circt-cocotb-driver.py --objdir=%T --topLevel=top --pythonModule=buffer_arg_init --pythonFolder="%S,%S/.." %t.sv 2>&1 | FileCheck %s
...
// RUN: circt-cocotb-driver.py --objdir=%T --topLevel=top --pythonModule=buffer_arg_init --pythonFolder="%S,%S/.." %t.sv %esi_prims 2>&1 | FileCheck %s
