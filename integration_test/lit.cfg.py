...
config.substitutions.append(("%TCL_PATH%", config.circt_src_root + "/build/lib/"))
config.substitutions.append(("%CIRCT_SOURCE%", config.circt_src_root))
config.substitutions.append(("%ESI_COLLATERAL_PATH%", config.esi_collateral_path))
...
# Enable btormc if it has been detected.
if config.boolector_path != "":
    tool_dirs.append(os.path.dirname(config.boolector_path))
    tools.append("btormc")
    config.available_features.add("btormc")
...
if config.vivado_path != "":
    ...
    config.substitutions.append(("%ieee-sim", os.path.join(config.vivado_path, "xsim")))
    config.substitutions.append(("%xsim%", os.path.join(config.vivado_path, "xsim")))
...
if config.questa_path != "":
    ...
    if "LM_LICENSE_FILE" in os.environ:
        llvm_config.with_environment("LM_LICENSE_FILE", os.environ["LM_LICENSE_FILE"])

    tool_dirs.append(config.questa_path)
    tools.append("vlog")
    tools.append("vsim")

    config.substitutions.append(("%questa", os.path.join(config.questa_path, "vsim")))
    config.substitutions.append(("%ieee-sim", os.path.join(config.questa_path, "vsim")))
...
if len(ieee_sims) > 1:
    ...
    config.substitutions = list(
        filter(
            lambda x: x[0] != "%ieee-sim" or x == ieee_sims[-1], config.substitutions
        )
    )
