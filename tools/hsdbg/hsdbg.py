...
class DotFile:
    ...
    def addAttributesToEdge(self, edge, attrs):
        ...
        self.modifiedDot[edge.line] = str(
            DotEdge(edge.src, edge.dst, edgeAttrs, edge.line, edge.customAttributes))
    ...
    def resolveVcdBundles(self):
        ...
        for basename, bundle in bundles.items():
            def resolveBundleSig(name):
                opt = [x for x in bundle if getDimensionless(
                    x).endswith(f"_{name}")]
                if len(opt) == 1:
                    return opt[0]
                return None
            ...
            if validSig and readySig:
                self.bundles.append(HandshakeBundle(
                    validSig, readySig, dataSig, vcdParent=self.vcdName, dotParent=self.dotName))
...
class HSDbg():
    ...
    def resolveDotToVCD(self):
        ...
        if len(topHier) != 1:
            raise Exception(
                "Expected a single module under the \"TOP\" module.")
        ...
        def indexModule(vcdHierName, dotHierName):
            ...
            signals = [s for s in items if s.replace(
                f"{vcdHierName}.", "").count(".") == 0]
            modules[vcdHierName] = IndexedModule(
                signals, dotHierName, vcdHierName)
        ...
        def recurseModuleHierarchy(vcdHierName, dotHierName, subHier):
            if subHier == {}:
                ...
                for module in subHier:
                    indexModule(vcdHierName, dotHierName)
                    recurseModuleHierarchy(joinHierName(
                        vcdHierName, module), joinHierName(
                        dotHierName, module), subHier[module])
    ...
    def updateGraph(self):
        ...
        subprocess.call(["dot", workingDotfile, "-Tsvg",
                         "-o", self.workingImagePath])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""A command line tool for visualizing handshake execution traces.
...
Legend:
    ...
    parser.add_argument("-p", "--port", help="The port to run the image server on. default='8080'",
    ...
    server = threading.Thread(
        target=lambda: runImageServer(args.port))
    server.start()
