...
%inp : !hir.interface<func()->(!hir.time,f32)>,
%outp :!hir.interface<group(!hir.time,array[2x2xf32])>){
  ...
  hir.for %i :i32 = %0 :!hir.const to %16 :!hir.const step %1 :!hir.const 
    ...
    %tf=hir.for %j :i32 = %0 :!hir.const to %16 :!hir.const step %1 :!hir.const 
      ...
      %tv, %v = hir.call %inp() at %tj 
        :!hir.interface<func() -> (!hir.time,f32)>
      ...
      hir.unroll_for %k1 = 0 to 1 step 1 iter_time(%tk1 = %tv){
        ...
        hir.send %val to %outp[%1,%k1,%0] at %tv + %1
          :f32 to
          !hir.interface<group(!hir.time,array[2x2xf32])>[!hir.const,!hir.const,!hir.const]
      }
      ...
      hir.send %t_send to %outp[%0] at %t_send
        :!hir.time to
        !hir.interface<group(!hir.time,array[2x2xf32])>[!hir.const]
