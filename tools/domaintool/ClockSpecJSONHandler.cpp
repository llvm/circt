...
public:
  ...
  LogicalResult emit() override {
    // TODO: This needs to be better configurable.  This won't make sense if
    // there are multiple handlers all generating different output.
    json::OStream json(llvm::outs(), /*indentSize=*/2);
    json.object([&] {
      ...
      json.attributeArray("synchronous_ports", [&] {
        for (auto &[_, syncPort] : syncPorts) {
          ...
          json.object([&] {
            ...
            json.attributeArray("name_pattern", [&] {
              for (auto port : ports)
                json.value(port);
            });
