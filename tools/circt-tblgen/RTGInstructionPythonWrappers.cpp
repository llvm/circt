...
static cl::opt<std::string>
    opPrefix("op-prefix",
             cl::desc("Prefix for MLIR operation calls (e.g., 'rtgtest')"),
             cl::cat(rtgPythonWrapperCat));
...
static StringRef getTypeSuffix(OperandType type) {
...
static void sanitizePythonFunctionName(std::string &name) {
  // List of Python keywords that cannot be used as function names
  static const char *pythonKeywords[] = {
      "and",  "as",     "assert", "break",  "class", "continue", "def",
      "del",  "elif",   "else",   "except", "False", "finally",  "for",
      "from", "global", "if",     "import", "in",    "is",       "lambda",
      "None", "not",    "or",     "pass",   "raise", "return",   "True",
...
struct OpInfo {
  SmallVector<SmallVector<OperandType>> ope;
  SmallVector<StringRef> operandNames;
  SmallVector<SideEffect> operandSideEffects;
...
generateTypeCombinations(ArrayRef<OperandTypeSet> operandKinds,
  ...
  for (size_t i = 1; i < operandKinds.size(); ++i) {
    ...
    // If any operand has no kinds, the result is empty
    if (currentKinds.empty()) {
      combinations.clear();
      return;
    }
...
emitDispatcherSignature(StringRef opMnemonic,
  ...
  for (auto [i, name, types] : llvm::enumerate(operandNames, operandTypes)) {
    if (i != 0)
      os << ", ";
    ...
    os << "Union[";
    for (auto [i, ty] : llvm::enumerate(types)) {
      if (i != 0)
        os << ", ";
      emitPythonTypeHint(ty, os);
    }
...
static void emitDispatcherBody(StringRef opMnemonic,
  ...
  for (auto [i, combination] : llvm::enumerate(combinations)) {
    ...
    if (i < combinations.size() - 1) {
      for (auto opIdx : unionOperandIndices) {
        if (i > 0)
          os << " and ";

        os << "isinstance(" << operandNames[opIdx] << ", ";
        emitPythonTypeHint(combination[opIdx], os);
        os << ")";
      }
      os << ":\n";
    }
...
static bool genRTGInstructionPythonWrappers(const RecordKeeper &records,
  ...
  if (opPrefix.empty()) {
    llvm::errs() << "error: --op-prefix is required for "
                    "-gen-rtg-instruction-python-wrappers\n";
    return true;
  }
  ...
  if (!headerFile.empty()) {
    ...
    if (!fileOrErr.get()->getBuffer().ends_with("\n"))
      os << "\n";
  }
